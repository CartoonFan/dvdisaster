 ...
 * d) "nm"     -> choose redundancy so that .ecc file does not exceed n
 *megabytes
 */
...
static int calculate_redundancy(char *image_name) {
  ...
  if (Closure->redundancy) /* get last char of redundancy parameter */
  {
    ...
    if (len)
      last = Closure->redundancy[len - 1];
  }

  switch (last) {
  case '%':
    p = atof(Closure->redundancy);
    if (p < 3.2 || p > 64.5)
      Stop(_("Redundancy %4.1f%% out of useful range [3.2%%..64.5%%]"), p);
    nr = (int)round((GF_FIELDMAX * p) / (100.0 + p));
    break;

  case 'm':
    if (!LargeStat(image_name, &filesize)) {
    ...
    if (fs < ecc_file_size(sectors, 8) || fs > ecc_file_size(sectors, 100))
      Stop(_("Ecc file size %lldm out of useful range [%lld .. %lld]"), fs,
           ecc_file_size(sectors, 8), ecc_file_size(sectors, 100));
    for (nr = 100; nr > 8; nr--)
...
void RS01Create(void) {
  ...
  ec->msg = g_strdup_printf(
      _("Encoding with Method RS01: %d roots, %4.1f%% redundancy."), nroots,
  ...
  } else /* Scan image for missing sectors and calculate the checksums */
  {
    ...
    if (image->sectorsMissing) {
      LargeClose(image->eccFile); /* Will be deleted anyways; no need to test
       for errors */
      image->eccFile = NULL;

      LargeUnlink(
          Closure->eccName); /* Do not leave a CRC-only .ecc file behind */

      if (Closure->stopActions) {
        SetLabelText(GTK_LABEL(wl->encFootline),
                     _("<span %s>Aborted by user request!</span> (partial "
                       "error correction file removed)"),
                     Closure->redMarkup);
  ...
  */
  ...
  /* Each chunk of parity blocks is built iteratively by processing the data in
     ...
     processed). So one buffer of n_layer_bytes = n_parity_blocks needs to be
     buffered. For practical reasons we require that the layer size is a
     multiple of the medium sector size of 2048 bytes. */
  ...
  /* Process the image.
     ...
     So after (s/n_layer_sectors)+1 iterations the whole image has been
     processed. */
  ...
  for (chunk = 0; chunk < s; chunk += n_layer_sectors) {
    ...
    {
      int sp = 1; /* sp==1 makes sure sp==0 after ndata bytes [since (223+1) mod
32 = 0]*/
      ...
      for (layer = 0; layer < ndata; layer++) {
        ...
        if (Closure->stopActions) /* User hit the Stop button */
        {
          ...
          image->eccFile = NULL;
          LargeUnlink(
              Closure->eccName); /* Do not leave partial .ecc file behind */
          goto terminate;
        }
    ...
    {
      int sp = 1; /* sp==1 makes sure sp==0 after ndata bytes [since (191+1) mod
64 = 0] */
      for (layer = 0; layer < ndata; layer++) {
        int offset = 0;
        unsigned char *par_idx = ec->parity;
