...
void CreateRS02VerifyWindow(Method *self, GtkWidget *parent) {
  ...
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 1, 0, 1, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 2);
  lab = wl->cmpImageSectors = gtk_label_new("0");
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 1, 2, 0, 1, GTK_EXPAND | GTK_FILL,
                   GTK_SHRINK, 0, 0);

  lab = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  SetLabelText(GTK_LABEL(lab), _("Data checksum:"));
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 1, 1, 2, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 2);
  lab = wl->cmpImageMd5Sum = gtk_label_new("0");
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 1, 2, 1, 2, GTK_EXPAND | GTK_FILL,
                   GTK_SHRINK, 0, 0);

  lab = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  SetLabelText(GTK_LABEL(lab), _("Ecc headers:"));
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 1, 2, 3, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 2);
  lab = wl->cmpEccHeaders = gtk_label_new(".");
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 1, 2, 2, 3, GTK_EXPAND | GTK_FILL,
                   GTK_SHRINK, 0, 0);

  lab = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  SetLabelText(GTK_LABEL(lab), _("Data section:"));
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 1, 3, 4, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 2);
  lab = wl->cmpDataSection = gtk_label_new(".");
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 1, 2, 3, 4, GTK_EXPAND | GTK_FILL,
                   GTK_SHRINK, 0, 0);

  lab = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  SetLabelText(GTK_LABEL(lab), _("Crc section:"));
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 1, 4, 5, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 2);
  lab = wl->cmpCrcSection = gtk_label_new(".");
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 1, 2, 4, 5, GTK_EXPAND | GTK_FILL,
                   GTK_SHRINK, 0, 0);

  lab = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  SetLabelText(GTK_LABEL(lab), _("Ecc section:"));
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 1, 5, 6, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 2);
  lab = wl->cmpEccSection = gtk_label_new(".");
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 1, 2, 5, 6, GTK_EXPAND | GTK_FILL,
                   GTK_SHRINK, 0, 0);

  lab = wl->cmpImageResult = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 2, 6, 7, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 4);
  ...
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 1, 6, 7, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 2);
  lab = wl->cmpEcc3Msg = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 1, 2, 6, 7, GTK_EXPAND | GTK_FILL,
                   GTK_SHRINK, 0, 0);

  lab = wl->cmpEccResult = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 2, 7, 8, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 4);
}
...
static void read_crc(verify_closure *cc, RS02Layout *lay) {
  ...
  if (!LargeSeek(cc->image->file, 2048 * (lay->dataSectors + 2)))
    Stop(_("Failed seeking to sector %lld in image: %s"), lay->dataSectors + 2,
         strerror(errno));
    ...
    for (i = 0; i < lay->ndata; i++) {
      if (block_idx[i] < lay->dataSectors) /* only data sectors have CRCs */
      {
        ...
        if (crc_idx >= 512) {
          ...
          err = CheckForMissingSector((unsigned char *)crc_buf, crc_sector,
                                      eh->mediumFP, eh->fpSector);
          if (err != SECTOR_PRESENT)
            ExplainMissingSector((unsigned char *)crc_buf, crc_sector, err,
                                 TRUE);
...
static int prognosis(verify_closure *vc, gint64 missing, gint64 expected) {
  ...
  for (eccblock = 0; eccblock < vc->lay->sectorsPerLayer; eccblock++) {
    ...
    for (j = 0; j < 255; j++) {
      ...
      if (sector < 0 || sector == vc->lay->firstEccHeader ||
  ...
  /* Why the first test?
     if(damaged_sectors && worst_ecc <= vc->lay->nroots && recoverable >=
     expected) */
...
void RS02Verify(Image *image) {
  ...
  if (Closure->guiMode) {
    if (expected_sectors == image->sectorSize) {
      ...
      if (expected_sectors > image->sectorSize)
        img_advice = g_strdup_printf(_("<span %s>Image file is %lld sectors "
                                       "shorter than expected.</span>"),
                                     Closure->redMarkup,
                                     expected_sectors - image->sectorSize);
      else
        img_advice = g_strdup_printf(_("<span %s>Image file is %lld sectors "
                                       "longer than expected.</span>"),
                                     Closure->redMarkup,
                                     image->sectorSize - expected_sectors);
    }
  }
  ...
  while (hdr_pos < expected_sectors) {
    ...
    if (hdr_pos < image->sectorSize) {
      ...
      if (CheckForMissingSector((unsigned char *)&eh, hdr_pos, cc->eh->mediumFP,
                                cc->eh->fpSector))
        hdr_correctable++;
      if (CheckForMissingSector(2048 + ((unsigned char *)&eh), hdr_pos + 1,
                                cc->eh->mediumFP, cc->eh->fpSector))
        hdr_correctable++;
    ...
    if (hdr_pos == lay->firstEccHeader)
      hdr_pos = (lay->protectedSectors + lay->headerModulo - 1) &
                ~(lay->headerModulo - 1);
    else
      hdr_pos += lay->headerModulo;
    ...
    if (Closure->guiMode) {
      if (!hdr_crc_errors && !hdr_missing)
        ...
        SetLabelText(
            GTK_LABEL(wl->cmpEccHeaders),
            _("<span %s>%lld ok, %lld CRC errors, %lld missing</span>"),
  ...
  for (s = 0; s < expected_sectors; s++) {
    ...
    if (current_missing) {
      ...
      if (s < lay->dataSectors)
        data_missing++;
      else if (s >= lay->dataSectors + 2 && s < lay->protectedSectors)
        crc_missing++;
      else
        ecc_missing++;
      defective = TRUE;
    ...
    if (!current_missing || s == expected_sectors - 1) {
      if (first_missing >= 0) {
        if (first_missing == last_missing)
          ...
          PrintCLI(_("* missing sectors  : %lld - %lld\n"), first_missing,
                   last_missing);
        first_missing = -1;
      }
    }
  ...
  if (!total_missing && !hdr_missing && !hdr_crc_errors && !data_crc_errors)
    ...
    if (!total_missing && !total_crc_errors)
      ...
      if (!total_crc_errors)
        PrintLog(_("* BAD image        : %lld sectors missing\n"),
                 total_missing);
      if (!total_missing)
        PrintLog(_("* suspicious image : all sectors present, but %lld CRC "
                   "errors\n"),
                 total_crc_errors);
      if (total_missing && total_crc_errors)
        PrintLog(
    ...
    PrintLog(
        _("  ... ecc headers    : %lld ok, %lld CRC errors, %lld missing\n"),
...
continue_with_ecc:
  ...
  if (eh->creatorVersion % 100) {
    ...
    if (Closure->guiMode) {
      if (!color_format)
        SwitchAndSetFootline(wl->cmpEccNotebook, 1, wl->cmpEccCreatedBy,
                             color_format, "dvdisaster", major, minor,
                             Closure->redMarkup, pl);
      else
        SwitchAndSetFootline(wl->cmpEccNotebook, 1, wl->cmpEccCreatedBy, format,
  ...
  if (!VerifyVersion(eh, 0)) {
    PrintLog(_("- requires         : dvdisaster-%d.%d (good)\n"),
             eh->neededVersion / 10000, (eh->neededVersion % 10000) / 100);
    ...
    if (Closure->guiMode) {
      SetLabelText(GTK_LABEL(wl->cmpEccRequires),
                   "<span %s>dvdisaster-%d.%d</span>", Closure->redMarkup,
                   eh->neededVersion / 10000,
                   (eh->neededVersion % 10000) / 100);
      if (!ecc_advice)
  ...
  if (image->sectorSize == expected_sectors) {
    ...
    if (image->sectorSize > expected_sectors &&
        image->sectorSize - expected_sectors <= 2)
    ...
    if (Closure->guiMode) {
      SetLabelText(GTK_LABEL(wl->cmpEccMediumSectors), "<span %s>%lld</span>",
                   Closure->redMarkup, expected_sectors);
      if (!ecc_advice && image->sectorSize > expected_sectors)
        ecc_advice = g_strdup_printf(
            _("<span %s>Image size does not match recorded size.</span>"),
            Closure->redMarkup);
    }
  }
  ...
  if (!data_missing) {
    ...
    if (n)
      PrintLog(_("- data md5sum      : %s (good)\n"), hdr_digest);
    else
      PrintLog(_("* data md5sum      : %s (BAD)\n"), hdr_digest);

    if (Closure->guiMode) {
      if (n)
        SetLabelText(GTK_LABEL(wl->cmpEcc1Msg), "%s", hdr_digest);
      else {
        SetLabelText(GTK_LABEL(wl->cmpEcc1Msg), "<span %s>%s</span>",
                     Closure->redMarkup, hdr_digest);
        SetLabelText(GTK_LABEL(wl->cmpImageMd5Sum), "<span %s>%s</span>",
                     Closure->redMarkup, data_digest);
      }
    }
  } else {
    PrintLog(_("- data md5sum      : %s\n"), "-");
  ...
  if (!crc_missing) {
    if (!memcmp(eh->crcSum, cc->crcSum, 16)) {
      ...
      if (Closure->guiMode) {
        SetLabelText(GTK_LABEL(wl->cmpEcc2Msg), "<span %s>%s</span>",
                     Closure->redMarkup, digest);
      }
  ...
  if (!ecc_missing) {
    if (!memcmp(eh->eccSum, ecc_sum, 16)) {
      ...
      if (Closure->guiMode) {
        SetLabelText(GTK_LABEL(wl->cmpEcc3Msg), "<span %s>%s</span>",
                     Closure->redMarkup, digest);
      }
      ecc_md5_failure = TRUE;
    }
  ...
  try_it = prognosis(cc, total_missing + data_crc_errors - hdr_correctable,
                     expected_sectors);
  ...
  if (Closure->guiMode) {
    if (ecc_advice) {
      ...
      g_free(ecc_advice);
    } else if (!crc_missing && !ecc_missing && !hdr_missing &&
               !hdr_crc_errors && !ecc_md5_failure)
