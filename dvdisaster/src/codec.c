  ...
  while (size--) {
    ...
    if (i == t_used) {
      ...
      if (t_used == t_max) {
        ...
        if (!color && !count)
          Stop("out of memory enlarging color table to %d", t_max);
      }
...
static void add_image(Image *pi) {
  if (++img_n >= img_max) {
    ...
    if (!img_list)
      Stop("out of memory expanding image list");
  }
...
static int add_tile(tile *new_tile, int x, int y) {
  ...
  if (new_needed) {
    ...
    if (++db_n >= db_max) {
      ...
      if (!tile_db)
        Stop("out of memory expanding tile data base");
    }

    tile_db[idx] = malloc(sizeof(tile));
    if (!tile_db[idx])
      Stop("out of memory adding tile to the data base");
  ...
  if (++oc_n >= oc_max) {
    ...
    if (!oc_list)
      Stop("out of memory expanding opcode table");
  }
...
static void create_tile(Image *pi, work_tile *t, unsigned int x,
                        unsigned int y) {
  /* Stop recursion if background or maximal tile size is reached */

  if ((x < t->minx || x > t->maxx) && t->maxx - t->minx >= MAX_TILE_SIZE - 1)
    return;

  if (y < t->basey)
    return;
  if (y - t->basey >= MAX_TILE_SIZE)
    return;
  ...
  if (x < t->minx) {
    t->minx = x;
    t->width = t->maxx - t->minx + 1;
  }
  if (x > t->maxx) {
    t->maxx = x;
    t->width = t->maxx - t->minx + 1;
  }
  if (y - t->basey + 1 > t->height)
    t->height = y - t->basey + 1;
  ...
  if (x > 0)
    create_tile(pi, t, x - 1, y);
  if (x < pi->width - 1)
    create_tile(pi, t, x + 1, y);
  if (y > 0)
    create_tile(pi, t, x, y - 1);
  if (y < pi->height - 1)
    create_tile(pi, t, x, y + 1);
}
...
void SavePPK(char *name) {
  ...
  /* The header contains the string ".pngpack", four zeros and the FILEFORMAT as
   * a 4 byte value. */
...
void LoadPPK(char *name, Image ***list_out, int *n_out) {
  ...
  if (!img_list)
    Stop("out of memory allocating the image table");
  ...
  for (i = 0; i < img_n; i++) {
    ...
    if (!pi)
      Stop("out of memory allocating image structure");
    ...
    if (!pi->name)
      Stop("out of memory allocating image name");
  ...
  if (!oc_list)
    Stop("out of memory allocating the opcode table");
  ...
  if (!tile_db)
    Stop("out of memory allocating the tile data base");
  ...
  for (i = 0; i < db_n; i++) {
    ...
    if (!t)
      Stop("out of memory allocating more tiles");
    tile_db[i] = t;
...
void RenderImage(Image *pi) {
  ...
  if (!memcmp(pi->checksum, checksum, 16))
    ...
    fprintf(stdout, " - DECODING FAILURE (checksum error).\n");
}
