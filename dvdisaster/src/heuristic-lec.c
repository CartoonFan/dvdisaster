...
void CalculatePQLoad(RawBuffer *rb) {
  ...
  for (q = 0; q < N_Q_VECTORS; q++) {
    ...
    if (err < 0)
      rb->qLoad[frame_idx] += 2;
    if (err == 1)
      rb->qLoad[frame_idx]++; /* We assume without any erasures specified there
   can't be more than 1 errors corrected. */
  }
  ...
  for (p = 0; p < N_P_VECTORS; p++) {
    ...
    if (err < 0)
      rb->pLoad[frame_idx] += 2;
    if (err == 1)
      rb->pLoad[frame_idx]++; /* We assume without any erasures specified there
   can't be more than 1 errors corrected. */
  }
}
...
int HeuristicLEC(unsigned char *cd_frame, RawBuffer *rb, unsigned char *out) {
  ...
  for (;;) /* iterate over P- and Q-Parity until failures converge */
  {
    ...
    for (p = 0; p < N_P_VECTORS; p++) {
      ...
      /* Erasure markings are overly pessimistic as each failing vector will
         mark a full row as an erasure. We assume that there are only 2 real
         erasures and try all combinations. The case for 2 erasure is also
         included here. */
    ...
    Verbose(
        "      P-failures/corrected/errors + decimated: %2d/%2d/%2d + %2d\n",
        p_failures, p_corrected, p_err, p_decimated);
    Verbose(
        "      Q-failures/corrected/errors + decimated: %2d/%2d/%2d + %2d\n",
        q_failures, q_corrected, q_err, q_decimated);
#endif

    if (p_failures + p_err + q_failures + q_err == 0)
      break;
    if (last_p_err <= p_err && last_q_err <= q_err &&
        last_p_failures <= p_failures && last_q_failures <= q_failures)
      break;

    if (iteration > N_P_VECTORS + N_Q_VECTORS)
      break;
...
static void initialize_frame(RawBuffer *rb) {
  ...
  memcpy(&(rb->recovered[16]), &(rb->rawBuf[best_sector][16]),
         rb->sampleSize - 16);
  if (!rb->xaMode)
  ...
  for (;;) /* iterate over P- and Q-Parity until failures converge */
  {
    ...
    for (q = 0; q < N_Q_VECTORS; q++) {
      ...
      if (err == 1 || err < 0) {
        /* If Q is correctable. */
        if (err == 1) {
          /* Check correction for plausibility: if corrected byte was read in
           * one of the read attempts. */
          if (check_q_plausibility(rb, q_vector, q, -1, -1)) {
            ...
            /* See whether we can find some Q parity bytes accepting the
             * original Q vector. */
        ...
        /* If correction is not plausible or possible then try 2
         * error-decimation. */
        if (err < 0) {
          /* Try error correction with decimated erasures.
          ...
          for (a = 0; a < Q_VECTOR_SIZE - 2; a++) {
            ...
            for (b = a + 1; b < Q_VECTOR_SIZE - 2; b++) {
              if (cq_vector[b] == 1)
                continue; /* again, no alternatives present */
      ...
      if (err == 1 || err < 0) {
        ...
        if (err == 1) {
          ...
          if (check_p_plausibility(rb, p_vector, p, -1, -1)) {
            ...
            /* See whether we can find some P parity bytes accepting the
             * original P vector. */
        ...
        if (err < 0) {
          ...
          for (a = 0; a < P_VECTOR_SIZE - 2; a++) /* fixme: why -2? */
          {
            ...
            for (b = a + 1; b < P_VECTOR_SIZE - 2; b++) /* fixme: why -2? */
            {
              ...
              if (err == 2) {
                if (check_p_plausibility(rb, p_vector, p, a, b)) {
                  solFound = TRUE;
                  break;
                }
              }
...
int does_P_ack(RawBuffer *rb, int q, int qpos, unsigned char *val,
  ...
  if (err == 0) {
    ...
    if (!check_q_plausibility(rb, tq_vector, q, qpos, -1) &&
        check_q_plausibility(rb, q_vector, q, qpos, -1))
      return 1;
  }
  ...
  if (err == 1) {
    ...
    if (tpos == ppos) {
      ...
      /* provisoric heuristic solution: take the version which was already read,
       * if it was already read */
      else {
        if (check_q_plausibility(rb, q_vector, q, qpos, -1)) {
          return 1;
        }
        if (check_p_plausibility(rb, p_vector, p, ppos, -1)) {
          *val = p_vector[ppos];
          return 1;
        }
      }
  ...
  /* p failure... we have to try all 2-erasure combinations containing this
   * position */
  if (err < 0)
    tryAllErasures = 1;
  ...
  if (tryAllErasures) {
    ...
    for (a = 0; a < P_VECTOR_SIZE; a++) {
      ...
      if (q_status != NULL) {
        /* if the corresponding q does not show an error we should not try to
         * correct it */
        idx = PToByteIndex(p, a);
        ByteIndexToQ(idx, &qi, &qposi);
        if (q_status[qi] == 0)
          continue;
      }
...
int does_Q_ack(RawBuffer *rb, int p, int ppos, unsigned char *val,
  ...
  if (err == 0) {
     ...
     */
    /*   then we will take the corrected byte */
    if (!check_p_plausibility(rb, tp_vector, p, ppos, -1) &&
        check_p_plausibility(rb, p_vector, p, ppos, -1))
      return 1;
  }
  ...
  if (err == 1) {
    ...
    if (tpos == ppos) {
       ...
       * if it was already read */
        ...
        if (check_p_plausibility(rb, p_vector, p, qpos, -1)) {
          return 1;
        }
        if (check_q_plausibility(rb, q_vector, q, ppos, -1)) {
          *val = q_vector[qpos];
          return 1;
        }
      }
  ...
  /* q failure... we have to try all 2-erasure combinations containing this
   * position */
  if (err < 0)
    tryAllErasures = 1;
  ...
  if (tryAllErasures) {
    ...
    for (a = 0; a < Q_VECTOR_SIZE; a++) {
      ...
      if (p_status != NULL) {
        /* if the corresponding p does not show an error we should not try to
         * correct it */
        idx = QToByteIndex(q, a);
        ByteIndexToP(idx, &pi, &pposi);
        if (p_status[pi] == 0)
          continue;
      }
    ...
    for (q = 0; q < N_Q_VECTORS; q++) {
      ...
      if (err == 1) {
        ...
        if (qpos == 43 || qpos == 44) {
          ...
          if (does_P_ack(rb, q, qpos, &q_vector[qpos], q_status)) {
            q_corrected++;
            SetQVector(rb->recovered, q_vector, q);
          }
        }
      ...
      /* q failure... we have to try all 2-erasure combinations of q and check p
       * (try all 2-erasure combinations of p if necessary) */
      if (err < 0) {
        int a, b;
        int manySols = 0;
        int undecidable = 0;
        ...
        for (i = 0; i < 45 - 2; i++) {
          ...
          /* p failure... we have to try all 2-erasure combinations containing
           * this position */
          if (err < 0)
            tryAllErasures = 1;
          ...
          if (tryAllErasures) {
            for (a = 0; a < P_VECTOR_SIZE; a++) {
              ...
              /* if this q does not show an error we should not try to correct
               * it */
              if (q_status[qi] == 0) {
        ...
        for (a = 0; a < 45 - 1; a++) {
          if (a < 45 - 2 && !qp_sector_exist[a])
            continue; /* if p reported no problems for first byte then we cannot
do much here */
          ...
          for (b = a + 1; b < 45; b++) {
            ...
            if (b < 45 - 2 && !qp_sector_exist[b])
              continue; /* if p reported no problems for second byte then we
cannot do much here */
            ...
            if (err == 2) {
              ...
              int p1, p2;
              idx = QToByteIndex(q, a);
              ByteIndexToP(idx, &p1, &ppos1);
              idx = QToByteIndex(q, b);
              ByteIndexToP(idx, &p2, &ppos2);
              ...
              /* both bytes were acknowledged by the respective ps so save the
               * change into temporary parameters */
              if (compFound & 0x01 || compFound & 0x02 || compFound & 0x04 ||
                  compFound & 0x08) {
                if (aMul <= 1 && bMul <= 1) {
                  /* FIXME: maybe error: did original code really want "+"
                   * precede "&"? */
#if 0 /* original code */
...
#endif
                  int quality = (compFound & 0x03) + (compFound & 0x04) * 2 +
                                (compFound & 0x08) * 2;
                  int bestQuality = (bestCompFound & 0x03) +
                                    (bestCompFound & 0x04) * 2 +
                                    (bestCompFound & 0x08) * 2;
                  ...
                  if (compFound & 0x01 && !(compFound & 0x04)) {
                    ...
                    if (does_Q_ack(rb, p1, pSol1, &c, NULL))
                      P1_ACK = 1;

                    p_vector[pSol1] = qp_sector[a][pSol1 * 2 + 1];
                    if (check_p_plausibility(rb, p_vector, p1, pSol1, -1))
                      P1_ACK = 1;

                    if (check_q_plausibility(rb, q_vector, q, a, -1))
                      P1_ACK = 1;
                  }

                  if (compFound & 0x02 && !(compFound & 0x08)) {
                    c = qp_sector[b][pSol2 * 2 + 1];
                    if (does_Q_ack(rb, p2, pSol2, &c, NULL))
                      P2_ACK = 1;

                    p_vector[pSol2] = qp_sector[b][pSol2 * 2 + 1];
                    if (check_p_plausibility(rb, p_vector, p2, pSol2, -1))
                      P2_ACK = 1;

                    if (check_q_plausibility(rb, q_vector, q, b, -1))
                      P2_ACK = 1;
                  }

                  if (!P1_ACK && !P2_ACK && !(compFound & 0x04) &&
                      !(compFound & 0x08))
                    quality = 0;
              ...
              if (a == 45 - 2) {
                if (check_q_plausibility(rb, q_vector, q, a, b)) {
                  /* this solution has low priority, because here we have no
                   * acknowledging p */
                  if (manySols == 0) {
                    bestA = a;
                    bestB = b;
        ...
        if (!undecidable && manySols > 0) {
          ...
          idx = QToByteIndex(q, bestA);
          qquality += rb->recovered[idx] == rb->reference[idx];
          idx = QToByteIndex(q, bestB);
          qquality += rb->recovered[idx] == rb->reference[idx];

          byteBadProbACK[1][bestCompFound & 0x03]
                        [((bestCompFound & 0x04) >> 2) +
                         ((bestCompFound & 0x08) >> 2)][!(qquality == 2)]++;
    ...
    for (p = 0; p < N_P_VECTORS; p++) {
      ...
      if (err == 1) {
        ...
        if (does_Q_ack(rb, p, ppos, &p_vector[ppos], p_status)) {
          p_corrected++;
          SetPVector(rb->recovered, p_vector, p);
        }
      }

      /* p failure... we have to try all 2-erasure combinations of q and check p
       * (try all 2-erasure combinations of q if necessary) */
      if (err < 0) {
        int a, b;
        int manySols = 0;
        int undecidable = 0;
        ...
        for (i = 0; i < 26; i++) {
          ...
          if (err == 1) {
            ...
            else if (tpos != 43 && tpos != 44)
              tryAllErasures = 1;
          }

          /* q failure... we have to try all 2-erasure combinations containing
           * this position */
          if (err < 0)
            tryAllErasures = 1;
        ...
        for (a = 0; a < 26 - 1; a++) {
          if (!pq_sector_exist[a])
            continue; /* if q reported no problems for first byte then we cannot
do much here */
          ...
          for (b = a + 1; b < 26; b++) {
            ...
            if (!pq_sector_exist[b])
              continue; /* if q reported no problems for second byte then we
cannot do much here */
            ...
            if (err == 2) {
              ...
              int q1, q2;
              idx = PToByteIndex(p, a);
              ByteIndexToQ(idx, &q1, &qpos1);
              idx = PToByteIndex(p, b);
              ByteIndexToQ(idx, &q2, &qpos2);
                ...
                if (aMul <= 1 && bMul <= 1) {
                  ...
                  if (compFound & 0x01 && !(compFound & 0x04)) {
                    if (qSol1 < 43) {
                      ...
                      if (does_P_ack(rb, q1, qSol1, &c, NULL))
                        Q1_ACK = 1;
                    }

                    q_vector[qSol1] = pq_sector[a][qSol1 * 2 + 1];
                    if (check_q_plausibility(rb, q_vector, q1, qSol1, -1))
                      Q1_ACK = 1;

                    if (check_p_plausibility(rb, p_vector, p, a, -1))
                      Q1_ACK = 1;
                  }
                  ...
                  if (compFound & 0x02 && !(compFound & 0x08)) {
                    if (qSol2 < 43) {
                      ...
                      if (does_P_ack(rb, q2, qSol2, &c, NULL))
                        Q2_ACK = 1;
                    }

                    q_vector[qSol2] = pq_sector[b][qSol2 * 2 + 1];
                    if (check_q_plausibility(rb, q_vector, q2, qSol2, -1))
                      Q2_ACK = 1;

                    if (check_p_plausibility(rb, p_vector, p, b, -1))
                      Q2_ACK = 1;
                  }

                  if (!Q1_ACK && !Q2_ACK && !(compFound & 0x04) &&
                      !(compFound & 0x08))
                    quality = 0;
        ...
        if (!undecidable && manySols > 0) {
          ...
          idx = PToByteIndex(p, bestA);
          pquality += rb->recovered[idx] == rb->reference[idx];
          idx = PToByteIndex(p, bestB);
          pquality += rb->recovered[idx] == rb->reference[idx];

          byteBadProbACK[1][bestCompFound & 0x03]
                        [((bestCompFound & 0x04) >> 2) +
                         ((bestCompFound & 0x08) >> 2)][!(pquality == 2)]++;
...
int BruteForceSearchPlausibleSector(RawBuffer *rb) {
  ...
  unsigned char zList[45][256]; /* stores different bytes which were read for
                         each position in a sector */
  unsigned char czList[45]; /* counts different bytes which were read for each
