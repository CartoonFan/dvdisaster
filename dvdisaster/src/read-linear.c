...
static void cleanup(gpointer data) {
  ...
  if (g_thread_self() == rc->worker) {
    g_printf(
        "Reading/Scanning terminated from worker thread - trouble ahead\n");
    return;
  }
  ...
  if (Closure->guiMode) {
    if (rc->unreportedError)
      SwitchAndSetFootline(
          Closure->readLinearNotebook, 1, Closure->readLinearFootline,
          _("<span %s>Aborted by unrecoverable error.</span> %lld sectors "
            "read, %lld sectors unreadable/skipped so far."),
  ...
  if (rc->canWrite)
    g_cond_free(rc->canWrite);
  if (rc->workerError)
    g_free(rc->workerError);
  ...
  if (rc->msg)
    g_free(rc->msg);
  if (rc->speedTimer)
    g_timer_destroy(rc->speedTimer);
  if (rc->readTimer)
    g_timer_destroy(rc->readTimer);
  if (rc->readMap)
    FreeBitmap(rc->readMap);
  if (rc->crcBuf)
  ...
  if (Closure->readAndCreate && Closure->guiMode &&
      !strncmp(Closure->methodName, "RS01", 4) // FIXME for RS03
      && !scan_mode && !aborted) {
    if (!full_read) {
      ...
      ContinueWithAction(ACTION_CREATE_CONT);
  ...
  /* In GUI mode both the reader and worker are spawned sub threads;
     however in CLI mode the reader is the main thread and must not be
     terminated. */
...
static void check_for_ecc_data(read_closure *rc) {
  ...
  if (rc->image->eccHeader && !strncmp((char *)rc->image->eccHeader, "RS02",
                                       4)) /* see if we have RS02 type ecc */
  {
...
reopen_image:
  if (!LargeStat(Closure->imageName, &image_size)) {
    ...
    if (!(rc->writerImage =
              LargeOpen(Closure->imageName, O_WRONLY | O_CREAT, IMG_PERMS)))
  ...
  if (!LargeSeek(rc->readerImage, (gint64)(2048 * FINGERPRINT_SECTOR)))
    ...
    if (n != 2048 || !fp_read ||
        (CheckForMissingSector(buf, FINGERPRINT_SECTOR, NULL, 0) !=
         SECTOR_PRESENT))
      unknown_fingerprint = TRUE;
  }
  ...
  if (!unknown_fingerprint && memcmp(image_fp, medium_fp, 16)) {
    if (!Closure->guiMode)
      ...
      if (!answer) {
        ...
        SwitchAndSetFootline(Closure->readLinearNotebook, 1,
                             Closure->readLinearFootline,
                             _("<span %s>Reading aborted.</span> Please select "
  ...
  unvisited sectors before trying to re-read the missing ones.
       Exception: We must start from the beginning if multiple reading passes
  are requested. */
...
static void fill_gap(read_closure *rc) {
  ...
  if (!rc->scanMode && rc->firstSector > rc->readMarker) {
    ...
    if (!LargeSeek(rc->writerImage, (gint64)(2048 * s)))
      Stop(_("Failed seeking to sector %lld in image [%s]: %s"), s, "fill",
           strerror(errno));

    while (s < rc->firstSector) {
      int n;

      CreateMissingSector(buf, s, rc->fingerprint, FINGERPRINT_SECTOR,
                          rc->volumeLabel);
      n = LargeWrite(rc->writerImage, buf, 2048);
      if (n != 2048)
        Stop(_("Failed writing to sector %lld in image [%s]: %s"), s, "fill",
             strerror(errno));
      s++;
    }
  }
...
static void prepare_crc_cache(read_closure *rc) {
  ...
  if (!rc->scanMode && !rc->rereading && rc->firstSector == 0 &&
  ...
  if (rc->eccMethod) {
    ...
    if (Closure->guiMode)
      SetLabelText(
          GTK_LABEL(Closure->readLinearHeadline), "<big>%s</big>\n<i>%s</i>",
          _("Reading CRC information from ecc data"), rc->dh->mediumDescr);
...
static void prepare_timer(read_closure *rc) {
  ...
  if (Closure->guiMode && Closure->spinupDelay)
    SwitchAndSetFootline(Closure->readLinearNotebook, 0,
                         Closure->readLinearFootline, "ignore");
...
static void show_progress(read_closure *rc) {
  ...
  if (rc->readPos > rc->readMarker)
    rc->readMarker = rc->readPos;
  ...
  if (rc->lastPercent != percent) {
    ...
    if (rc->readOK <= rc->lastReadOK) /* nothing read since last sample? */
    {
      ...
      if (Closure->readErrors - rc->previousReadErrors > 0)
        ...
        color = Closure->additionalSpiralColor;
    ...
    } else /* something was read since last sample */
    {
      ...
      if (rc->firstSpeedValue) {
        ...
        PrintProgress(_("Read position: %3d.%1d%% (%4.1fx)"), percent / 10,
                      percent % 10, rc->speed);
...
static gpointer worker_thread(read_closure *rc) {
  ...
  for (;;) {
    ...
    if (!rc->scanMode) {
      ...
      if (!LargeSeek(rc->writerImage, (gint64)(2048 * s))) {
        rc->workerError = g_strdup_printf(
            _("Failed seeking to sector %lld in image [%s]: %s"), s, "store",
            strerror(errno));
        goto update_mutex;
      }

      n = LargeWrite(rc->writerImage, rc->alignedBuf[rc->writePtr]->buf,
                     2048 * nsectors);
      if (n != 2048 * nsectors) {
        rc->workerError = g_strdup_printf(
            _("Failed writing to sector %lld in image [%s]: %s"), s, "store",
            strerror(errno));
        goto update_mutex;
      }
      ...
      if (Closure->crcCache) {
        for (i = 0; i < nsectors; i++)
          Closure->crcCache[s + i] =
              Crc32(rc->alignedBuf[rc->writePtr]->buf + 2048 * i, 2048);
      }
    }
...
void ReadMediumLinear(gpointer data) {
  ...
  rc->worker =
      g_thread_create((GThreadFunc)worker_thread, (gpointer)rc, TRUE, &err);
  if (!rc->worker)
...
next_reading_pass:
  ...
  while (rc->readPos <= rc->lastSector) {
    ...
    if (Closure->stopActions) /* somebody hit the Stop button */
    {
      SwitchAndSetFootline(
          Closure->readLinearNotebook, 1, Closure->readLinearFootline,
          _("<span %s>Aborted by user request!</span> %lld sectors read, %lld "
            "sectors unreadable/skipped so far."),
          Closure->redMarkup, rc->readOK, Closure->readErrors);
    ...
    if (rc->readPos + nsectors >
        rc->lastSector) /* don't read past the (CD) media end */
  ...
  reread:
    if (!rc->scanMode && rc->readPos < rc->readMarker) {
      ...
      /* else query dead sectors from image */
        ...
        for (i = 0; i < num_compare; i++) {
          ...
          if (n != 2048)
            Stop(_("unexpected read error in image for sector %lld"),
                 rc->readPos);
          ...
          if (err != SECTOR_PRESENT)
            ...
            if (!rc->crcBuf ||
                CheckAgainstCrcBuffer(rc->crcBuf, rc->readPos + i,
                                      sector_buf) != CRC_BAD) {
    ...
    status = ReadSectors(rc->dh, rc->alignedBuf[rc->readPtr]->buf, rc->readPos,
                         nsectors);
    ...
    if (status && !Closure->ignoreFatalSense && rc->dh->sense.sense_key &&
      ...
      if (!answer) {
        SwitchAndSetFootline(
            Closure->readLinearNotebook, 1, Closure->readLinearFootline,
            _("<span %s>Aborted by user request!</span> %lld sectors read, "
              "%lld sectors unreadable/skipped so far."),
            Closure->redMarkup, rc->readOK, Closure->readErrors);
        rc->unreportedError = FALSE; /* suppress respective error message */
        goto terminate;
      }
    ...
    if (!status) {
      for (i = 0; i < nsectors; i++) {
        ...
        err = CheckForMissingSector(sector_buf + i * 2048, rc->readPos + i,
                                    NULL, 0);
        if (err != SECTOR_PRESENT)
          ExplainMissingSector(sector_buf + i * 2048, rc->readPos + i, err,
                               FALSE);
      }
    }
    ...
    if (status) {
      ...
      if (nsectors >= Closure->sectorSkip)
        nfill = nsectors;
      else {
        int skip = rc->dh->clusterSize > Closure->sectorSkip
                       ? rc->dh->clusterSize
                       : Closure->sectorSkip;
        if (rc->readPos + skip > rc->lastSector)
          nfill = rc->lastSector - rc->readPos + 1;
        else
          nfill = skip - ((rc->readPos + skip) & cluster_mask);
      }

      /* If we are reading past the read marker we must take care
             to fill up any holes with dead sector markers before skipping
         forward. When sectorSkip is 0 and nsectors > 1, we will re-read all
         these sectors again one by one, so we catch this case in order not to
         write the markers twice.  */
      ...
      if (!rc->scanMode && rc->readPos + nfill > rc->readMarker &&
        ...
        for (i = 0; i < nfill; i++) {
          ...
          CreateMissingSector(rc->alignedBuf[rc->readPtr]->buf, rc->readPos + i,
                              rc->fingerprint, FINGERPRINT_SECTOR,
                              rc->volumeLabel);
      ...
      if (Closure->sectorSkip && nsectors > 1) {
        ...
        rc->readPos -=
            nsectors; /* nsectors will be added again after the goto */
#if 0
  ...
  if (!rc->scanMode && (Closure->readErrors || Closure->crcErrors) &&
      rc->pass < Closure->readingPasses) {
    if (Closure->guiMode)
      SetLabelText(
          GTK_LABEL(Closure->readLinearHeadline),
  ...
  if (rc->doMD5sums && rc->eccMethod)
    ...
    ClearCrcCache(); /* deferred until here to avoid race condition */
  ...
  t = NULL;
  if (rc->rereading) {
    if (!Closure->readErrors)
      t = g_strdup_printf(_("%lld sectors read.     "), rc->readOK);
    else
      t = g_strdup_printf(_("%lld sectors read; %lld unreadable sectors."),
                          rc->readOK, Closure->readErrors);
  } else /* not rereading */
  {
    if (!Closure->readErrors &&
        !Closure->crcErrors) { /* No sector errors. Check size... */
      if (rc->eccFile) {
        if (rc->dh->sectors != rc->ei->sectors)
          t = g_strdup_printf(_("All sectors successfully read, but wrong "
                                "image length (%lld sectors difference)"),
    ...
    } else /* we have unreadable or damaged sectors */
    {
      if (Closure->readErrors && !Closure->crcErrors)
        ...
        t = g_strdup_printf(_("%lld CRC errors, %lld unreadable sectors."),
  ...
  if (Closure->guiMode) {
    if (rc->scanMode)
      SwitchAndSetFootline(Closure->readLinearNotebook, 1,
                           Closure->readLinearFootline, "%s%s",
                           _("Scanning finished: "), t);
    else
      SwitchAndSetFootline(Closure->readLinearNotebook, 1,
                           Closure->readLinearFootline, "%s%s",
                           _("Reading finished: "), t);
  }
  if (t)
    g_free(t);
  if (md5_failure)
  ...
  if (rc->dh->mainType == CD && tao_tail && tao_tail == Closure->readErrors &&
      !Closure->noTruncate) {
    int answer;
    ...
    if (Closure->guiMode)
      answer = ModalWarning(
          ...
          _("%d sectors missing at the end of the disc.\n"
            ...
            "The Image will be truncated accordingly. See the manual for "
            "details.\n"),
          tao_tail);
    else
      answer = ModalWarning(
