...
static void print_greetings(FILE *where) {
  ...
  g_fprintf(where, "%s\n%s.\n", Closure->versionString,
            _("Copyright 2004-2012 Carsten Gnoerlich"));
 ...
 * Does not currently work if there are multiple threads besides the main
 * thread.
 */

void RegisterCleanup(char *error_title, void (*cleanup)(gpointer),
                     gpointer data) {
  Closure->cleanupProc = cleanup;
  Closure->cleanupData = data;
  if (Closure->errorTitle)
    g_free(Closure->errorTitle);
...
GThread *CreateGThread(GThreadFunc f, gpointer data) {
  ...
  if (!t)
    Stop("Could not create thread: %s\n", err->message);
...
static gboolean message_idle_func(gpointer data) {
  ...
  dialog = gtk_message_dialog_new_with_markup(
      mi->window, GTK_DIALOG_DESTROY_WITH_PARENT, mi->type, GTK_BUTTONS_CLOSE,
      mi->msg, NULL);

  gtk_label_set_line_wrap(
      GTK_LABEL(((struct _GtkMessageDialog *)dialog)->label), FALSE);
...
GtkWidget *CreateMessage(char *format, GtkMessageType type, ...) {
  ...
  dialog =
      gtk_message_dialog_new(Closure->window, GTK_DIALOG_DESTROY_WITH_PARENT,
                             type, GTK_BUTTONS_CLOSE, utf8, NULL);
...
void SetLabelText(GtkLabel *label, char *format, ...) {
  ...
  if (format) {
    ...
    if (!tmp)
      tmp = g_strdup_printf("SetLabelText(%s) failed", format);
    li->text = g_locale_to_utf8(tmp, -1, NULL, NULL, NULL);
    g_free(tmp);
  } else
    li->text = g_locale_to_utf8("(null)", -1, NULL, NULL, NULL);
  va_end(argp);
...
static gboolean progress_idle_func(gpointer data) {
  ...
  switch (pi->max) {
  case 100:
    g_sprintf(text, "%3d%%", pi->percent);
    break;
  case 1000:
    g_sprintf(text, "%3d.%1d%%", pi->percent / 10, pi->percent % 10);
    break;
  }
...
static gboolean modal_idle_func(gpointer data) {
  ...
  dialog =
      gtk_message_dialog_new(Closure->window, GTK_DIALOG_DESTROY_WITH_PARENT,
                             mi->message_type, mi->button_type, "%s", mi->msg);
  gtk_label_set_line_wrap(
      GTK_LABEL(((struct _GtkMessageDialog *)dialog)->label), FALSE);
...
void SwitchAndSetFootline(GtkWidget *notebook, int page, GtkWidget *label,
                          char *format, ...) {
  va_list argp;
  char *tmp;
...
void ReverseCancelOK(GtkDialog *dialog) {
  ...
  button =
      ((GtkBoxChild *)(g_list_first(GTK_BOX(box)->children)->data))->widget;
...
static void insert_button(GtkDialog *dialog) {
  ...
  g_signal_connect(G_OBJECT(check), "toggled", G_CALLBACK(dont_ask_again_cb),
                   NULL);
...
int ConfirmImageDeletion(char *file) {
  ...
  answer = ModalDialog(
      GTK_MESSAGE_QUESTION, GTK_BUTTONS_OK_CANCEL, insert_button,
...
int ConfirmEccDeletion(char *file) {
  ...
  answer =
      ModalDialog(GTK_MESSAGE_QUESTION, GTK_BUTTONS_OK_CANCEL, insert_button,
