...
CrcBuf *RS03GetCrcBuf(Image *image) {
  ...
  if (!LargeSeek(file, 2048 * (lay->firstCrcPos))) {
    if (lay->target == ECC_FILE)
      Stop(_("Failed seeking to sector %lld in ecc file: %s"), lay->firstCrcPos,
           strerror(errno));
    else
      Stop(_("Failed seeking to sector %lld in image: %s"), lay->firstCrcPos,
           strerror(errno));
  }
...
void RS03ReadSectors(Image *image, RS03Layout *lay, unsigned char *buf,
                     gint64 layer, gint64 layer_sector, gint64 how_many,
                     int flags) {
  ...
  if (layer >= lay->ndata) {
    ...
    start_sector = lay->firstEccPos +
                   (layer - lay->ndata) * lay->sectorsPerLayer + layer_sector;
    stop_sector = start_sector + how_many - 1;
    ...
    for (n = start_sector; n <= stop_sector; n++) {
      if (n >= file_sector_size) {
        if (n >= expected_sectors) {
          ...
          CreateMissingSector(bufptr, n, lay->eh->mediumFP, FINGERPRINT_SECTOR,
                              volume_label);
        }
        byte_size -= 2048;
      }
  ...
  if (!LargeSeek(target_file, (gint64)(2048 * start_sector)))
    Stop(_("Failed seeking to sector %lld in image: %s"), start_sector,
         strerror(errno));

  n = LargeRead(target_file, buf, byte_size);
  if (n != byte_size)
    Stop(_("Failed reading sector %lld in image: %s"), start_sector,
         strerror(errno));
}
...
RS03Layout *CalcRS03Layout(gint64 data_sectors, EccHeader *eh, int target) {
  ...
  if (target == ECC_FILE) {
      ...
      case 'm': {
        ...
        if (ecc_size < ecc_file_size(lay->dataSectors, 8) ||
          ...
          Stop(_("Ecc file size %lldm out of useful range [%lld .. %lld]"),
               ecc_size, ecc_file_size(lay->dataSectors, 8),
    ...
    lay->sectorsPerLayer =
        (lay->dataSectors + lay->ndata - 2) / (lay->ndata - 1);
    lay->totalSectors =
        lay->dataSectors + 2 + (lay->nroots + 1) * lay->sectorsPerLayer;
  ...
  if (target == ECC_IMAGE) {
    ...
    if (eh)
      lay->sectorsPerLayer = eh->sectorsPerLayer;
    else
      lay->sectorsPerLayer = lay->mediumCapacity / GF_FIELDMAX;
    lay->dataSectors = data_sectors;
    lay->totalSectors = GF_FIELDMAX * lay->sectorsPerLayer;

    lay->ndata =
        (data_sectors + 2 + lay->sectorsPerLayer - 1) / lay->sectorsPerLayer;
    ...
    lay->ndata++; /* CRC layer is also protected and counted as part of the data
 portion */
  ...
  if (target == ECC_FILE)
    ...
    Verbose("Calculated layout for RS03 image:\n");
