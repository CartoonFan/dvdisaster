...
void RS01AddVerifyValues(Method *method, int percent, gint64 totalMissing,
                         gint64 totalCrcErrors, gint64 newMissing,
                         gint64 newCrcErrors) {
...
void CreateRS01VerifyWindow(Method *self, GtkWidget *parent) {
  ...
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 2, 4, 5, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 4);
  ...
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 1, 7, 8, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 2);
  lab = wl->cmpEccMd5Sum = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 1, 2, 7, 8, GTK_EXPAND | GTK_FILL,
                   GTK_SHRINK, 0, 0);

  lab = wl->cmpEccResult = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 2, 8, 9, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 4);
}
  ...
  if (excess_sectors) {
    ...
    if (!image->sectorsMissing) {
      ...
      if (!image->crcErrors) {
        ...
        if (Closure->guiMode) {
          SetLabelText(GTK_LABEL(wl->cmpImageResult),
                       _("<span %s>Image complete, but contains checksum "
                         "errors!</span>"),
                       Closure->redMarkup);
          SetLabelText(GTK_LABEL(wl->cmpImageMd5Sum), "%s", idigest);
        }
...
process_ecc:
  ...
  if (!image) {
    PrintLog(_("not present\n"));
    if (Closure->guiMode)
      SwitchAndSetFootline(wl->cmpEccNotebook, 0, wl->cmpEccEmptyMsg,
                           _("No error correction file present."));
    goto skip_ecc;
  }
  ...
  if (eh->creatorVersion) {
    ...
    if (eh->creatorVersion % 100) {
      ...
      if (eh->creatorVersion < 6000)
        format = "%s-%d.%d.%d";
      else if (eh->creatorVersion <= 6500)
        format = "%s-%d.%d (pl%d)";
      else {
      ...
      if (Closure->guiMode) {
        if (color_format)
          ...
          SwitchAndSetFootline(wl->cmpEccNotebook, 1, wl->cmpEccCreatedBy,
                               format, "dvdisaster", major, minor, pl);
      }
    } else {
  ...
  if (eh->creatorVersion >= 6600 &&
      eh->inLast != 2048)     /* image file whose length is */
  ...
  if (!image->file) {
    if (!ecc_in_last) {
      ...
      if (Closure->guiMode)
        SetLabelText(GTK_LABEL(wl->cmpEccMediumSectors), "%lld",
                     image->expectedSectors);
    } else {
      ...
      if (Closure->guiMode)
        SetLabelText(GTK_LABEL(wl->cmpEccMediumSectors),
                     _("%lld sectors + %d bytes"), image->expectedSectors - 1,
                     ecc_in_last);
    }
  }
  ...
  if (image->file) {
    ...
    {    /* TAO case (1 or 2 sectors more than expected) */
      if (image->sectorSize > image->expectedSectors &&
          image->sectorSize - image->expectedSectors <= 2) {
        PrintLog(
            _("* medium sectors   : %lld (BAD, perhaps TAO/DAO mismatch)\n"),
            image->expectedSectors);
        if (Closure->guiMode) {
          if (!ecc_in_last)
            SetLabelText(GTK_LABEL(wl->cmpEccMediumSectors),
                         "<span %s>%lld</span>", Closure->redMarkup,
                         image->expectedSectors);
          else
            SetLabelText(GTK_LABEL(wl->cmpEccMediumSectors),
                         "<span %s>%lld sectors + %d bytes</span>",
                         Closure->redMarkup, image->expectedSectors - 1,
                         ecc_in_last);
        }
      } else /* more than 2 Sectors difference */
      {
        if (!ecc_in_last) {
          ...
          if (Closure->guiMode) {
            ...
            if (!ecc_advice)
              ecc_advice =
                  g_strdup_printf(_("<span %s>Image size does not match error "
                                    "correction file.</span>"),
                                  Closure->redMarkup);
          }
    ...
    if (image && image->file && !image->sectorsMissing && !excess_sectors) {
      ...
      if (Closure->guiMode) {
        if (n)
          ...
          SetLabelText(GTK_LABEL(wl->cmpEccImgMd5Sum), "<span %s>%s</span>",
  ...
  if (image && image->file) {
    if (image->fpState != FP_PRESENT) {
      PrintLog(_("* fingerprint match: NOT POSSIBLE - related sector is "
                 "missing in image!\n"));
      if (Closure->guiMode)
      ...
      if (memcmp(image->imageFP, eh->mediumFP, 16)) {
        PrintLog(_("* fingerprint match: MISMATCH - .iso and .ecc don't belong "
                   "together!\n"));
        ...
        if (Closure->guiMode) {
          ...
          if (!ecc_advice)
            ecc_advice =
                g_strdup_printf(_("<span %s>Image and error correction files "
                                  "do not belong together!</span>"),
                                Closure->redMarkup);
        }
      } else {
  ...
  ecc_expected =
      2048 * ((image->expectedSectors + eh->dataBytes - 1) / eh->dataBytes);
  ecc_blocks = (image->eccFile->size -
                image->expectedSectors * sizeof(guint32) - sizeof(EccHeader)) /
               eh->eccBytes;
  ...
  if (ecc_expected == ecc_blocks) {
    ...
    if (Closure->guiMode)
      SetLabelText(GTK_LABEL(wl->cmpEccBlocks),
                   _("<span %s>%lld (bad, expected %lld)</span>"),
  ...
  while (!LargeEOF(image->eccFile)) {
    ...
    if (last_percent != percent) {
      if (!Closure->guiMode)
        ...
        SetLabelText(GTK_LABEL(wl->cmpEccMd5Sum), "%3d%%", percent);
  ...
  if (memcmp(eh->eccSum, digest, 16)) {
    ...
    if (Closure->guiMode) {
      SetLabelText(GTK_LABEL(wl->cmpEccMd5Sum), _("<span %s>bad</span>"),
                   Closure->redMarkup);
      if (!ecc_advice)
        ecc_advice = g_strdup_printf(
            _("<span %s>Error correction file may be damaged!</span>"),
            Closure->redMarkup);
    }
  } else {
