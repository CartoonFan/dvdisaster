...
static int draw_text(GdkDrawable *d, PangoLayout *l, char *text, int x, int y,
                     GdkColor *color, int redraw) {
  static GdkPixmap *pixmap;
  static int pixmap_width, pixmap_height;
  int w, h, pw;
  int erase_to = Closure->readAdaptiveSpiral->mx -
                 Closure->readAdaptiveSpiral->diameter / 2;
...
static void redraw_labels(GtkWidget *widget, int erase_mask) {
  ...
  y = Closure->readAdaptiveSpiral->my -
      Closure->readAdaptiveSpiral->diameter / 2;
  h = draw_text(d, Closure->readLinearCurve->layout, _("Adaptive reading:"), x,
                y, Closure->foreground, erase_mask & REDRAW_TITLE);

  y += h + h / 2;
  if (Closure->readAdaptiveSubtitle) {
    char *c = Closure->readAdaptiveSubtitle +
              strlen(Closure->readAdaptiveSubtitle) / 2;
    ...
    if (c) /* split text into two lines */
    {
      *c = 0;
      ...
      h = draw_text(d, Closure->readLinearCurve->layout, c + 1, x, y + h,
                    Closure->foreground, erase_mask & REDRAW_SUBTITLE);
      *c = ' ';
  ...
  y += 4 * h;
  h = draw_text(d, Closure->readLinearCurve->layout, _("Sectors processed"), x,
                y, Closure->foreground, erase_mask & REDRAW_TITLE);

  y += h;
  snprintf(buf, 255, "  %s: %lld", _("readable"), readable);
  h = draw_text(d, Closure->readLinearCurve->layout, buf, x, y,
                Closure->foreground, erase_mask & REDRAW_PROGRESS);
  ...
  if (min_required > 0 && readable > 0) {
    int percent =
        round(((1000 * readable) / (readable + correctable + missing)));

    if (!missing)             /* Make sure target percentage is reached */
      percent = min_required; /* in spite of rounding errors            */

    y += h;
    snprintf(buf, 255, _("Readable: %d.%d%% / %d.%d%% required"), percent / 10,
             percent % 10, min_required / 10, min_required % 10);
    h = draw_text(d, Closure->readLinearCurve->layout, buf, x, y,
                  Closure->foreground, erase_mask & REDRAW_PROGRESS);
  }

  y += h;
  snprintf(buf, 255, _("Total recoverable: %d.%d%%"), percent / 10,
           percent % 10);
  h = draw_text(d, Closure->readLinearCurve->layout, buf, x, y,
                Closure->foreground, erase_mask & REDRAW_PROGRESS);
  ...
  if (Closure->readAdaptiveErrorMsg && erase_mask & REDRAW_ERRORMSG) {
    ...
    SetText(Closure->readLinearCurve->layout, Closure->readAdaptiveErrorMsg, &w,
            &h);
    y = Closure->readAdaptiveSpiral->my +
        Closure->readAdaptiveSpiral->diameter / 2 - h;
...
static void update_geometry(GtkWidget *widget) {
  ...
  Closure->readAdaptiveSpiral->mx =
      a->width - 15 - Closure->readAdaptiveSpiral->diameter / 2;
...
void ChangeSegmentColor(GdkColor *color, int segment) {
  ...
  if (Closure->readAdaptiveSpiral->cursorPos == segment)
    ...
    g_idle_add(segment_idle_func, GINT_TO_POINTER(100 + segment));
}
...
void RemoveFillMarkers() { g_idle_add(remove_fill_idle_func, NULL); }
...
void ResetAdaptiveReadWindow() {
  ...
  if (Closure->readAdaptiveDrawingArea->window) {
    ...
    gdk_window_invalidate_rect(Closure->readAdaptiveDrawingArea->window, &rect,
                               FALSE);
  }
}
...
void SetAdaptiveReadMinimumPercentage(int value) { min_required = value; }
...
void CreateAdaptiveReadWindow(GtkWidget *parent) {
  ...
  gtk_label_set_ellipsize(GTK_LABEL(Closure->readAdaptiveHeadline),
                          PANGO_ELLIPSIZE_END);
  gtk_box_pack_start(GTK_BOX(parent), Closure->readAdaptiveHeadline, FALSE,
                     FALSE, 3);
  ...
  Closure->readAdaptiveSpiral = CreateSpiral(Closure->grid, Closure->background,
                                             10, 5, ADAPTIVE_READ_SPIRAL_SIZE);

  gtk_widget_set_size_request(d_area, -1,
                              Closure->readAdaptiveSpiral->diameter);
}
