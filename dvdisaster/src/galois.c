...
GaloisTables *CreateGaloisTables(gint32 gf_generator) {
  ...
  if (b != 1)
    Stop("Failed to create the Galois field log tables!\n");
...
ReedSolomonTables *CreateReedSolomonTables(GaloisTables *gt,
                                           ...
                                           gint32 prim_elem, int nroots_in) {
  ...
  for (i = 0, root = first_consecutive_root * prim_elem; i < rt->nroots;
    ...
    for (j = i; j > 0; j--) {
      if (rt->gpoly[j] != 0)
        rt->gpoly[j] =
            rt->gpoly[j - 1] ^
            gt->alphaTo[mod_fieldmax(gt->indexOf[rt->gpoly[j]] + root)];
      else
  ...
  for (i = 0; i < rt->nroots; i++)
    for (j = 0; j < GF_FIELDSIZE; j++)
      *lut++ = gt->alphaTo[mod_fieldmax(gt->indexOf[j] +
