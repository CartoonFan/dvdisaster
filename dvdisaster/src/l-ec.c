...
int PToByteIndex(int p, int i) { return 12 + p + i * 86; }
...
int DecodePQ(ReedSolomonTables *rt, unsigned char *data, int padding,
  ...
  for (j = 1; j < shortened_size; j++)
    for (i = 0; i < NROOTS; i++)
      if (syndrome[i] == 0)
        ...
        syndrome[i] =
            data[j] ^
            gt->alphaTo[mod_fieldmax(gt->indexOf[syndrome[i]] +
  ...
  if (erasure_count > 0) {
    lambda[1] = gt->alphaTo[mod_fieldmax(LEC_PRIM_ELEM *
                                         (GF_FIELDMAX - 1 - erasure_list[0]))];
  ...
  while (++r <= NROOTS) {
    ...
    for (i = 0; i < r; i++)
      if ((lambda[i] != 0) && (syndrome[r - i - 1] != GF_ALPHA0))
        discr_r ^= gt->alphaTo[mod_fieldmax(gt->indexOf[lambda[i]] +
                                            syndrome[r - i - 1])];
      ...
      if (2 * el <= r + erasure_count - 1) {
        ...
        for (i = 0; i <= NROOTS; i++)
          b[i] = (lambda[i] == 0) ? GF_ALPHA0
                                  : mod_fieldmax(gt->indexOf[lambda[i]] -
                                                 discr_r + GF_FIELDMAX);
  ...
  for (j = lambda_roots - 1; j >= 0; j--) {
    ...
    num2 =
        gt->alphaTo[mod_fieldmax(root[j] * (LEC_FIRST_ROOT - 1) + GF_FIELDMAX)];
    den = 0;
    ...
    if (num1 != 0 && location >= padding) {
      ...
      data[location - padding] ^=
          gt->alphaTo[mod_fieldmax(gt->indexOf[num1] + gt->indexOf[num2] +
