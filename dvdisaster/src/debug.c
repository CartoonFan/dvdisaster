...
static void random_error1(Image *image, char *arg) {
  ...
  cpos = strchr(arg, ',');
  if (!cpos)
    Stop(_("2nd argument is missing"));
  *cpos = 0;
  ...
  if (n_eras < 8 || n_eras > 100 || n_errors < 1 || n_errors > n_eras)
    Stop(_("Number of roots must be 8..100;\n"
           "the number of erasures must be > 0 and less than the number of "
           "roots.\n"));
  ...
  PrintLog(_("\nGenerating random correctable erasures (for %d roots, max "
             "erasures = %d).\n"),
           n_eras, n_errors);
    ...
    for (i = 0; i < n_data; i++) {
      ...
      if (block_sel[i] && block_idx[i] < image->sectorSize) {
        if (!LargeSeek(image->file, (gint64)(2048 * block_idx[i])))
          Stop(_("Failed seeking to sector %lld in image: %s"), block_idx[i],
               strerror(errno));

        CreateMissingSector(missing, block_idx[i], image->imageFP,
                            FINGERPRINT_SECTOR, NULL);
        ...
        if (LargeWrite(image->file, missing, write_size) != write_size)
          Stop(_("Failed writing to sector %lld in image: %s"), block_idx[i],
               strerror(errno));
      }
  ...
  PrintProgress(_("Progress: 100%%\n"
                  "Recover the image using the --fix option before doing "
                  "another --random-errors run.\n"
                  "Otherwise you'll accumulate >= %d erasures/ECC block and "
    ...
    for (i = 0; i < 255; i++) {
      if (block_sel[i]) {
        ...
        if (i < eh->dataBytes) {
          ...
          s = RS02EccSectorIndex(lay, i - eh->dataBytes, si);
  ...
  for (si = 0; si < lay->sectorsPerLayer; si++) {
    ...
    for (i = 0; i < 255; i++) {
      if (block_sel[i]) {
        ...
        if (!LargeSeek(file, (gint64)(2048 * s))) // FIXME: wrong for ecc files
          Stop(_("Failed seeking to sector %lld in image: %s"), s,
               strerror(errno));
...
void RandomError(char *arg) {
  ...
  if (image && image->eccFileMethod)
    ...
    Stop("Internal error: No ecc method identified.");
...
void Byteset(char *arg) {
  ...
  *cpos = 0;
  ...
  cpos = strchr(arg, ',');
  if (!cpos)
    Stop(_("3rd argument is missing"));
  *cpos = 0;
...
void RandomImage(char *image_name, char *n_sectors, int mark) {
  ...
  PrintLog(_("\nCreating random image with %lld sectors.\n\n"
           ...
           sectors, mark ? "--marked-image" : "--random-image", sectors,
           Closure->randomSeed);
  ...
  while (s < sectors) {
    ...
    do
      buf[i--] = (Random32() ^ invert);
    while (i >= 0);
#else
    do
      buf[i--] = SwapBytes32(Random32() ^ invert);
    while (i >= 0);
#endif
...
void ZeroUnreadable(void) {
  ...
  for (s = 0; s < ii->sectors; s++) {
    ...
    if (CheckForMissingSector(buf, s, ii->mediumFP, FINGERPRINT_SECTOR) !=
        SECTOR_PRESENT) {
...
void HexDump(unsigned char *buf, int len, int step) {
  ...
  for (i = 0; i < len; i += step) {
    ...
    for (j = 0; j < step; j++)
      if (i + j >= len)
        PrintLog((j & 0x07) == 0x07 ? "    " : "   ");
      else
        PrintLog("%02x%s", buf[i + j], (j & 0x07) == 0x07 ? "  " : " ");
...
void ShowSector(char *arg) {
  ...
  if (!LargeSeek(ii->file, (gint64)(2048 * sector)))
    Stop(_("Failed seeking to sector %lld in image: %s"), sector,
         strerror(errno));
...
void CopySector(char *arg) {
  ...
  *cpos = 0;
  ...
  cpos = strchr(arg, ',');
  if (!cpos)
    Stop(_("4th argument is missing"));
  *cpos = 0;
  to_path = arg;
  ...
  if (!LargeSeek(from, (gint64)(2048 * from_sector)))
    Stop(_("Failed seeking to sector %lld in image: %s"), from_sector,
         strerror(errno));

  if (LargeRead(from, buf, 2048) != 2048)
    Stop(_("Failed reading sector %lld in image: %s"), from_sector,
         strerror(errno));

  if (!LargeSeek(to, (gint64)(2048 * to_sector)))
    Stop(_("Failed seeking to sector %lld in image: %s"), to_sector,
         strerror(errno));

  if (LargeWrite(to, buf, 2048) != 2048)
    Stop(_("Failed writing to sector %lld in image: %s"), to_sector,
         strerror(errno));
...
void MergeImages(char *arg, int mode) {
  ...
  *cpos = 0;
  ...
  LargeStat(left_path, &left_sectors);
  left_sectors /= 2048;

  if (!(right = LargeOpen(right_path, O_RDONLY, IMG_PERMS)))
    Stop(_("Can't open %s:\n%s"), right_path, strerror(errno));

  LargeStat(right_path, &right_sectors);
  right_sectors /= 2048;

  /*** Compare/merge the images */

  if (!mode)
    PrintLog("Comparing %s (%lld sectors) with %s (%lld sectors).\n", left_path,
             left_sectors, right_path, right_sectors);
  else
    PrintLog("Merging %s (%lld sectors) with %s (%lld sectors).\n", left_path,
             left_sectors, right_path, right_sectors);
  ...
  for (s = 0; s < min_sectors; s++) {
    ...
    if (memcmp(left_buf, right_buf, 2048)) {
      ...
      } else if (CheckForMissingSector(right_buf, s, NULL, 0) !=
                 SECTOR_PRESENT) {
        PrintLog("> Sector %lld missing\n", s);
      } else {
    ...
    if (!mode)
      ...
      for (s = left_sectors; s < right_sectors; s++) {
        if (LargeRead(right, buf, 2048) != 2048)
          Stop(_("Failed reading sector %lld in image: %s"), s,
               strerror(errno));
