...
static void menu_cb(GtkWidget *widget, gpointer data) {
  ...
  case MENU_FILE_QUIT:
    /* Extract current file selections so that they are saved in the .dvdisaster
     * file */

    g_free(Closure->imageName);
    Closure->imageName =
        g_strdup(gtk_entry_get_text(GTK_ENTRY(Closure->imageEntry)));
    ...
    Closure->eccName =
        g_strdup(gtk_entry_get_text(GTK_ENTRY(Closure->eccEntry)));
...
static GtkWidget *add_menu_button(GtkWidget *parent, char *title, int action) {
  ...
  g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(menu_cb),
...
GtkWidget *CreateMenuBar(GtkWidget *parent) {
  ...
  menu_strip = gtk_menu_new();
  item = add_menu_button(menu_strip, _("menu|Medium info"),
                         MENU_TOOLS_MEDIUM_INFO);
  if (!Closure->deviceNodes->len)
    gtk_widget_set_sensitive(item, FALSE);

  if (Closure->debugMode && !Closure->screenShotMode)
    add_menu_button(menu_strip, _("menu|Raw sector editor"),
                    MENU_TOOLS_RAW_EDITOR);
...
void AttachTooltip(GtkWidget *widget, char *short_descr, char *long_descr) {
  ...
  g_signal_connect(G_OBJECT(widget), "enter_notify_event",
                   G_CALLBACK(tooltip_cb), (gpointer)long_copy);
...
static void file_select_cb(GtkWidget *widget, gpointer data) {
  ...
  switch (action) { /*** Image file selection */

  case MENU_FILE_IMAGE:
    if (!Closure->imageFileSel) {
      Closure->imageFileSel =
          gtk_file_selection_new(_utf("windowtitle|Image file selection"));
      ...
      g_signal_connect(G_OBJECT(Closure->imageFileSel), "destroy",
                       ...
                       GINT_TO_POINTER(MENU_FILE_IMAGE_DESTROY));
      g_signal_connect(
          G_OBJECT(GTK_FILE_SELECTION(Closure->imageFileSel)->ok_button),
          "clicked", G_CALLBACK(file_select_cb),
          GINT_TO_POINTER(MENU_FILE_IMAGE_OK));

      g_signal_connect(
          G_OBJECT(GTK_FILE_SELECTION(Closure->imageFileSel)->cancel_button),
          "clicked", G_CALLBACK(file_select_cb),
          GINT_TO_POINTER(MENU_FILE_IMAGE_CANCEL));
    }
    gtk_file_selection_set_filename(
        GTK_FILE_SELECTION(Closure->imageFileSel),
  ...
  case MENU_FILE_IMAGE_OK:
    g_free(Closure->imageName);
    Closure->imageName = g_strdup(gtk_file_selection_get_filename(
        GTK_FILE_SELECTION(Closure->imageFileSel)));
  ...
  case MENU_FILE_ECC:
    if (!Closure->eccFileSel) {
      Closure->eccFileSel = gtk_file_selection_new(
          _utf("windowtitle|Error correction file selection"));
      ReverseCancelOK(GTK_DIALOG(Closure->eccFileSel));
      g_signal_connect(G_OBJECT(Closure->eccFileSel), "destroy",
                       G_CALLBACK(file_select_cb),
                       GINT_TO_POINTER(MENU_FILE_ECC_DESTROY));
      g_signal_connect(
          G_OBJECT(GTK_FILE_SELECTION(Closure->eccFileSel)->ok_button),
          "clicked", G_CALLBACK(file_select_cb),
          GINT_TO_POINTER(MENU_FILE_ECC_OK));

      g_signal_connect(
          G_OBJECT(GTK_FILE_SELECTION(Closure->eccFileSel)->cancel_button),
          "clicked", G_CALLBACK(file_select_cb),
          GINT_TO_POINTER(MENU_FILE_ECC_CANCEL));
    }
    gtk_file_selection_set_filename(
        GTK_FILE_SELECTION(Closure->eccFileSel),
  ...
  case MENU_FILE_ECC_OK:
    g_free(Closure->eccName);
    Closure->eccName = g_strdup(gtk_file_selection_get_filename(
        GTK_FILE_SELECTION(Closure->eccFileSel)));
    if (Closure->autoSuffix)
...
GtkWidget *CreateToolBar(GtkWidget *parent) {
  ...
  AttachTooltip(ebox, _("tooltip|Drive selection"),
  ...
  g_signal_connect(G_OBJECT(combo_box), "changed", G_CALLBACK(drive_select_cb),
                   NULL);
  ...
  AttachTooltip(combo_box, _("tooltip|Drive selection"),
  ...
  icon = gtk_image_new_from_stock("dvdisaster-open-img",
                                  GTK_ICON_SIZE_LARGE_TOOLBAR);
  button = gtk_button_new();
  gtk_button_set_relief(GTK_BUTTON(button), GTK_RELIEF_NONE);
  gtk_container_add(GTK_CONTAINER(button), icon);
  g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(file_select_cb),
  ...
  sep = gtk_vseparator_new();
  gtk_box_pack_start(GTK_BOX(box), sep, FALSE, FALSE, 3);
  AttachTooltip(button, _("tooltip|Image file selection"),
                _("Selects a new image file."));
  AttachTooltip(Closure->imageEntry, _("tooltip|Current image file"),
                _("Shows the name of the current image file."));

  /*** Ecc file selection */

  icon = gtk_image_new_from_stock("dvdisaster-open-ecc",
                                  GTK_ICON_SIZE_LARGE_TOOLBAR);
  button = gtk_button_new();
  gtk_button_set_relief(GTK_BUTTON(button), GTK_RELIEF_NONE);
  gtk_container_add(GTK_CONTAINER(button), icon);
  ...
  sep = gtk_vseparator_new();
  gtk_box_pack_start(GTK_BOX(box), sep, FALSE, FALSE, 3);
  AttachTooltip(button, _("tooltip|Error correction file selection"),
                _("Selects a new error correction file."));
  AttachTooltip(Closure->eccEntry, _("tooltip|Current error correction file"),
                _("Shows the name of the current error correction file."));

  /*** Preferences button */

  icon = gtk_image_new_from_stock("dvdisaster-gtk-preferences",
                                  GTK_ICON_SIZE_LARGE_TOOLBAR);
  ...
  g_signal_connect(G_OBJECT(prefs), "clicked", G_CALLBACK(menu_cb),
                   (gpointer)MENU_PREFERENCES);
  gtk_box_pack_start(GTK_BOX(box), prefs, FALSE, FALSE, 0);
  AttachTooltip(prefs, _("tooltip|Preferences"),
                _("Customize settings for creating images, error correction "
                  "files and other stuff."));

  /*** Help button */

  icon = gtk_image_new_from_stock("dvdisaster-gtk-help",
                                  GTK_ICON_SIZE_LARGE_TOOLBAR);
  ...
  g_signal_connect(G_OBJECT(help), "clicked", G_CALLBACK(menu_cb),
                   (gpointer)MENU_HELP_MANUAL);
  gtk_box_pack_start(GTK_BOX(box), help, FALSE, FALSE, 0);
  AttachTooltip(
      help, _("tooltip|User manual"),
      _("Displays the user manual (external HTML browser required)."));

  /*** Quit button */

  icon = gtk_image_new_from_stock("dvdisaster-gtk-quit",
                                  GTK_ICON_SIZE_LARGE_TOOLBAR);
  quit = gtk_button_new();
  gtk_button_set_relief(GTK_BUTTON(quit), GTK_RELIEF_NONE);
  gtk_container_add(GTK_CONTAINER(quit), icon);
  g_signal_connect(G_OBJECT(quit), "clicked", G_CALLBACK(menu_cb),
                   (gpointer)MENU_FILE_QUIT);
