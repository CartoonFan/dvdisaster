...
static void action_cb(GtkWidget *widget, gpointer data) {
  ...
  if (action != ACTION_STOP) {
    ...
    if (Closure->crcImageName &&
        strcmp(Closure->imageName, Closure->crcImageName))
      ClearCrcCache();
    ...
    if (Closure->eccName) {
      ...
      if (!strcmp(Closure->eccName + len - 4, ".iso")) {
        CreateMessage(
            _("The error correction file type must not be \".iso\".\n\n"
              "If you intended to create or use an .iso image\n"
              "which is augmented with error correction data,\n"
              "please leave the error correction file name blank."),
            GTK_MESSAGE_ERROR);
  ...
  case ACTION_CREATE_CONT:
    ...
    gtk_notebook_set_current_page(GTK_NOTEBOOK(Closure->notebook),
                                  method->tabWindowIndex);
...
static GtkWidget *create_action_bar(GtkNotebook *notebook) {
  ...
  Closure->readButton = wid =
      create_button(_("button|Read"), "dvdisaster-read");
  g_signal_connect(G_OBJECT(wid), "clicked", G_CALLBACK(action_cb),
                   (gpointer)ACTION_READ);
  gtk_box_pack_start(GTK_BOX(vbox), wid, FALSE, FALSE, 0);
  AttachTooltip(wid, _("tooltip|Read Image"),
                _("Reads a CD/DVD image into a file (or tries to complete an "
                  "existing image file)."));
  ...
  Closure->createButton = wid =
      create_button(_("button|Create"), "dvdisaster-create");
  g_signal_connect(G_OBJECT(wid), "clicked", G_CALLBACK(action_cb),
                   (gpointer)ACTION_CREATE);
  gtk_box_pack_start(GTK_BOX(vbox), wid, FALSE, FALSE, 0);
  AttachTooltip(wid, _("tooltip|Create error correction data"),
                _("Creates error correction data. Requires an image file."));

  /*** Scan */

  Closure->scanButton = wid =
      create_button(_("button|Scan"), "dvdisaster-scan");
  g_signal_connect(G_OBJECT(wid), "clicked", G_CALLBACK(action_cb),
                   (gpointer)ACTION_SCAN);
  gtk_box_pack_start(GTK_BOX(vbox), wid, FALSE, FALSE, 0);
  AttachTooltip(wid, _("tooltip|Scan medium"),
  ...
  gtk_box_pack_start(GTK_BOX(vbox), wid, FALSE, FALSE, 0);
  AttachTooltip(
      wid, _("tooltip|Repair image"),
      _("Repairs an image. Requires an image file and error correction data."));

  /*** Verify */

  Closure->testButton = wid =
      create_button(_("button|Verify"), "dvdisaster-verify");
  g_signal_connect(G_OBJECT(wid), "clicked", G_CALLBACK(action_cb),
                   (gpointer)ACTION_VERIFY);
  gtk_box_pack_start(GTK_BOX(vbox), wid, FALSE, FALSE, 0);
  AttachTooltip(
      wid, _("tooltip|Consistency check"),
...
static void log_cb(GtkWidget *widget, gpointer data) { ShowLog(); }
...
void CreateMainWindow(int *argc, char ***argv) {
  ...
  g_signal_connect(G_OBJECT(window), "delete_event", G_CALLBACK(delete_cb),
                   NULL);
  ...
  if (!sig_okay) {
    ...
    msg =
        g_strdup_printf(_("<b>dvdisaster is not properly installed</b>\n\n"
  ...
  gtk_box_pack_start(GTK_BOX(status_box), GTK_WIDGET(Closure->status), TRUE,
                     TRUE, 5);
  ...
  AttachTooltip(button, _("tooltip|Protocol for current action"),
                _("Displays additional information created during the current "
                  "or last action."));

  box = gtk_hbox_new(FALSE, 0);
  gtk_container_add(GTK_CONTAINER(button), box);

  icon = gtk_image_new_from_stock("dvdisaster-gtk-index",
