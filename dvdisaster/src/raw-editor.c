...
static int rb_info_cmp_p(const void *c1, const void *c2) {
  ...
  if (rbi1->pFailures < rbi2->pFailures)
    return -1;
  else if (rbi1->pFailures > rbi2->pFailures)
    return 1;
  else if (rbi1->qFailures < rbi2->qFailures)
    return -1;
  else if (rbi1->qFailures > rbi2->qFailures)
    return 1;
  else
...
static int rb_info_cmp_q(const void *c1, const void *c2) {
  ...
  if (rbi1->qFailures < rbi2->qFailures)
    return -1;
  else if (rbi1->qFailures > rbi2->qFailures)
    return 1;
  else if (rbi1->pFailures < rbi2->pFailures)
...
static void file_select_cb(GtkWidget *widget, gpointer data) {
  ...
  case ACTION_BROWSE_LOAD: /* open the dialog */
    if (!rec->fileSel) {
      ...
      rec->fileSel =
          gtk_file_selection_new(_utf("windowtitle|Raw sector dump selection"));
      ReverseCancelOK(GTK_DIALOG(rec->fileSel));
      g_signal_connect(G_OBJECT(rec->fileSel), "destroy",
                       G_CALLBACK(file_select_cb),
                       GINT_TO_POINTER(ACTION_FILESEL_DESTROY));
      g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(rec->fileSel)->ok_button),
                       "clicked", G_CALLBACK(file_select_cb),
                       GINT_TO_POINTER(ACTION_FILESEL_OK));
      g_signal_connect(
          G_OBJECT(GTK_FILE_SELECTION(rec->fileSel)->cancel_button), "clicked",
          G_CALLBACK(file_select_cb), GINT_TO_POINTER(ACTION_FILESEL_CANCEL));
      sprintf(filename, "%s/", Closure->dDumpDir);
      gtk_file_selection_set_filename(GTK_FILE_SELECTION(rec->fileSel),
                                      filename);
    }
  ...
  case ACTION_FILESEL_OK:
    if (rec->filepath)
      g_free(rec->filepath);
    rec->filepath = g_strdup(
        gtk_file_selection_get_filename(GTK_FILE_SELECTION(rec->fileSel)));
    ...
    SetLabelText(GTK_LABEL(rec->rightLabel),
                 _("%s loaded, LBA %lld, %d samples."), rec->filepath,
                 rec->rb->lba, rec->rb->samplesRead);
    break;
...
static void save_sector(raw_editor_context *rec) {
  ...
  if (!LargeSeek(image, (gint64)(2048 * FINGERPRINT_SECTOR)))
    ...
    if (n != 2048 || (CheckForMissingSector(buf, FINGERPRINT_SECTOR, NULL, 0) !=
                      SECTOR_PRESENT))
      unknown_fingerprint = TRUE;
  }
  ...
  if (!LargeSeek(image, (gint64)(2048 * rb->lba))) {
    LargeClose(image);
    Stop(_("Failed seeking to sector %lld in image [%s]: %s"), rb->lba,
         "raw-editor", strerror(errno));
  }

  n = LargeWrite(image, rb->recovered + rb->dataOffset, 2048);
  if (n != 2048) {
    LargeClose(image);
    Stop(_("Failed writing to sector %lld in image [%s]: %s"), rb->lba,
         "raw-editor", strerror(errno));
  }
...
static void buffer_io_cb(GtkWidget *widget, gpointer data) {
  ...
  case ACTION_LOAD_BUFFER: /* open the dialog */
    if (!rec->loadBufSel) {
      ...
      rec->loadBufSel =
          gtk_file_selection_new(_utf("windowtitle|Load buffer from file"));
      ReverseCancelOK(GTK_DIALOG(rec->loadBufSel));
      g_signal_connect(G_OBJECT(rec->loadBufSel), "destroy",
                       G_CALLBACK(buffer_io_cb),
                       GINT_TO_POINTER(ACTION_FILESEL_LOAD_DESTROY));
      g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(rec->loadBufSel)->ok_button),
                       "clicked", G_CALLBACK(buffer_io_cb),
                       GINT_TO_POINTER(ACTION_FILESEL_LOAD_OK));
      g_signal_connect(
          G_OBJECT(GTK_FILE_SELECTION(rec->loadBufSel)->cancel_button),
          "clicked", G_CALLBACK(buffer_io_cb),
          GINT_TO_POINTER(ACTION_FILESEL_LOAD_CANCEL));
      sprintf(filename, "%s/", Closure->dDumpDir);
      gtk_file_selection_set_filename(GTK_FILE_SELECTION(rec->loadBufSel),
                                      filename);
    }
  ...
  case ACTION_SAVE_BUFFER: /* open the dialog */
    if (!rec->saveBufSel) {
      ...
      rec->saveBufSel =
          gtk_file_selection_new(_utf("windowtitle|Save buffer to file"));
      ...
      g_signal_connect(G_OBJECT(GTK_FILE_SELECTION(rec->saveBufSel)->ok_button),
                       "clicked", G_CALLBACK(buffer_io_cb),
                       GINT_TO_POINTER(ACTION_FILESEL_SAVE_OK));
      g_signal_connect(
          G_OBJECT(GTK_FILE_SELECTION(rec->saveBufSel)->cancel_button),
          "clicked", G_CALLBACK(buffer_io_cb),
          GINT_TO_POINTER(ACTION_FILESEL_SAVE_CANCEL));
      sprintf(filename, "%s/", Closure->dDumpDir);
      gtk_file_selection_set_filename(GTK_FILE_SELECTION(rec->saveBufSel),
                                      filename);
    }
  ...
  case ACTION_FILESEL_LOAD_OK: {
    ...
    path = (char *)gtk_file_selection_get_filename(
        GTK_FILE_SELECTION(rec->loadBufSel));
  ...
  case ACTION_FILESEL_SAVE_OK: {
    ...
    path = (char *)gtk_file_selection_get_filename(
        GTK_FILE_SELECTION(rec->saveBufSel));
...
static void evaluate_vectors(raw_editor_context *rec) {
  ...
  if (CheckEDC(rb->recovered, rb->xaMode) &&
    ...
    SetLabelText(GTK_LABEL(rec->leftLabel),
                 _("*** Well done: Sector has been recovered! ***"));
  } else
    SetLabelText(GTK_LABEL(rec->leftLabel),
                 _("Current buffer state: P %d/%d, Q %d/%d"), rec->p2, rec->p1,
                 rec->q2, rec->q1);
}
...
static void render_sector(raw_editor_context *rec) {
  ...
  gdk_draw_rectangle(d, Closure->drawGC, TRUE, 0, 0, rec->daWidth,
                     rec->daHeight);
  ...
  for (j = 0, y = 0; j < P_VECTOR_SIZE; j++, y += rec->charHeight) {
    for (i = 0, x = 0; i < N_P_VECTORS; i++, x += rec->charWidth) {
      ...
      if (rec->tags[idx]) {
        ...
        gdk_draw_rectangle(d, Closure->drawGC, TRUE, x, y, rec->charWidth,
                           rec->charHeight);
...
static gboolean button_cb(GtkWidget *widget, GdkEventButton *event,
  ...
  case ON_CLICK_CORRECT_Q: {
    ...
    if (type == 'P') {
      ...
      int bytepos = 12 + mouse_x / rec->charWidth +
                    N_P_VECTORS * (mouse_y / rec->charHeight);
      int ignore;
    ...
    if (type == 'P')
      GetPVector(rec->tags, vector, v);
    else
      GetQVector(rec->tags, vector, v);
    ...
    for (i = 0; i < v_size; i++) {
      ...
      if (erasures > 2) {
        SetLabelText(GTK_LABEL(rec->rightLabel),
                     _("%c Vector %d has >2 erasures (nothing done)."), type,
                     v);
        return TRUE;
      }
      eras[erasures++] = i;
    ...
    if (err == 0)
      SetLabelText(GTK_LABEL(rec->rightLabel), _("%c Vector %d already good."),
                   type, v);
    else if (err == 1 || err == 2) {
      if (type == 'P')
        SetPVector(rb->recovered, vector, v);
      else
        SetQVector(rb->recovered, vector, v);
      evaluate_vectors(rec);
      render_sector(rec);
      undo_remember(rec);
      SetLabelText(GTK_LABEL(rec->rightLabel),
                   _("%c Vector %d corrected (%d erasures)."), type, v,
                   e_scratch);
    } else
      SetLabelText(GTK_LABEL(rec->rightLabel),
                   _("%c Vector %d not correctable (%d erasures)."), type, v,
                   e_scratch);
  } break;
  ...
  case ON_CLICK_FIND_OTHER_P: {
    ...
    render_sector(rec);
    SetLabelText(GTK_LABEL(rec->rightLabel),
                 _("Exchanged P vector %d with version %d (of %d)."), v, i + 1,
                 rb->pn[v]);
    rec->lastPFrame = i;
    if (v != rec->lastPVector)
      undo_remember(rec);
  ...
  case ON_CLICK_FIND_OTHER_Q: {
    ...
    bytepos = 12 + mouse_x / rec->charWidth +
              N_P_VECTORS * (mouse_y / rec->charHeight);
    ByteIndexToQ(bytepos, &v, &ignore);
    ...
    render_sector(rec);
    SetLabelText(GTK_LABEL(rec->rightLabel),
                 _("Exchanged Q vector %d with version %d (of %d)."), v, i + 1,
                 rb->qn[v]);
    rec->lastQFrame = i;
    if (v != rec->lastQVector)
      undo_remember(rec);
...
static void action_cb(GtkWidget *widget, gpointer data) {
  ...
  case ACTION_BROWSE_PREV:
    if (rec->currentSample)
      ...
      rec->currentSample = rec->rb->samplesRead - 1;
    rec->sectorChanged = FALSE;
    memcpy(rec->rb->recovered, rec->rbInfo[rec->currentSample].rawSector,
           rec->rb->sampleSize);
    evaluate_vectors(rec);
    render_sector(rec);
    undo_remember(rec);
  ...
  case ACTION_SORT_BY_P:
    ...
    undo_remember(rec);
    SetLabelText(GTK_LABEL(rec->rightLabel),
                 _("Sector with lowest P failures selected."));
    break;
  ...
  case ACTION_SORT_BY_Q:
    ...
    undo_remember(rec);
    SetLabelText(GTK_LABEL(rec->rightLabel),
                 _("Sector with lowest Q failures selected."));
    break;
  ...
  case ACTION_SMART_LEC: {
    ...
    render_sector(rec);
    undo_remember(rec);
    SetLabelText(GTK_LABEL(rec->rightLabel), _("Smart L-EC: %s"), message);
    break;
  }
  }
}
...
void CreateRawEditor(void) {
  ...
  if (!rec->window) /* No window to reuse? */
  {
    ...
    rec->rightLabel = label =
        gtk_label_new(_("Please load a raw sector file!"));
    gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.0);
    gtk_box_pack_start(GTK_BOX(hbox), label, FALSE, FALSE, 5);
    ...
    button = gtk_radio_button_new_with_label_from_widget(
        GTK_RADIO_BUTTON(button), _utf("button|Q vector"));
    g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                     (gpointer)ON_CLICK_CORRECT_Q);
    gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 0);

    button = gtk_radio_button_new_with_label_from_widget(
        GTK_RADIO_BUTTON(button), _utf("button|Find other P"));
    g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                     (gpointer)ON_CLICK_FIND_OTHER_P);
    gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 0);

    button = gtk_radio_button_new_with_label_from_widget(
        GTK_RADIO_BUTTON(button), _utf("button|Find other Q"));
    g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                     (gpointer)ON_CLICK_FIND_OTHER_Q);
    gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 0);

    button = gtk_radio_button_new_with_label_from_widget(
        GTK_RADIO_BUTTON(button), _utf("button|Tag erasures"));
    g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
    ...
    g_signal_connect(G_OBJECT(rec->drawingArea), "expose_event",
                     G_CALLBACK(expose_cb), NULL);
    g_signal_connect(G_OBJECT(rec->drawingArea), "button_press_event",
