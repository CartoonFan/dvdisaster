...
DeviceHandle *open_spti_device(char *device) {
  ...
  if (strlen(device) != 2 || toupper(*device) < 'C' || toupper(*device) > 'Z' ||
      device[1] != ':') {
  ...
  dh->fd = CreateFile(device_path, GENERIC_READ | GENERIC_WRITE,
                      FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
                      0, NULL);

  if (dh->fd ==
      INVALID_HANDLE_VALUE) /* Might be Win9x or missing priviledges */
  {
    g_free(dh->device);
    g_free(dh);
    return NULL;
...
static int send_spti_packet(HANDLE fd, unsigned char *cmd, int cdb_size,
                            char *buf, int size, Sense *sense, int data_mode) {
  ...
  ss.spt.SenseInfoOffset =
      offsetof(SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER, SenseBuf);
  memcpy(ss.spt.Cdb, cmd, cdb_size);

  success = DeviceIoControl(fd, IOCTL_SCSI_PASS_THROUGH_DIRECT, &ss,
                            sizeof(SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER), &ss,
...
DeviceHandle *OpenDevice(char *device) {
  ...
  if ((*device >= 'c' && *device <= 'z') || (*device >= 'C' && *device <= 'Z'))
