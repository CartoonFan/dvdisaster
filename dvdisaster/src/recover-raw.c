...
void DumpSector(RawBuffer *rb, char *path) {
  ...
  fprintf(file,
          ...
          rb->samplesRead, rb->sampleSize, (long long)rb->lba, rb->sampleSize);
...
static void lba_to_msf(int lba, unsigned char *minute, unsigned char *second,
                       unsigned char *frame) {
  *frame = lba % 75;
  lba /= 75;
...
int MSFtoLBA(unsigned char minute_bcd, unsigned char second_bcd,
             unsigned char frame_bcd) {
...
static int int_to_bcd(int value) { return ((value / 10) << 4) | (value % 10); }
...
int CheckMSF(unsigned char *frame, int lba, int sloppy) {
  ...
  if (frame[12] != min || frame[13] != sec || frame[14] != frm)
    return FALSE;
...
static unsigned char sync_pattern[12] = {0,   255, 255, 255, 255, 255,
                                         255, 255, 255, 255, 255, 0};

void InitializeCDFrame(unsigned char *cd_frame, int sector, int xa_mode,
                       int reinitialize) {
  ...
  if (!xa_mode)
    memset(&(cd_frame[2068]), 0,
           8); /* data tracks have always 8 zero bytes here */
}
...
int CheckEDC(unsigned char *cd_frame, int xa_mode) {
  ...
  if (xa_mode) {
    expected_crc = cd_frame[2072] << 24 | cd_frame[2073] << 16 |
                   cd_frame[2074] << 8 | cd_frame[2075];
  } else /* Get CRC from byte position 2064 */
  {
    expected_crc = cd_frame[0x810] << 24 | cd_frame[0x811] << 16 |
                   cd_frame[0x812] << 8 | cd_frame[0x813];
  }
...
static int simple_lec(RawBuffer *rb, unsigned char *frame, char *msg) {
  ...
  if (q_failures || p_failures || q_corrected || p_corrected) {
    PrintCLIorLabel(Closure->status,
                    "Sector %lld  L-EC P/Q results: %d/%d failures, %d/%d "
                    "corrected (%s).\n",
...
static int p_decode(RawBuffer *rb, unsigned char *vector,
                    unsigned char *state) {
  ...
  if (err < 0 || err > 2) {
    ...
    if (erasure_count > 0 && erasure_count <= 2) {
      memcpy(working_vector, vector, P_VECTOR_SIZE);
      err =
          DecodePQ(rb->rt, working_vector, P_PADDING, erasures, erasure_count);
    }
  }
...
static int q_decode(RawBuffer *rb, unsigned char *vector,
                    unsigned char *state) {
  ...
  if (err < 0 || err > 2) {
    ...
    if (erasure_count > 0 && erasure_count <= 2) {
      memcpy(working_vector, vector, Q_VECTOR_SIZE);
      err =
          DecodePQ(rb->rt, working_vector, Q_PADDING, erasures, erasure_count);
    }
  }
...
int IterativeLEC(RawBuffer *rb) {
  ...
  for (;;) /* iterate over P- and Q-Parity until failures converge */
  {
    ...
    if (last_p_failures > p_failures || last_q_failures > q_failures) {
...
int TryCDFrameRecovery(RawBuffer *rb, unsigned char *outbuf) {
  ...
  if (memcmp(rb->recovered, sync_pattern, 12)) {
    ...
    if (CheckEDC(rb->recovered, rb->xaMode) &&
        CheckMSF(rb->recovered, rb->lba, STRICT_MSF_CHECK)) {
      PrintCLIorLabel(Closure->status,
                      "Sector %lld: Recovered in raw reader after correcting "
                      "sync pattern.\n",
                      rb->lba);
  ...
  if (CheckEDC(rb->recovered, rb->xaMode) &&
      CheckMSF(rb->recovered, rb->lba, STRICT_MSF_CHECK)) {
    PrintCLIorLabel(Closure->status,
                    "Sector %lld: Recovered in raw reader by plausible sector "
                    "search (0).\n",
                    rb->lba);
    memcpy(outbuf, rb->recovered + rb->dataOffset, 2048);
    return 0;
  ...
  if (CheckEDC(rb->recovered, rb->xaMode) &&
      CheckMSF(rb->recovered, rb->lba, STRICT_MSF_CHECK)) {
    PrintCLIorLabel(Closure->status,
                    "Sector %lld: Recovered in raw reader by brute force "
                    "plausible sector search (0).\n",
                    rb->lba);
    memcpy(outbuf, rb->recovered + rb->dataOffset, 2048);
    return 0;
  }
