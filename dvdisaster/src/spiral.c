...
Spiral *CreateSpiral(GdkColor *outline, GdkColor *fill, int start_radius,
                     int segment_size, int n_segments) {
...
void DrawSpiralLabel(Spiral *spiral, PangoLayout *layout, char *text,
                     GdkColor *color, int x, int line) {
  GdkDrawable *d = spiral->drawable;
  int w, h, y;

  SetText(layout, text, &w, &h);
  if (line > 0)
    y = spiral->my + spiral->diameter / 2 + 20 + (line - 1) * (10 + h);
  else
    y = spiral->my - spiral->diameter / 2 - 20 - h + (line + 1) * (10 + h);
  gdk_gc_set_rgb_fg_color(Closure->drawGC, color);
