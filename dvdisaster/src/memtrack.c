...
void remember(void *ptr, int size, char *file, int line) {
  ...
  if (phCnt[hash_idx] >= phMax[hash_idx]) {
    if (!phMax[hash_idx])
      ...
      phMax[hash_idx] *= 2;
    if (!(ptrhash[hash_idx] =
              realloc(ptrhash[hash_idx], sizeof(memchunk *) * phMax[hash_idx])))
...
gchar *g_locale_to_utf8_ext(const gchar *str, gssize len, gsize *bytes_read,
                            gsize *bytes_written_out, GError **error,
                            char *file, int line) {
  char *ret;
  gsize bytes_written;
