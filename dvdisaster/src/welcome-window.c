...
static gboolean expose_cb(GtkWidget *widget, GdkEventExpose *event,
  ...
  if (!Closure->drawGC) {
    ...
    MudflapRegister(widget->style, sizeof(GtkStyle),
                    "welcome-window:expose_cb");
    ...
    if (Closure->welcomeMessage ||
        Closure->version != Closure->dotFileVersion) {
      GtkWidget *button;

      Closure->invisibleDash =
          g_strdup_printf("<span color=\"#%02x%02x%02x\">-</span>",
                          bg->red >> 8, bg->green >> 8, bg->blue >> 8);
      AboutText(box,
                _("- Development frameworks for Windows and OS X updated.\n"
...
void CreateWelcomePage(GtkNotebook *notebook) {
  ...
  show_msg =
      Closure->welcomeMessage || Closure->version != Closure->dotFileVersion;
  ...
  if (!show_msg) {
    GtkWidget *widget;
    return;
    widget = gtk_image_new_from_stock("dvdisaster-create",
  ...
  AboutText(box, _("<span weight=\"bold\" size=\"xx-large\">Welcome to "
                   "dvdisaster!</span>"));
  ...
  AboutTextWithLink(
      box, _("Please see the manual for [typical uses] of dvdisaster.\n\n"),
      "howtos.html");
