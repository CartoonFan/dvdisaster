...
static void remove_old_ecc(ecc_closure *ec) {
  if (ec->image->eccHeader) {
    ...
    if (!TruncateImage(ec->image, (guint64)(2048 * data_sectors)))
      Stop(_("Could not truncate %s: %s\n"), Closure->imageName,
           strerror(errno));
  }
}
...
static void check_image(ecc_closure *ec) {
  ...
  for (sectors = 0; sectors < lay->dataSectors; sectors++) {
    ...
    if (n != expected)
      Stop(_("Failed reading sector %lld in image: %s"), sectors,
           strerror(errno));

    /* Look for the dead sector marker */

    err = CheckForMissingSector(
        buf, sectors, image->fpState == FP_PRESENT ? image->imageFP : NULL,
        FINGERPRINT_SECTOR);
    if (err != SECTOR_PRESENT) {
      if (err == SECTOR_MISSING)
        ...
        Stop(
            _("Sector %lld in the image is marked unreadable\n"
    ...
    *crcptr++ = Crc32(buf, 2048);
    ...
    if (last_percent != percent) {
      PrintProgress(_("Preparing image (checksums, adding space): %3d%%"),
                    percent);
...
static void expand_image(ecc_closure *ec) {
  ...
  for (sectors = 0; sectors < lay->eccSectors; sectors++) {
    ...
    CreateMissingSector(buf, lay->dataSectors + sectors, image->imageFP,
                        FINGERPRINT_SECTOR, "RS02 generation placeholder");
...
static void write_crc(ecc_closure *ec) {
  ...
  if (!LargeSeek(image->file, 2048 * crc_sector))
    Stop(_("Failed seeking to sector %lld in image: %s"), crc_sector,
         strerror(errno));
    ...
    for (i = 0; i < lay->ndata; i++) {
      if (layer_index < lay->dataSectors) {
        ...
        if (crc_idx >= 512) {
          ...
          if (n != 2048)
            Stop(_("Failed writing to sector %lld in image: %s"), crc_sector,
                 strerror(errno));
...
static void create_reed_solomon(ecc_closure *ec) {
  ...
  /*** Allocate buffers for the parity calculation and image data caching.
     ...
     n_parity_blocks. We use all the amount of memory allowed by cacheMB for
     caching the parity blocks. */

  n_parity_blocks =
      ((guint64)Closure->cacheMB << 20) / (guint64)nroots; /* 1 MB = 2^20 */
  ...
  /* Each chunk of parity blocks is built iteratively by processing the data in
     layers (first all bytes at pos 0, then pos 1, until ndata layers have been
     processed). So we need to buffer n_layer_bytes = n_parity_blocks of input
     data. For practical reasons we require that the layer size is a multiple of
     the medium sector size of 2048 bytes. */
  ...
  for (chunk = 0; chunk < lay->sectorsPerLayer; chunk += n_layer_sectors) {
    ...
    if (chunk + n_layer_sectors < lay->sectorsPerLayer)
      ...
      actual_layer_sectors = lay->sectorsPerLayer - chunk;
    ...
    sp = nroots -
         ndata % nroots; /* => (ndata + sp) mod nroots = 0 so that parity */
    ...
    for (layer = 0; layer < ndata; layer++) {
      ...
      for (si = 0; si < actual_layer_bytes; si++) {
        ...
        if (feedback != GF_ALPHA0) /* non-zero feedback term */
        {
          ...
          {
          case 170:
            par_idx[spk++] ^= enc_alpha_to[feedback + *--gpoly];
          case 169:
            par_idx[spk++] ^= enc_alpha_to[feedback + *--gpoly];
          case 168:
            par_idx[spk++] ^= enc_alpha_to[feedback + *--gpoly];
...
void RS02Create(void) {
  ...
  lay = ec->lay = CalcRS02Layout(image->sectorSize, 0);

  /*** Announce what we are going to do */

  if (Closure->guiMode) /* Preliminary fill text for the head line */
  {
    ec->msg = g_strdup_printf(_("Encoding with Method RS02: %lld MB data, %lld "
                                "MB ecc (%d roots; %4.1f%% redundancy)."),
                              lay->dataSectors / 512, lay->eccSectors / 512,
    ...
    ec->msg =
        g_strdup_printf(_("Augmenting image with Method RS02:\n %lld MB data, "
                          "%lld MB ecc (%d roots; %4.1f%% redundancy)."),
                        lay->dataSectors / 512, lay->eccSectors / 512,
                        lay->nroots, lay->redundancy);
