...
static void recursive_q_correction(RawBuffer *, unsigned char *, int, int,
                                   int *, int *);

static void recursive_p_correction(RawBuffer *rb, unsigned char *frame,
                                   int byte_idx, int new_byte, int *better,
                                   int *worse) {
  ...
  recursive_q_correction(rb, frame, PToByteIndex(p, pos), p_vector[pos], better,
                         worse);
}
...
static void recursive_q_correction(RawBuffer *rb, unsigned char *frame,
  ...
  recursive_p_correction(rb, frame, QToByteIndex(q, pos), q_vector[pos], better,
                         worse);
}
  ...
  for (q = 0; q < N_Q_VECTORS; q++) {
    ...
    while (n_iterations--) {
      ...
      if (q_after <= q_before)
        ...
        malus += (q_after - q_before) * MALUS_CROSSED_DESTROY;

      verbose(" Q:%d->%d; bonus: %3d malus: %3d\n", q_before, q_after, bonus,
              malus);
      ...
      if (q_after == 1) {
        ...
        recursive_p_correction(rb, scratch, byte_idx, vector[pos], &better,
                               &worse);
      ...
      if (q_after < 2 && found_better_solution(shc, bonus, malus)) {
        ...
        snprintf(shc->msg, SMART_LEC_MESSAGE_SIZE,
                 "Q%02d improved from %d->%d by crossing P vectors (bonus %d/ "
                 "malus %d).",
  ...
  for (p = 0; p < N_P_VECTORS; p++) {
    ...
    while (n_iterations--) {
      ...
      if (p_after <= p_before)
        ...
        malus += (p_after - p_before) * MALUS_CROSSED_DESTROY;

      verbose(" P:%d->%d; bonus: %3d malus: %3d\n", p_before, p_after, bonus,
              malus);
      ...
      if (p_after == 1) {
        ...
        recursive_q_correction(rb, scratch, byte_idx, vector[pos], &better,
                               &worse);
        verbose("  recursive gain: better/worse: %d / %d\n", better, worse);
      ...
      if (p_after < 2 && found_better_solution(shc, bonus, malus)) {
        ...
        snprintf(shc->msg, SMART_LEC_MESSAGE_SIZE,
                 "P%02d improved from %d->%d by crossing Q vectors (bonus %d/ "
                 "malus %d).",
...
static void evaluate_new_frame(sh_context *shc, unsigned char *new, int *better,
                               int *worse) {
  RawBuffer *rb = shc->rb;
  unsigned char vector[Q_VECTOR_SIZE];
  int p, q;
  ...
  for (p = 0; p < N_P_VECTORS; p++) {
    ...
    if (err > shc->pState[p])
      *worse += err - shc->pState[p];
    else
      *better += shc->pState[p] - err;
  }
  ...
  for (q = 0; q < N_Q_VECTORS; q++) {
    ...
    if (err > shc->qState[q])
      *worse += err - shc->qState[q];
    else
      *better += shc->qState[q] - err;
  }
}
  ...
  for (p = 0; p < N_P_VECTORS; p++) {
    ...
    for (i = 0; i < rb->pn[p]; i++) {
      ...
      if (found_better_solution(shc, bonus, malus)) {
        ...
        snprintf(shc->msg, SMART_LEC_MESSAGE_SIZE,
                 "Found alternative vector for P%02d (bonus %d/ malus %d).", p,
                 bonus, malus);
      }
    }
  }
  ...
  for (q = 0; q < N_Q_VECTORS; q++) {
    ...
    for (i = 0; i < rb->qn[q]; i++) {
      ...
      if (found_better_solution(shc, bonus, malus)) {
        memcpy(shc->bestFrame, scratch, rb->sampleSize);
        snprintf(shc->msg, SMART_LEC_MESSAGE_SIZE,
                 "Found alternative vector for Q%02d (bonus %d/ malus %d).", q,
                 bonus, malus);
      }
    }
  }
        ...
        if (current_p[j] != alternative_p[j]) {
          ...
          if (shc->qState[crossed] > 0) {
            ...
            if (better > 0) {
              ...
              bonus =
                  better * BONUS_CROSSED_CORRECT - cycle_penalty(shc, scratch);
              ...
              if (found_better_solution(shc, bonus, malus)) {
                ...
                snprintf(shc->msg, SMART_LEC_MESSAGE_SIZE,
                         "Q%02d improved by swapping %d in P%02d, %d/%d (bonus "
                         "%d/ malus %d).",
          ...
          if (shc->pState[crossed] > 0) {
            ...
            if (better > 0) {
              ...
              if (found_better_solution(shc, bonus, malus)) {
                ...
                snprintf(shc->msg, SMART_LEC_MESSAGE_SIZE,
                         "P%02d improved by swapping %d in Q%02d, %d/%d (bonus "
                         "%d/ malus %d).",
...
static void recursive_double_q_correction(RawBuffer *, unsigned char *, int,
                                          int, int, int, int *, int *);
...
static void recursive_double_p_correction(RawBuffer *rb, unsigned char *frame,
  ...
  if (p1 != p2) {
    ...
    new_err2 = old_err2 = new_err1;

  verbose("rec 2p: old %d %d, new %d %d - %d %d\n", old_err1, old_err2,
          new_err1, new_err2, p1, p2);
  ...
  if (new_err1 < old_err1 && new_err2 < old_err2) {
    ...
    recursive_double_q_correction(rb, frame, PToByteIndex(p1, pos1),
                                  p_vector1[pos1], PToByteIndex(p2, pos2),
                                  p_vector2[pos2], better, worse);
    return;
  }
...
static void recursive_double_q_correction(RawBuffer *rb, unsigned char *frame,
  ...
  if (new_err1 < old_err1 && new_err2 < old_err2) {
    ...
    recursive_double_p_correction(rb, frame, QToByteIndex(q1, pos1),
                                  q_vector1[pos1], QToByteIndex(q2, pos2),
                                  q_vector2[pos2], better, worse);
    return;
  }
  ...
  for (p = 0; p <= N_P_VECTORS; p++) {
    ...
    for (e1 = 0; e1 < n_q - 1; e1++)
      for (e2 = e1 + 1; e2 < n_q; e2++) {
        ...
        better = worse = 0;
        memcpy(scratch, rb->recovered, rb->sampleSize);
        recursive_double_q_correction(
            rb, scratch, QToByteIndex(crossed_q[e1], crossed_q_idx[e1]),
            p_vector[erasure_pos[e1]],
            QToByteIndex(crossed_q[e2], crossed_q_idx[e2]),
            p_vector[erasure_pos[e2]], &better, &worse);

        if (better > 0) {
          int bonus, malus;

          verbose("P%02d with erasure pos %02d/%02d: %d better, %d worse\n", p,
                  erasure_pos[e1], erasure_pos[e2], better, worse);

          bonus = better * BONUS_CROSSED_CORRECT - cycle_penalty(shc, scratch);
          malus = worse * MALUS_CROSSED_DESTROY;
          ...
          if (found_better_solution(shc, bonus, malus)) {
            ...
            snprintf(shc->msg, SMART_LEC_MESSAGE_SIZE,
                     "P%02d with erasures %02d/%02d: %d better, %d worse "
                     "(bonus %d/ malus %d).",
                     p, erasure_pos[e1], erasure_pos[e2], better, worse, bonus,
                     malus);
          }
        }
      }
    ...
    for (i = 0; i < rb->pn[p]; i++) /* try all alternative p vectors */
    {
      ...
      while (n_iterations--) {
        ...
        for (j = 0; j < n_q; j++) {
          ...
          err = decode_q(rb, q_vector, &err_pos);
          if (err == 1)
            SetQVector(scratch, q_vector, crossing_q[j]);
        }
        ...
        if (err == 1) {
          ...
          if (prev_state > new_state) {
            bonus =
                SWAPPED_WITH_IMPROVEMENT_BONUS - cycle_penalty(shc, scratch);

            if (found_better_solution(shc, bonus, malus)) {
              memcpy(shc->bestFrame, scratch, rb->sampleSize);
              snprintf(shc->msg, SMART_LEC_MESSAGE_SIZE,
                       "P%02d swapped; Q%02d improved (%d/%d)", p, crossed_q,
                       bonus, malus);
            }
          }
        }
        ...
        if (current_p[j] != alternative_p[j]) {
          ...
          if (err == 1) {
            ...
            for (k = 0; k < Q_VECTOR_SIZE; k++) {
              ...
              if (!err)
                bonus += INDIRECT_IMPROVEMENT_BONUS_FOR_Q;
              ...
              if (err > shc->pState[p])
                malus += INDIRECT_IMPROVEMENT_MALUS_FOR_P;
              ...
              if (found_better_solution(shc, bonus, malus)) {
                memcpy(shc->bestFrame, scratch, rb->sampleSize);
                snprintf(shc->msg, SMART_LEC_MESSAGE_SIZE,
                         "Q%02d improved indirectly by P%02d, P%02d (%d/%d)", q,
                         p, p2, bonus, malus);
              }
            }
          }
...
int SmartLEC(RawBuffer *rb) {
  ...
  CreateMissingSector(prev_state, rb->lba, NULL, 0, "SmartLEC() dummy sector");
  while (TRUE) {
...
void EndIterativeSmartLEC(void *shc) { free_sh_context((sh_context *)shc); }
