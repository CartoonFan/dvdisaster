...
void InitializeCurve(void *rc_ptr, int max_rate, int can_c2) {
  ...
  if (can_c2)
    Closure->readLinearCurve->enable = DRAW_FCURVE | DRAW_LCURVE;
  else
    Closure->readLinearCurve->enable = DRAW_FCURVE;
...
static gboolean curve_idle_func(gpointer data) {
  ...
  g_snprintf(buf, 80, _("Unreadable / skipped sectors: %lld"),
             Closure->readErrors);
  ...
  for (i = rc->lastSegment; i < ci->percent; i++)
    ...
    case 0:
      DrawSpiralSegment(Closure->readLinearSpiral, Closure->blueSector, i);
      break;
    case 1:
      DrawSpiralSegment(Closure->readLinearSpiral, Closure->greenSector, i);
      break;
    case 2:
      DrawSpiralSegment(Closure->readLinearSpiral, Closure->redSector, i);
      break;
    case 3:
      DrawSpiralSegment(Closure->readLinearSpiral, Closure->darkSector, i);
      break;
    case 4:
      DrawSpiralSegment(Closure->readLinearSpiral, Closure->yellowSector, i);
      break;
    }
  ...
  if (resize_curve) {
    Closure->readLinearCurve->maxY =
        Closure->readLinearCurve->fvalue[ci->percent] + 1;

    update_geometry();
    gdk_window_clear(Closure->readLinearDrawingArea->window);
    redraw_curve();
    rc->lastPlotted = ci->percent;
    rc->lastPlottedY = CurveY(Closure->readLinearCurve,
                              Closure->readLinearCurve->fvalue[ci->percent]);
    g_free(ci);
    rc->activeRenderers--;
    return FALSE;
  }
  ...
  y0 = CurveY(Closure->readLinearCurve,
              Closure->readLinearCurve->fvalue[rc->lastPlotted]);
  if (rc->lastPlottedY)
    y0 = rc->lastPlottedY;
  ...
  for (i = rc->lastPlotted + 1; i <= ci->percent; i++) {
    ...
    gint y1 =
        CurveY(Closure->readLinearCurve, Closure->readLinearCurve->fvalue[i]);
    gint l1 = CurveLogY(Closure->readLinearCurve,
                        Closure->readLinearCurve->lvalue[i]);
    ...
    if (Closure->readLinearCurve->lvalue[i]) {
      ...
      gdk_draw_rectangle(Closure->readLinearDrawingArea->window,
                         Closure->drawGC, TRUE, x0, l1, x0 == x1 ? 1 : x1 - x0,
                         Closure->readLinearCurve->bottomLY - l1);
    }
    if (x0 < x1) {
      gdk_gc_set_rgb_fg_color(Closure->drawGC, Closure->curveColor);
      gdk_draw_line(Closure->readLinearDrawingArea->window, Closure->drawGC, x0,
                    y0, x1, y1);
...
static void update_geometry(void) {
  ...
  Closure->readLinearSpiral->mx =
      a->width - 15 - Closure->readLinearSpiral->diameter / 2;
  Closure->readLinearSpiral->my = a->height / 2;

  if (Closure->eccType != ECC_NONE || Closure->crcErrors) {
    int w, h;

    SetText(Closure->readLinearCurve->layout, _("Sectors with CRC errors"), &w,
            &h);
  ...
  gtk_box_set_child_packing(GTK_BOX(Closure->readLinearFootlineBox),
                            Closure->readLinearSpeed, TRUE, TRUE,
                            Closure->readLinearCurve->leftX, GTK_PACK_START);
  gtk_box_set_child_packing(GTK_BOX(Closure->readLinearFootlineBox),
                            Closure->readLinearErrors, TRUE, TRUE,
                            Closure->readLinearCurve->leftX, GTK_PACK_START);
}
...
static void redraw_curve(void) {
  ...
  gdk_draw_layout(d, Closure->drawGC, x, Closure->readLinearCurve->topY - h - 5,
  ...
  if (Closure->eccType != ECC_NONE || Closure->crcErrors)
    DrawSpiralLabel(Closure->readLinearSpiral, Closure->readLinearCurve->layout,
                    _("Sectors with CRC errors"), Closure->yellowSector, x,
                    pos++);
...
void CreateLinearReadWindow(GtkWidget *parent) {
  ...
  gtk_label_set_ellipsize(GTK_LABEL(Closure->readLinearHeadline),
                          PANGO_ELLIPSIZE_END);
  gtk_box_pack_start(GTK_BOX(parent), Closure->readLinearHeadline, FALSE, FALSE,
                     3);
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), Closure->readLinearFootline,
                           ignore);

  Closure->readLinearCurve =
      CreateCurve(d_area, _("Speed"), "%dx", 1000, CURVE_MEGABYTES);
  Closure->readLinearCurve->leftLogLabel = g_strdup(_("C2 errors"));
  Closure->readLinearSpiral =
      CreateSpiral(Closure->grid, Closure->background, 10, 5, 1000);
}
