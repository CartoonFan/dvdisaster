...
CrcBuf *RS02GetCrcBuf(Image *image) {
  ...
  if (csc->lay)
    g_free(csc->lay);
  lay = csc->lay = CalcRS02Layout(uchar_to_gint64(image->eccHeader->sectors),
                                  image->eccHeader->eccBytes);
...
void RS02UpdateCksums(Image *image, gint64 sector, unsigned char *buf) {
  ...
  if (sector < csc->lay->dataSectors) {
    if (sector < csc->lay->dataSectors - 1)
      ...
      MD5Update(&csc->dataCtxt, buf, image->eccHeader->inLast);
  }

  /* md5sum the crc portion */
  if (sector >= csc->lay->dataSectors + 2 &&
      sector < csc->lay->protectedSectors)
 ...
 ****
 * Reading sectors beyond lay->protectedSectors always returns a zero padding
 *sector.
 */

void RS02ReadSector(Image *image, RS02Layout *lay, unsigned char *buf,
                    gint64 s) {
  int n;
  ...
  if (s == lay->firstEccHeader || s == lay->firstEccHeader + 1) {
    memset(buf, 0, 2048);
    return;
  }
...
gint64 RS02EccSectorIndex(RS02Layout *lay, gint64 slice, gint64 n) {
  ...
  nh = ecc_idx /
       (lay->headerModulo - 2); /* number of interleaved headers crossed */
...
gint64 RS02SectorIndex(RS02Layout *lay, gint64 slice, gint64 n) {
  ...
  if (slice < lay->ndata) {
    ...
    if (sector < lay->protectedSectors)
      ...
      return -1; /* padding sector (invalid)! */
  }
...
void RS02SliceIndex(RS02Layout *lay, gint64 sector, gint64 *slice, gint64 *n) {
  ...
  first_repeat =
      (lay->protectedSectors + lay->headerModulo - 1) / lay->headerModulo;
  ...
  if (sector < first_repeat) {
    *slice =
        lay->ndata + (sector - lay->protectedSectors) / lay->sectorsPerLayer;
  ...
  base = first_repeat -
         lay->protectedSectors; /* ecc sectors before first repeat */

  sector -= first_repeat;
  sector -= 2; /* subtract first repeated header */
  sector -=
      2 * (sector / (lay->headerModulo - 0)); /* and other crossed repeats */
...
RS02Layout *CalcRS02Layout(gint64 data_sectors, int requested_roots) {
  ...
  lay->protectedSectors =
      lay->dataSectors + 2 + lay->crcSectors; /* two sectors for header */
  ...
  if (!Closure->guiMode && !requested_roots && Closure->redundancy) {
    ...
    case '%': /* pick redundancy directly */
    {
      ...
      for (requested_roots = 7; requested_roots < 171; requested_roots++) {
        double redundancy = ((double)requested_roots * 100.0) /
                            ((double)(GF_FIELDMAX - requested_roots));
  ...
  if (requested_roots > 0)
    ...
    lay->rsSectors =
        lay->mediumCapacity - lay->protectedSectors; /* just to start */
    lay->nroots = (GF_FIELDMAX * lay->rsSectors) /
                  lay->mediumCapacity; /* iteration below */
  }
  ...
  ecc_area =
      lay->nroots * ((lay->protectedSectors + lay->ndata - 1) / lay->ndata);
  ...
  while (lay->nroots > 7) {
    gint64
        first_repeat;   /* first header which is interleaved with ecc sectors */
    ...
    // lay->headers  = 1 + (lay->rsSectors + lay->protectedSectors -
    // first_repeat) / lay->headerModulo;

    lay->eccSectors = 2 + lay->crcSectors + lay->rsSectors + 2 * lay->headers;
    lay->sectorsPerLayer =
        (lay->protectedSectors + lay->ndata - 1) / lay->ndata;
