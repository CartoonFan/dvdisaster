...
static void init_defective_sector_file(char *path, RawBuffer *rb,
                                       LargeFile **file,
...
static void open_defective_sector_file(RawBuffer *rb, char *path,
                                       LargeFile **file,
                                       DefectiveSectorHeader *dsh) {
  guint64 length;
  int n;
  ...
  *file = LargeOpen(path, O_RDWR, IMG_PERMS);
  ...
  if (dsh->sectorSize == 2352) /* old non C2-style raw dump? */
  {
    ...
    for (i = 0, ptr = buf; i < dsh->nSectors; i++, ptr += 2352) {
      ...
      if (n != dsh->sectorSize)
        Stop(_("Failed reading from defective sector file: %s"),
             strerror(errno));
    }
  ...
  if ((dsh->properties & DSH_HAS_FINGERPRINT) && rb->validFP) {
    if (memcmp(dsh->mediumFP, rb->mediumFP, 16))
      Stop(
          _("Fingerprints of medium and defective sector cache do not match!"));
  }
}
...
int SaveDefectiveSector(RawBuffer *rb, int can_c2_scan) {
  ...
  filename = g_strdup_printf("%s/%s%lld.raw", Closure->dDumpDir,
                             Closure->dDumpPrefix, (long long)rb->lba);
  ...
  for (i = 0; i < rb->samplesRead; i++) {
    ...
    for (j = 0; j < i;
         j++) /* Some drives return cached data after first read */
    {
  ...
  PrintCLIorLabel(Closure->status,
                  _(" [Appended %d/%d sectors to cache file %s; LBA=%lld, "
                    "ssize=%d, %d sectors]\n"),
                  count, rb->samplesRead, filename, dsh->lba, dsh->sectorSize,
                  dsh->nSectors);
...
int TryDefectiveSectorCache(RawBuffer *rb, unsigned char *outbuf) {
  ...
  path = g_strdup_printf("%s/%s%lld.raw", Closure->dDumpDir,
                         Closure->dDumpPrefix, (long long)rb->lba);
...
void ReadDefectiveSectorFile(DefectiveSectorHeader *dsh, RawBuffer *rb,
                             char *path) {
  LargeFile *file;
