...
static void ecc_cleanup(gpointer data) {
  ...
  if (ec->image)
    CloseImage(ec->image);
  if (ec->lock)
    g_mutex_free(ec->lock);
  if (ec->ioCond)
    g_cond_free(ec->ioCond);
  if (ec->eh)
    g_free(ec->eh);
  if (ec->rt)
    FreeReedSolomonTables(ec->rt);
  if (ec->gt)
    FreeGaloisTables(ec->gt);
  if (ec->writeHandle)
    LargeClose(ec->writeHandle);
  if (ec->paritybase)
    g_free(ec->paritybase);
  if (ec->msg)
    g_free(ec->msg);
  if (ec->avgTimer)
    g_timer_destroy(ec->avgTimer);
  if (ec->contTimer)
    g_timer_destroy(ec->contTimer);
  if (ec->firstCrc)
    g_free(ec->firstCrc);
...
static void abort_encoding(ecc_closure *ec, int truncate) {
  ...
  if (truncate && ec->lay) {
    ...
    else if (!LargeTruncate(ec->image->file,
                            (gint64)(2048 * ec->lay->dataSectors)))
      Stop(_("Could not truncate %s: %s\n"), Closure->imageName,
           strerror(errno));

    SetLabelText(GTK_LABEL(wl->encFootline),
                 _("<span %s>Aborted by user request!</span> (partial ecc data "
                   "removed from image)"),
                 Closure->redMarkup);
  } else {
    SetLabelText(GTK_LABEL(wl->encFootline),
...
static void remove_old_ecc(ecc_closure *ec) {
  ...
  if (ec->image->eccHeader) {
    ...
    if (!LargeTruncate(ec->image->file, (gint64)(2048 * data_sectors)))
      Stop(_("Could not truncate %s: %s\n"), Closure->imageName,
           strerror(errno));
...
static void expand_image(ecc_closure *ec) {
  ...
  for (sectors = 0; sectors < lay->dataPadding; sectors++) {
    ...
    CreatePaddingSector(pad_sector, lay->dataSectors + sectors + 2,
  ...
  for (sectors = 0; sectors < lay->sectorsPerLayer; sectors++) {
    ...
    CreateMissingSector(pad_sector, lay->firstCrcPos + sectors, image->imageFP,
                        FINGERPRINT_SECTOR, "CRC padding by expand_image()");
  ...
  for (sectors = 0; sectors < ecc_padding; sectors++) {
    ...
    CreateMissingSector(dead_sector, lay->firstEccPos + sectors, image->imageFP,
                        FINGERPRINT_SECTOR, "ECC padding by expand_image()");
...
static void flip_buffers(ecc_closure *ec) {
  ...
  ctmp = ec->ioCrc;
  ec->ioCrc = ec->encoderCrc;
  ec->encoderCrc = ctmp;
  dtmp = ec->ioData;
  ec->ioData = ec->encoderData;
  ec->encoderData = dtmp;
  dtmp = ec->ioMmapBase;
  ec->ioMmapBase = ec->encoderMmapBase;
  ec->encoderMmapBase = dtmp;
  etmp = ec->ioMmapSize;
  ec->ioMmapSize = ec->encoderMmapSize;
  ec->encoderMmapSize = etmp;
}
...
static void read_next_chunk(ecc_closure *ec, guint64 chunk) {
  ...
  for (layer = 0; layer < lay->ndata - 1; layer++) /* exclude CRC layer */
  {
    ...
    if (Closure->eccTarget == ECC_FILE &&
      ...
      RS03ReadSectors(ec->image, lay, ec->ioData[layer], layer, ec->ioChunk,
                      n_sectors, RS03_READ_DATA);
    } else /* can use memory mapping */
    {
      ...
      if (ec->ioLayerSectors == ec->chunkSize)
        ...
        ec->ioMmapSize[layer] = 2048 * ec->ioLayerSectors + shift;

      ec->ioMmapBase[layer] =
          mmap(NULL, ec->ioMmapSize[layer], PROT_READ, MMAP_FLAGS,
               ec->image->file->fileHandle, page_offset);
...
#else  /* NO MMAP */
    RS03ReadSectors(ec->image, lay, ec->ioData[layer], layer, ec->ioChunk,
                    ec->ioLayerSectors, RS03_READ_DATA);
#endif /* HAVE_MMAP */
    ...
    if (err != SECTOR_PRESENT) { /* Remove partial ecc data */
      if (Closure->eccTarget == ECC_FILE) {
        LargeClose(ec->writeHandle);
        ec->writeHandle = NULL;
        LargeUnlink(Closure->eccName);
      ...
      Stop(_("Incomplete image\n\n"
           ...
           err == SECTOR_MISSING
               ? "\n"
               : _("\nThis image was probably mastered from defective "
                   "source(s).\n"
...
#ifndef HAVE_MMAP
    if (ec->ioChunk + ec->ioLayerSectors < lay->sectorsPerLayer) {
      RS03ReadSectors(ec->image, lay, ec->ioData[layer] + ec->chunkBytes, layer,
                      ec->ioChunk + ec->ioLayerSectors, 1, RS03_READ_DATA);
    }
...
static void flush_crc(ecc_closure *ec, LargeFile *file_out) {
  ...
  if (!LargeSeek(file_out, crc_sect)) {
    ...
    Stop(_("Failed seeking to sector %lld in image: %s"), crc_sect,
         strerror(errno));
  }
  for (i = 0; i < ec->encoderLayerSectors; i++)
    if (LargeWrite(file_out, ec->encoderCrc + 512 * i, 2048) != 2048) {
      ec->abortImmediately = TRUE;
      Stop(_("Failed writing to sector %lld in image: %s"), crc_sect,
           strerror(errno));
    }
}
...
static gpointer io_thread(ecc_closure *ec) {
  ...
  Verbose(
      "Cache allocation: %lldK+%lldK+%lldK=%lldM (data+parity+descrambling)\n",
      (long long)((2 * ec->chunkBytes * ndata) / 1024),
      (long long)((n_parity_bytes) / 1024),
      (long long)((ec->chunkBytes * nroots) / 1024),
      (long long)((2 * ec->chunkBytes * ndata + n_parity_bytes +
                   ec->chunkBytes * nroots) /
                  (1024 * 1024)));
  ...
  for (chunk = 0; chunk < lay->sectorsPerLayer; chunk += ec->chunkSize) {
    ...
    if (parity_available) { // flush_crc(ec, file_out);
...
static gpointer encoder_thread(ecc_closure *ec) {
  ...
  for (;;) {
    ...
    g_mutex_lock(ec->lock);
    while (ec->sectorsToEncode && !ec->abortImmediately &&
...
#if 1
      if (layer < ndata - 1) {
        /* The first ecc block CRC needs to be cached for
                                    wrap-around */
    ...
    if (ec->lastPercent != percent) {
      ...
      if (Closure->guiMode) {
        ...
        if (elapsed > 1.0) {
          gdouble mbs = ((double)ndata * (ec->progress - ec->lastProgress)) /
                        (512.0 * elapsed);
        ...
        PrintProgress(_("Ecc generation: %3d.%1d%%"), percent / 10,
                      percent % 10);
    } else
      g_mutex_unlock(ec->lock);
...
static void create_reed_solomon(ecc_closure *ec) {
  ...
  if (Closure->guiMode) {
    ...
    if (Closure->useSSE2)
      ...
      SetLabelText(GTK_LABEL(ec->wl->encThreads), _("%d threads"),
                   Closure->codecThreads);
    SetLabelText(GTK_LABEL(ec->wl->encPerformance), "");
  ...
  for (i = 0; i < Closure->codecThreads; i++) {
    ...
    ec->thread[i] =
        g_thread_create((GThreadFunc)encoder_thread, (gpointer)ec, TRUE, &err);
    if (!ec->thread[i]) {
      g_mutex_unlock(ec->lock);
      ec->abortImmediately = TRUE;
...
void RS03Create(void) {
  ...
  if (Closure->guiMode) /* Preliminary fill text for the head line */
    SetLabelText(GTK_LABEL(wl->encHeadline),
                 _("<big>Augmenting the image with error correction "
                   "data.</big>\n<i>%s</i>"),
                 _("- checking image -"));
  ...
  lay = ec->lay = CalcRS03Layout(image->sectorSize, 0, Closure->eccTarget);
  lay->eh = ec->eh;
  ...
  if (Closure->guiMode) /* Preliminary fill text for the head line */
  {
    ec->msg = g_strdup_printf(_("Encoding with Method RS03: %lld MB data, %lld "
                                "MB ecc (%d roots; %4.1f%% redundancy)."),
                              lay->dataSectors / 512, ecc_sectors / 512,
    ...
    if (lay->target == ECC_IMAGE)
      ...
      SetLabelText(
          GTK_LABEL(wl->encHeadline),
          _("<big>Creating the error correction file.</big>\n<i>%s</i>"),
          ec->msg);

  } else {
    if (Closure->eccTarget == ECC_IMAGE)
      ec->msg = g_strdup_printf(
          _("Augmenting image with Method RS03 [%d threads]:\n"
            "%lld MB data, %lld MB ecc (%d roots; %4.1f%% redundancy)."),
          Closure->codecThreads, lay->dataSectors / 512, ecc_sectors / 512,
          lay->nroots, lay->redundancy);
    else
      ec->msg = g_strdup_printf(
          _("Creating the error correction file with Method RS03 [%d "
            "threads]:\n"
            "%lld MB data, %lld MB ecc (%d roots; %4.1f%% redundancy)."),
          Closure->codecThreads, lay->dataSectors / 512, ecc_sectors / 512,
          lay->nroots, lay->redundancy);
  ...
  if (Closure->eccTarget == ECC_IMAGE && lay->nroots < 8)
    Stop(_("Not enough space on medium left for error correction data.\n"
           "Data portion of image: %lld sect.; maximum possible size: %lld "
           "sect.\n"
  ...
  if (Closure->eccTarget == ECC_IMAGE)
    PrintLog(_("Image has been augmented with error correction data.\n"
             ...
             (lay->totalSectors) / 512, lay->totalSectors);
  else
  ...
  if (Closure->guiMode) {
    ...
    SetLabelText(GTK_LABEL(ec->wl->encBottleneck),
                 _("%d times CPU bound; %d times I/O bound"), ec->cpuBound,
                 ec->ioBound);
  }
