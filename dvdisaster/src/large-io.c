...
LargeFile *LargeOpen(char *name, int flags, mode_t mode) {
  ...
  if ((stat(cp_path, &mystat) == 0) && !S_ISREG(mystat.st_mode)) {
    g_free(cp_path), g_free(lf);
    return NULL;
  }
...
int LargeEOF(LargeFile *lf) { return lf->offset >= lf->size; }
...
static void insert_buttons(GtkDialog *dialog) {
  gtk_dialog_add_buttons(dialog, GTK_STOCK_REDO, 1, GTK_STOCK_CANCEL, 0, NULL);
}
