...
static void fix_cleanup(gpointer data) {
  ...
  if (Closure->guiMode) {
    if (fc->earlyTermination)
      SwitchAndSetFootline(fc->wl->fixNotebook, 1, fc->wl->fixFootline,
  ...
  if (fc->lay)
    g_free(fc->lay);
  if (fc->gt)
    FreeGaloisTables(fc->gt);
  if (fc->rt)
    FreeReedSolomonTables(fc->rt);
...
static void expand_image(Image *image, gint64 new_size) {
  ...
  for (sectors = 0; sectors < new_sectors; sectors++) {
    ...
    CreateMissingSector(buf, image->sectorSize + sectors, image->imageFP,
                        FINGERPRINT_SECTOR, "RS03 fix placeholder");
...
void RS03Fix(Image *image) {
  ...
  if (eh->methodFlags[0] & MFLAG_ECC_FILE)
    msg = g_strdup_printf(
        _("Error correction file using Method RS03, %d roots, %4.1f%% "
          "redundancy."),
        eh->eccBytes, ((double)eh->eccBytes * 100.0) / (double)eh->dataBytes);
  else
    msg = g_strdup_printf(
        _("Image contains error correction data: Method RS03, %d roots, "
          "%4.1f%% redundancy."),
        eh->eccBytes, ((double)eh->eccBytes * 100.0) / (double)eh->dataBytes);
  ...
  if (image->sectorSize > expected_sectors) {
    ...
    if (diff > 0 && diff <= 2) {
      int answer =
          ModalWarning(GTK_MESSAGE_QUESTION, GTK_BUTTONS_OK_CANCEL, NULL,
    ...
    if (diff > 2 && Closure->guiMode) {
      int answer =
          ModalDialog(GTK_MESSAGE_QUESTION, GTK_BUTTONS_OK_CANCEL, NULL, trans,
                      diff, _("Is it okay to remove the superfluous sectors?"));
  ...
  cache_size =
      2 *
      Closure->cacheMB; /* ndata+nroots=255 medium sectors are approx. 0.5MB */
  ...
  RS03ReadSectors(image, lay, (unsigned char *)last_crc_sector2, lay->ndata - 1,
                  lay->sectorsPerLayer - 1, 1, RS03_READ_CRC);
  ...
  for (s = 0; s < lay->sectorsPerLayer; s++) {
    ...
    if (cache_sector >= cache_size) {
      ...
      for (i = 0; i < ndata - 1; i++) {
        RS03ReadSectors(image, lay, fc->imgBlock[i], i, ecc_idx, cache_size,
                        RS03_READ_DATA);
      }
      ...
      memcpy(last_crc_sector2,
             fc->imgBlock[ndata - 1] + 2048 * (cache_size - 1), 2048);
    ...
    if (cache_sector == 0) {
      ...
      err = CheckForMissingSector((unsigned char *)crc_buf, lay->firstCrcPos,
      ...
      /* fixme: replace 0 with real CRC sector number */
      err =
          CheckForMissingSector((unsigned char *)crc_buf, block_idx[ndata - 1],
                                eh->mediumFP, eh->fpSector);
      crc_valid = (err == SECTOR_PRESENT);
    }
    crc_idx = 0;
    ...
    for (i = 0; i < lay->ndata; i++) {
      int err = CheckForMissingSector(fc->imgBlock[i] + cache_offset,
                                      block_idx[i], eh->mediumFP, eh->fpSector);
    ...
    for (i = lay->ndata; i < GF_FIELDMAX; i++) {
      int err = CheckForMissingSector(fc->imgBlock[i] + cache_offset,
                                      ...
                                      eh->mediumFP, eh->fpSector);
      // FIXME: wrong sector number in ecc files
      if (err) {
        erasure_map[i] = 1;
        erasure_list[erasure_count++] = i;
        damaged_sectors++;
    ...
    {
      if (!Closure->guiMode) {
        PrintCLI(_("* Ecc block %lld: %3d unrepairable sectors: "), s,
                 erasure_count);

        for (i = 0; i < erasure_count; i++) {
          gint64 loc = erasure_list[i];
          PrintCLI("%lld ", loc < ndata
                                ? block_idx[loc]
                                : fc->eccIdx[loc - ndata][cache_sector]);
        }
        PrintCLI("\n");
      }
    ...
    for (bi = 0; bi < 2048; bi++) /* Run through each ecc block byte */
    {
      ...
      int lambda[nroots + 1],
          syn[nroots]; /* Err+Eras Locator poly * and syndrome poly */
      ...
      for (j = 1; j < GF_FIELDMAX; j++) {
        ...
        for (i = 0; i < nroots; i++) {
          if (syn[i] == 0)
            ...
            syn[i] =
                data ^
                gf_alpha_to[mod_fieldmax(gf_index_of[syn[i]] +
      ...
      /* If it is already correct by coincidence, we have nothing to do any
       * further */
      ...
      if (erasure_count > 0) {
        lambda[1] = gf_alpha_to[mod_fieldmax(
            RS_PRIM_ELEM * (GF_FIELDMAX - 1 - erasure_list[0]))];
      ...
      el = erasure_count;
      while (++r <=
             nroots) /* Compute discrepancy at the r-th step in poly-form */
      {
        discr_r = 0;
        for (i = 0; i < r; i++)
          if ((lambda[i] != 0) && (syn[r - i - 1] != GF_ALPHA0))
            discr_r ^= gf_alpha_to[mod_fieldmax(gf_index_of[lambda[i]] +
                                                syn[r - i - 1])];

        discr_r = gf_index_of[discr_r]; /* Index form */

        if (discr_r == GF_ALPHA0) { /* B(x) = x*B(x) */
          memmove(b + 1, b, nroots * sizeof(b[0]));
          b[0] = GF_ALPHA0;
        } else { /* T(x) = lambda(x) - discr_r*x*b(x) */
          t[0] = lambda[0];
          for (i = 0; i < nroots; i++) {
            if (b[i] != GF_ALPHA0)
              t[i + 1] =
                  lambda[i + 1] ^ gf_alpha_to[mod_fieldmax(discr_r + b[i])];
            else
              t[i + 1] = lambda[i + 1];
          }
          ...
          if (2 * el <= r + erasure_count - 1) {
            ...
            for (i = 0; i <= nroots; i++)
              b[i] = (lambda[i] == 0) ? GF_ALPHA0
                                      : mod_fieldmax(gf_index_of[lambda[i]] -
                                                     discr_r + GF_FIELDMAX);
      ...
      for (i = 1, k = RS_PRIMTH_ROOT - 1; i <= GF_FIELDMAX;
           i++, k = mod_fieldmax(k + RS_PRIMTH_ROOT)) {
      ...
      if (deg_lambda != count) {
        PrintLog("Decoder problem (%d != %d) for %d sectors: ", deg_lambda,
                 count, erasure_count);

        for (i = 0; i < erasure_count; i++) {
          gint64 loc = erasure_list[i];
          PrintLog("%lld ", loc < ndata
                                ? block_idx[loc]
                                : fc->eccIdx[loc - ndata][cache_sector]);
        }
        PrintLog("\n");
      ...
      for (j = count - 1; j >= 0; j--) {
        ...
        num2 = gf_alpha_to[mod_fieldmax(root[j] * (RS_FIRST_ROOT - 1) +
                                        GF_FIELDMAX)];
        den = 0;
        ...
        if (num1 != 0) {
          ...
          if (erasure_map[location] != 1) /* erasure came from CRC error */
          {
            ...
            int new =
                old ^
                gf_alpha_to[mod_fieldmax(gf_index_of[num1] + gf_index_of[num2] +
                                         GF_FIELDMAX - gf_index_of[den])];
            char *msg;
            gint64 sector;

            if (erasure_map[location] == 3) /* erasure came from CRC error */
            {
              msg = _("-> CRC-predicted error in sector %lld at byte %4d "
                      "(value %02x '%c', expected %02x '%c')\n");
            } else {
              msg = _("-> Non-predicted error in sector %lld at byte %4d "
                      "(value %02x '%c', expected %02x '%c')\n");
            ...
            if (location < ndata)
              ...
              sector = fc->eccIdx[location - ndata][cache_sector];

            PrintCLI(msg, sector, bi, old, isprint(old) ? old : '.', new,
                     isprint(new) ? new : '.');
          }

          fc->imgBlock[location][offset] ^=
              gf_alpha_to[mod_fieldmax(gf_index_of[num1] + gf_index_of[num2] +
                                       GF_FIELDMAX - gf_index_of[den])];
        }
      }
    ...
    if (erasure_count) {
      ...
      for (i = 0; i < 255; i++) {
        ...
        if (i < ndata) {
          ...
          ecc_corr++;
          sec = fc->eccIdx[i - ndata][cache_sector];
        }
        corrected++;
        ...
        if (sec != lay->dataSectors - 1)
          length = 2048;
        else
          length = eh->inLast; /* non-image file may be clipped */

        /* Write back into the image */

        if (lay->target == ECC_IMAGE || sec < lay->dataSectors) {
          if (!LargeSeek(image->file, (gint64)(2048 * sec)))
            Stop(_("Failed seeking to sector %lld in image [%s]: %s"), sec,
                 "FW", strerror(errno));
  ...
  if (corrected > 0)
    PrintLog(_("Repaired sectors: %lld (%lld data, %lld ecc)\n"), corrected,
             data_corr, ecc_corr);
  if (uncorrected > 0) {
    PrintLog(_("Unrepaired sectors: %lld\n"), uncorrected);
    if (Closure->guiMode)
      SwitchAndSetFootline(
          wl->fixNotebook, 1, wl->fixFootline,
  ...
  if (Closure->guiMode && t)
    SwitchAndSetFootline(wl->fixNotebook, 1, wl->fixFootline, "%s %s",
                         _("Repair results:"), t);
  ...
  Verbose("%lld of %lld ecc blocks damaged (%lld / %lld sectors)\n",
          damaged_eccblocks, 2048 * lay->sectorsPerLayer, damaged_eccsecs,
          lay->sectorsPerLayer);
  if (data_count != (ndata - 1) * lay->sectorsPerLayer)
    g_printf("ONLY %lld of %lld data sectors processed\n",
             (long long int)data_count,
             (long long int)(ndata - 1) * lay->sectorsPerLayer);
  else
    Verbose("all data sectors processed\n");

  if (crc_count != lay->sectorsPerLayer)
    g_printf("%lld of %lld crc sectors processed\n", (long long int)crc_count,
             (long long int)lay->sectorsPerLayer);
  else
    Verbose("all  crc sectors processed\n");

  if (ecc_count != nroots * lay->sectorsPerLayer)
    g_printf("%lld of %lld ecc sectors processed\n", (long long int)ecc_count,
             (long long int)nroots * lay->sectorsPerLayer);
  else
    Verbose("all  ecc sectors processed\n");
