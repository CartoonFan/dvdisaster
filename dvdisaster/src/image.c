...
void ExamineECC(Image *image) {
  ...
  for (i = 0; i < Closure->methodList->len; i++) {
    ...
    if (method->recognizeEccImage && method->recognizeEccImage(image)) {
      Verbose("...augmented image found\n");
      image->eccMethod = method;
      image->expectedSectors = method->expectedImageSize(image->eccHeader);
      return;
    }
  }
...
Image *OpenEccFileForImage(Image *image, char *filename, int flags,
                           mode_t mode) {
  int new_image = 0;
  int i;
  ...
  for (i = 0; i < Closure->methodList->len; i++) {
    ...
    if (method->recognizeEccFile) {
      image->eccFileState =
          method->recognizeEccFile(image->eccFile, &image->eccFileHeader);
      if (image->eccFileState == ECCFILE_PRESENT) {
        image->eccFileMethod = method;
        image->expectedSectors =
            method->expectedImageSize(image->eccFileHeader);
...
void ReportImageEccInconsistencies(Image *image) {
...
int ImageReadSectors(Image *image, void *buf, guint64 first, int n) {
  ...
  case IMAGE_FILE:
    ...
    if (LargeRead(image->file, buf, n * 2048) == n * 2048) {
      if (CheckForMissingSectors(buf, first, NULL, 0, n, &first_defect) ==
          SECTOR_PRESENT)
        return n;
      else
        return 0;
