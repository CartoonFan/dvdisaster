...
EccHeader *ValidHeader(unsigned char *buf, gint64 hdr_pos) {
  ...
  if ((CheckForMissingSector(buf, hdr_pos, NULL, 0) != SECTOR_PRESENT) ||
      (CheckForMissingSector(buf + 2048, hdr_pos + 1, NULL, 0) !=
       SECTOR_PRESENT))
    return NULL;
...
EccHeader *FindRS03HeaderInImage(Image *image) {
  ...
  ii = image->isoInfo;
  if (!ii)
    Verbose(" . NO ISO structures found!\n");
...
int RS03RecognizeImage(Image *image) {
  ...
  if (Closure->debugMode && Closure->mediumSize)
    ...
    if (file_size < CDR_SIZE)
      layer_size = CDR_SIZE / GF_FIELDMAX;
    else if (file_size < DVD_SL_SIZE)
      layer_size = DVD_SL_SIZE / GF_FIELDMAX;
    else if (file_size < DVD_DL_SIZE)
      layer_size = DVD_DL_SIZE / GF_FIELDMAX;
    else if (file_size < BD_SL_SIZE)
      layer_size = BD_SL_SIZE / GF_FIELDMAX;
    else
      layer_size = BD_DL_SIZE / GF_FIELDMAX;
  }
  ...
  for (ecc_block = 0; ecc_block < layer_size; ecc_block++) {
    ...
    for (i = 0; i < 255; i++) {
      ...
      if (n != 2048)
        Stop(_("Failed reading sector %lld in image: %s"), sector,
             strerror(errno));
    }
