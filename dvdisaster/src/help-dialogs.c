...
static gint help_cb(GtkWidget *widget, GdkEvent *event, gpointer data) {
  ...
  case GDK_BUTTON_PRESS:
    if (!lwoh->inside)
      return FALSE; /* Bug in Gtk for Windows? */
...
LabelWithOnlineHelp *CreateLabelWithOnlineHelp(char *title, char *ascii_text) {
  ...
  g_signal_connect(G_OBJECT(ebox), "button_press_event", G_CALLBACK(help_cb),
                   (gpointer)lwoh);
  g_signal_connect(G_OBJECT(ebox), "enter_notify_event", G_CALLBACK(help_cb),
                   (gpointer)lwoh);
  g_signal_connect(G_OBJECT(ebox), "leave_notify_event", G_CALLBACK(help_cb),
                   (gpointer)lwoh);
...
LabelWithOnlineHelp *CloneLabelWithOnlineHelp(LabelWithOnlineHelp *orig,
                                              char *ascii_text) {
  LabelWithOnlineHelp *lwoh;
  GtkWidget *ebox = gtk_event_box_new();
...
void SetOnlineHelpLinkText(LabelWithOnlineHelp *lwoh, char *ascii_text) {
  ...
  g_sprintf(text, "<span underline=\"single\" color=\"blue\">%s</span>",
            ascii_text);
...
void AddHelpParagraph(LabelWithOnlineHelp *lwoh, char *format, ...) {
  ...
  g_signal_connect(label, "expose_event", G_CALLBACK(wrapper_fix_cb),
                   get_new_int(lwoh));
}
...
void AddHelpWidget(LabelWithOnlineHelp *lwoh, GtkWidget *widget) {
  ...
  gtk_box_pack_start(GTK_BOX(lwoh->vbox), gtk_hseparator_new(), FALSE, FALSE,
                     10);
}
...
static gboolean log_jump_func(gpointer data) {
  ...
  a = gtk_scrolled_window_get_vadjustment(
      GTK_SCROLLED_WINDOW(Closure->logScroll));
  gtk_adjustment_set_value(a, a->upper - a->page_size);
  gtk_scrolled_window_set_vadjustment(GTK_SCROLLED_WINDOW(Closure->logScroll),
                                      a);
  g_mutex_unlock(Closure->logLock);
...
static gboolean log_idle_func(gpointer data) {
  ...
  if (Closure->logBuffer)
    gtk_text_buffer_set_text(Closure->logBuffer, Closure->logString->str,
                             Closure->logString->len);
  g_mutex_unlock(Closure->logLock);
...
char *find_file(char *file, size_t *size, char *lang) {
  ...
  if (Closure->binDir) {
    if (lang)
      ...
      path = g_strdup_printf("%s/%s", Closure->binDir, file);
  ...
  if (Closure->docDir) {
    if (lang)
      ...
      path = g_strdup_printf("%s/%s", Closure->docDir, file);
...
GtkWidget *ShowTextfile(char *title, char *explanation, char *file,
  ...
  dialog = gtk_dialog_new_with_buttons(
      utf, Closure->window, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_STOCK_CLOSE,
  ...
  if (scroll_out)
    *scroll_out = GTK_SCROLLED_WINDOW(scroll_win);
...
static gint about_cb(GtkWidget *widget, GdkEvent *event, gpointer data) {
  ...
  case GDK_BUTTON_PRESS:
    ...
    if (!strcmp(label, "GPL"))
      ShowGPL();
    else if (!strcmp(label, "MODIFYING"))
      show_modifying();
    else
...
void AboutLink(GtkWidget *parent, char *label, char *action) {
  ...
  gtk_widget_set_events(ebox, GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK |
                                  GDK_BUTTON_PRESS_MASK);
  g_signal_connect(G_OBJECT(ebox), "button_press_event", G_CALLBACK(about_cb),
                   (gpointer)action);
  g_signal_connect(G_OBJECT(ebox), "enter_notify_event", G_CALLBACK(about_cb),
                   (gpointer)label_copy);
  g_signal_connect(G_OBJECT(ebox), "leave_notify_event", G_CALLBACK(about_cb),
                   (gpointer)label_copy);
...
void AboutDialog() {
  ...
  about = gtk_dialog_new_with_buttons(
      _utf("windowtitle|About dvdisaster"), Closure->window,
  ...
  g_signal_connect_swapped(about, "response", G_CALLBACK(gtk_widget_destroy),
                           about);
  ...
  text = g_strdup_printf(
      "<span weight=\"bold\" size=\"xx-large\">dvdisaster</span><i> "
      "Version %s</i>",
  ...
  AboutText(vbox,
            _("dvdisaster provides a margin of safety against data loss\n"
  ...
  AboutTextWithLink(
      vbox,
      _("This software comes with  <b>absolutely no warranty</b>.\n"
