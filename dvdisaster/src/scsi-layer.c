...
int InquireDevice(DeviceHandle *dh, int probe_only) {
  ...
  if (SendPacket(dh, cmd, 6, ab->buf, 36, &sense, DATA_READ) < 0) {
    ...
    if (probe_only)
      return 0x1f; /* don't care about failure, just return invalid device */
    ...
    Stop(_("Can open %s, but INQUIRY fails.\n"
           "Chances are that you're using ide-scsi emulation for an ATAPI "
           "drive,\n"
           "and try to access it via /dev/cdrom or /dev/hd?.\n"
           "Either use /dev/scd? or /dev/sr? instead, or disable ide-scsi "
           "emulation.\n"),
         dh->device);
    return 0;
#else
    ...
    for (i = 0, j = 8; j < 36; j++) {
      if (j == 32) {
        vidx = i;
        if (i > 0 &&
            !isspace((int)ibuf[i - 1])) /* separate the version string */
          ibuf[i++] = ' ';
      }
    ...
    if (ab->buf[0] != 0x05 && !probe_only) {
      PrintCLI("\n");
      if (ab->buf[0])
        Stop(_("Device %s (%s) is not a CDROM drive."), dh->device, ibuf);
      else
        Stop(_("Device %s (%s) is a hard disk."), dh->device, ibuf);
    }
  }
...
static int try_fallback_type_check(DeviceHandle *dh) {
  ...
  if (SendPacket(dh, cmd, 12, ab->buf, MIN_TRANSFER_LEN, sense, DATA_READ) <
      0) {
...
static int get_configuration(DeviceHandle *dh) {
  ...
  switch (dh->profile) {
  /* Note: subType handling is different for CD for
                            historical reasons */
  case 0x08:
    dh->profileDescr = "CD-ROM";
    dh->shortProfile = "CD-ROM";
    dh->mainType = CD;
    break;
  case 0x09:
    dh->profileDescr = "CD-R";
    dh->shortProfile = "CD-R";
    dh->mainType = CD;
    break;
  case 0x0a:
    dh->profileDescr = "CD-RW";
  ...
  case 0x10:
    dh->profileDescr = "DVD-ROM";
    dh->shortProfile = "DVD-ROM";
    dh->mainType = DVD;
    dh->subType = DVD;
    break;
  case 0x11:
    dh->profileDescr = "DVD-R Sequential recording";
  ...
  case 0x12:
    ...
    break;
  case 0x13:
    dh->profileDescr = "DVD-RW Restricted overwrite";
    ...
    break;
  case 0x14:
    dh->profileDescr = "DVD-RW Sequential overwrite";
    dh->shortProfile = "DVD-RW";
    dh->mainType = DVD;
    dh->isDash = TRUE;
    dh->rewriteable = TRUE;
    ...
    break;
  case 0x15:
    dh->profileDescr = "DVD-R DL Sequential recording";
    ...
    break;
  case 0x16:
    dh->profileDescr = "DVD-R DL Layer jump recording";
    dh->shortProfile = "DVD-R DL";
    dh->mainType = DVD;
    dh->isDash = TRUE;
    dh->subType = DVD_DASH_R_DL;
  ...
  case 0x40:
    dh->profileDescr = "BD-ROM";
    dh->shortProfile = "BD-ROM";
    dh->mainType = BD;
    dh->subType = BD;
    break;
  case 0x41:
    dh->profileDescr = "BD-R Sequential recording mode";
    dh->shortProfile = "BD-R";
    dh->mainType = BD;
    dh->subType = BD_R;
    break;
  case 0x42:
    dh->profileDescr = "BD-R Random recording mode";
    dh->shortProfile = "BD-R";
    dh->mainType = BD;
    dh->subType = BD_R;
    break;
  case 0x43:
    dh->profileDescr = "BD-RE";
  ...
  default:
    dh->profileDescr = "Unknown profile";
...
static int query_incomplete(DeviceHandle *dh, int probe_only) {
  ...
  if (!strncmp((char *)(ab->buf + 1), "CD001", 5)) {
    ...
    Verbose("-> not an ISO root sector\n");
...
static int query_cd(DeviceHandle *dh, int probe_only) {
  ...
  if (length >
      1024) /* don't let the drive hack us using a buffer overflow ;-) */
  {
    if (Closure->verbose)
      HexDump(buf, 1024, 16);
  ...
  if (control & 4)
    switch (buf[13]) {
    case 0x00:
      dh->typeDescr = g_strdup_printf("%s mode 1", dh->profileDescr);
      dh->subType = DATA1;
      break;
    case 0x10:
      dh->typeDescr = g_strdup_printf("%s/CD-I", dh->profileDescr);
      dh->subType = UNSUPPORTED;
      break;
    case 0x20:
      dh->typeDescr = g_strdup_printf("%s XA", dh->profileDescr);
      dh->subType = XA21;
      break;
    default:
      dh->typeDescr = g_strdup_printf("%s ??", dh->profileDescr);
      dh->subType = UNSUPPORTED;
      break;
    }
  else {
...
static int query_dvd(DeviceHandle *dh, int probe_only) {
  ...
  if (length >
      4096) /* don't let the drive hack us using a buffer overflow ;-) */
  {
    FreeAlignedBuffer(ab);
    if (!probe_only)
      Stop(_("Could not query dvd physical structure - implausible packet "
             "length %d\n"),
           length);
    return FALSE;
  }
  ...
  if (dh->layers == 1) {
    ...
    if (dh->userAreaSize < 0 || dh->userAreaSize > MAX_DVD_SL_SIZE) {
      LogWarning(_("READ DVD STRUCTURE: implausible medium size, "
                   "%lld-%lld=%lld sectors\n"),
  ...
  if (SendPacket(dh, cmd, 12, buf, MIN_TRANSFER_LEN, sense, DATA_READ) == 0) {
    ...
    Verbose("#DVD: lead-in could not be queried\n");
  ...
  {
  case 1:
    dh->bookDescr = "DVD-RAM";
    dh->rewriteable = TRUE;
    dh->subType = DVD_RAM;
    break;
  case 2:
    dh->bookDescr = dh->layers == 1 ? "DVD-R" : "DVD-R DL";
  ...
  case 3:
    ...
    break;
  case 4:
    dh->bookDescr = "HD DVD-ROM";
    dh->subType = UNSUPPORTED;
    break;
  case 5:
    dh->bookDescr = "HD DVD-RAM";
    dh->subType = UNSUPPORTED;
    break;
  case 6:
    dh->bookDescr = "HD DVD-R";
    dh->subType = UNSUPPORTED;
    break;
  case 9:
    dh->bookDescr = "DVD+RW";
  ...
  case 10:
    ...
    break;
  case 13:
    dh->bookDescr = "DVD+RW DL";
    dh->rewriteable = TRUE;
    dh->subType = DVD_PLUS_RW_DL;
    break;
  case 14:
    dh->bookDescr = "DVD+R DL";
    dh->subType = DVD_PLUS_R_DL;
    break;
  ...
  case 0: /* tricky case: real or faked DVD-ROM? */
  {
    ...
    if (layer_type &
        0x06) /* strange thing: (re-)writeable but neither plus nor dash */
    {
  ...
  default:
    dh->typeDescr =
        g_strdup_printf("DVD book type 0x%02x", (phy_info4 >> 4) & 0x0f);
    dh->subType = UNSUPPORTED;
    break;
  }
...
static int query_type(DeviceHandle *dh, int probe_only) {
  ...
  if (SendPacket(dh, cmd, 10, buf, MIN_TRANSFER_LEN, sense, DATA_READ) == 0) {
    ...
    if (SendPacket(dh, cmd, 10, buf, length, sense, DATA_READ) == 0) {
      ...
      Verbose("# status is %x%s disc type %x\n", dh->discStatus,
              dh->incomplete ? " (incomplete!)," : ",", buf[8]);
    } else
      Verbose("# READ DISC INFORMATION failed\n");
  } else
    Verbose("# Getting size of READ DISC INFORMATION failed\n");
  FreeAlignedBuffer(ab);
  ...
  case CD:
    ...
    return dh->incomplete ? query_incomplete(dh, probe_only)
                          : query_cd(dh, probe_only);
...
static int query_blank(DeviceHandle *dh) {
  ...
  if (dh->mainType == DVD) {
    ...
    case DVD_PLUS_R_DL:
      ...
      if (SendPacket(dh, cmd, 10, buf, MIN_TRANSFER_LEN, sense, DATA_READ) <
          0) {
        FreeAlignedBuffer(ab);
        return FALSE;
      }
    ...
    case DVD_RAM: {
      ...
      for (i = 0, idx = 12; i < n_lists; i++, idx += 8) {
        ...
        size = (gint64)(buf[idx] << 24 | buf[idx + 1] << 16 |
...
static void try_c2_scan(DeviceHandle *dh) {
  ...
  ret = SendPacket(dh, cdb, 12, rb->workBuf->buf, CD_RAW_C2_SECTOR_SIZE, sense,
                   DATA_READ);
  ...
  if (!ret ||
      sense->sense_key !=
          5) /* good status or error != illegal request means C2 works */
  {
...
static int read_mode_page(DeviceHandle *dh, AlignedBuffer *ab,
                          int *parameter_list_length, unsigned char *read_mode,
                          unsigned char *read_retries) {
  Sense sense;
...
static int set_mode_page(DeviceHandle *dh, AlignedBuffer *ab,
                         int parameter_list_length, unsigned char read_mode,
                         unsigned char read_retries) {
  Sense sense;
  unsigned char cdb[16];
  ...
  ret = SendPacket(dh, cdb, 10, ab->buf, parameter_list_length, &sense,
                   DATA_WRITE);
...
void SetRawMode(DeviceHandle *dh, int action) {
  ...
  if (action == MODE_PAGE_UNSET) {
    ...
    if (!set_mode_page(dh, ab, pll, dh->previousReadMode,
                       dh->previousRetries)) {
      FreeAlignedBuffer(ab);
      return;
    }
  ...
  if (!read_mode_page(dh, ab, &pll, &dh->previousReadMode,
                      &dh->previousRetries)) {
    FreeAlignedBuffer(ab);
    return;
  }
  ...
  if (Closure->internalAttempts >= 0)
    ...
    new_read_retries = dh->previousRetries;
...
static int query_copyright(DeviceHandle *dh) {
  ...
  {
    FreeAlignedBuffer(ab);
    Stop(_("Could not query dvd copyright info - implausible packet length "
           "%d\n"),
         length);
    return TRUE;
  }
...
static int check_sector(DeviceHandle *dh, GString *msg_out, guint64 sector,
                        int n_sectors) {
  ...
  } else /* Read error */
  {
    ...
    if (dh->sense.sense_key == 0x05 /* illegal request */
        || dh->sense.sense_key ==
               0x00) /* or None; happens at least on Pioneer drives */
    {
      if (dh->sense.asc == 0x63 && dh->sense.ascq == 0x00)
        result = 1; /* end of user area */
      else
        result = 2; /* other illegal request, */
                    /* usually 0x21 Logical block address out of range*/
    } else
      result = 3; /* other read error */
  }

  if (n_sectors == 1)
    g_string_append_printf(msg_out, _("Sector %lld: %s\n"), sector, msg);
  else
    g_string_append_printf(msg_out, _("Sectors %lld-%lld: %s\n"), sector,
                           sector + n_sectors - 1, msg);
...
static unsigned int query_size(Image *image) {
  ...
  dh->readCapacity =
      (gint64)(buf[0] << 24 | buf[1] << 16 | buf[2] << 8 | buf[3]);
  FreeAlignedBuffer(ab);
  ...
  if (dh->mainType == DVD && dh->layers == 1 &&
      dh->readCapacity > MAX_DVD_SL_SIZE)
    implausible = TRUE;

  if (dh->mainType == DVD && dh->layers == 2 &&
      dh->readCapacity > MAX_DVD_DL_SIZE)
    implausible = TRUE;
  ...
  if (Closure->examineRS02) {
    if (image->expectedSectors > 0) {
      ...
      Verbose("Medium size could NOT be determined from ECC header.\n");
  }
  ...
  if (image->isoInfo) {
    if (Closure->ignoreIsoSize) {
      Verbose("Note: NOT examining ISO/UDF filesystem as requested by user "
              "option!\n");
    } else {
    ...
    Verbose("Medium size could NOT be determined from ISO/UDF filesystem.\n");
  ...
  if (dh->mainType == CD) {
    Verbose("CD medium - using size from READ CAPACITY: %lld sectors\n",
            dh->readCapacity + 1);
    return dh->readCapacity +
           1; /* size is the number of the last sector, starting with 0 */
  }
  ...
  if (dh->readCapacity ==
      dh->userAreaSize) /* If they are equal just return one */
  {
  ...
  } else /* Tricky case. Try some heuristics. */
  {
    ...
    if (!decision) {
      ...
      decision_msg =
          _("FAILED to determine image size.\n"
            "Using smaller value as this is right on >90%% of all drives,\n"
            "but CONTINUE AT YOUR OWN RISK (the image may be "
            "incomplete/unusable)");
    }
    ...
    default:
      Stop(_("Failed to determine image size.\n"
...
int TestUnitReady(DeviceHandle *dh) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    if (dh->sense.sense_key == 2) /* Not Ready */
      if ((dh->sense.asc == 0x04 &&
           dh->sense.ascq == 0x01) /* but in process of becoming so */
    ...
    if (continue_waiting) {
      PrintCLIorLabel(Closure->status, _("Waiting 10 seconds for drive: %d\n"),
                      9 - i);
...
static int read_dvd_sector(DeviceHandle *dh, unsigned char *buf, int lba,
                           int nsectors) {
  Sense *sense = &dh->sense;
  unsigned char cmd[MAX_CDB_SIZE];
  int ret;
...
static int read_cd_sector(DeviceHandle *dh, unsigned char *buf, int lba,
                          int nsectors) {
  Sense *sense = &dh->sense;
  unsigned char cmd[MAX_CDB_SIZE];
  int ret;
...
static int read_raw_cd_sector(DeviceHandle *dh, unsigned char *outbuf, int lba,
                              int nsectors) {
  Sense *sense = &dh->sense;
  unsigned char cdb[MAX_CDB_SIZE];
  unsigned char c2bit;
  ...
  case DATA1: /* data mode 1 */
    cdb[1] = 2 << 2;
    cdb[9] =
        0xb8 | c2bit; /* we want Sync + Header + User data + EDC/ECC + C2 */
    offset = 16;
    break;
  ...
  ret = SendPacket(dh, cdb, 12, rb->workBuf->buf, nsectors * rb->sampleSize,
                   sense, DATA_READ);
  RememberSense(sense->sense_key, sense->asc, sense->ascq);
  ...
  if (!ret && nsectors == 1 &&
      !ValidateRawSector(rb, rb->workBuf->buf, "verify incoming")) {
    ret = -1;
  }
  ...
  for (i = 0, s = 0; i < nsectors; i++, s += rb->sampleSize)
    if (CheckForMissingSector(rb->workBuf->buf + s, lba + i, NULL, 0) !=
        SECTOR_PRESENT) {
...
int ReadSectors(DeviceHandle *dh, unsigned char *buf, gint64 s, int nsectors) {
  ...
  for (retry = 1; retry <= recommended_attempts; retry++) {
    ...
    if (status) /* current try was unsucessful */
    {
      ...
      //	 if(dh->canReadDefective && nsectors > 1 && Closure->sectorSkip
      //== 0)
      if (nsectors > 1 && Closure->sectorSkip == 0) {
        PrintCLIorLabel(Closure->status, _("Sectors %lld - %lld: %s\n"), s,
                        s + nsectors - 1, GetLastSenseString(FALSE));
        return status;
      }
      ...
      if (last_key == 3 && last_asc == 255 && last_ascq == 2 && dh->rawBuffer) {
        ...
        PrintCLIorLabel(Closure->status,
                        ...
                        retry, GetLastSenseString(FALSE),
                        MSFtoLBA(frame[12], frame[13], frame[14]));
      } else
        PrintCLIorLabel(Closure->status, _("Sector %lld, try %d: %s\n"), s,
                        retry, GetLastSenseString(FALSE));
    ...
    } else                        /* good return status */
    {
      if (recommended_attempts > 1 && retry > 1)
        PrintCLIorLabel(Closure->status, _("Sector %lld, try %d: success\n"), s,
                        retry);
  ...
  if (dh->canReadDefective && status && Closure->defectiveDump) {
    if (SaveDefectiveSector(dh->rawBuffer,
                            dh->canC2Scan)) /* any new sectors? */
    {
...
int ReadSectorsFast(DeviceHandle *dh, unsigned char *buf, gint64 s,
                    int nsectors) {
  int status = -1;
...
Image *OpenImageFromDevice(char *device) {
  ...
  if (!TestUnitReady(dh)) {
        ...
        && dh->sense.asc == 0x3a) /* Medium not present */
    {
      ...
      Stop(_("Device %s does not become ready:\n%s\n\n"), device,
           GetSenseString(dh->sense.sense_key, dh->sense.asc, dh->sense.ascq,
                          FALSE));
      return NULL;
    }
  }
  ...
  if (dh->sessions > 1) {
    ...
    CloseDevice(dh);
    Stop(_("This software does not support multisession (%d sessions) media."),
         sessions);
    return NULL;
  }
  ...
  case CD:
    if (Closure->readRaw) {
      ...
      dh->rawBuffer->validFP = GetImageFingerprint(
          image, dh->rawBuffer->mediumFP, FINGERPRINT_SECTOR);
  ...
  case CD: {
    ...
    if (!image->isoInfo) // || dh->rs02Size > 0)
      tmp = g_strdup_printf(
          _("Medium: %s, %lld sectors%s"), dh->typeDescr, dh->sectors,
          image->expectedSectors ? ", Ecc" : ""); // fixme: validate
    else
      tmp = g_strdup_printf(
          _("Medium \"%s\": %s, %lld sectors%s created %s"),
          image->isoInfo->volumeLabel, dh->typeDescr, dh->sectors,
          image->expectedSectors ? ", Ecc," : ",", // fixme: validate
          image->isoInfo->creationDate);

    if (dh->manuID[0] && dh->manuID[0] != '-')
      dh->mediumDescr =
          g_strdup_printf("%s, %s %s.", tmp, _("Manuf.-ID:"), dh->manuID);
    else
      dh->mediumDescr = g_strdup_printf("%s.", tmp);
    g_free(tmp);
    PrintLog("%s\n\n", dh->mediumDescr);
    break;
  }
...
int SendReadCDB(char *device, unsigned char *buf, unsigned char *cdb,
                int cdb_len, int alloc_len) {
  DeviceHandle *dh = NULL;
  Sense sense;
  int i, status;
