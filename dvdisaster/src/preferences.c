...
#if 0
#define gtk_widget_set_sensitive(widget, state)                                \
  {                                                                            \
    printf("%s, line %d - gtk_widget_set_sensitive()\n", __FILE__, __LINE__);  \
...
void FreePreferences(void *context) {
  ...
  if (pc->formatLinear)
    g_free(pc->formatLinear);
  if (pc->formatAdaptive)
    g_free(pc->formatAdaptive);

  if (pc->jumpScaleInfoA)
    g_free(pc->jumpScaleInfoA);
  if (pc->jumpScaleInfoB)
    g_free(pc->jumpScaleInfoB);
  if (pc->minAttemptsScaleInfoA->format)
    g_free(pc->minAttemptsScaleInfoA->format);
  if (pc->minAttemptsScaleInfoB->format)
    g_free(pc->minAttemptsScaleInfoB->format);
  if (pc->minAttemptsScaleInfoA)
    g_free(pc->minAttemptsScaleInfoA);
  if (pc->minAttemptsScaleInfoB)
    g_free(pc->minAttemptsScaleInfoB);
  if (pc->maxAttemptsScaleInfoA->format)
    g_free(pc->maxAttemptsScaleInfoA->format);
  if (pc->maxAttemptsScaleInfoB->format)
    g_free(pc->maxAttemptsScaleInfoB->format);
  if (pc->maxAttemptsScaleInfoA)
    g_free(pc->maxAttemptsScaleInfoA);
  if (pc->maxAttemptsScaleInfoB)
    g_free(pc->maxAttemptsScaleInfoB);
  ...
  if (pc->redTextB)
    g_free(pc->redTextB);
  if (pc->greenTextA)
    g_free(pc->greenTextA);
  if (pc->greenTextB)
    g_free(pc->greenTextB);
  if (pc->barColorA)
    g_free(pc->barColorA);
  if (pc->barColorB)
    g_free(pc->barColorB);
  ...
  if (pc->logColorB)
    g_free(pc->logColorB);
  if (pc->curveColorA)
    g_free(pc->curveColorA);
  if (pc->curveColorB)
    g_free(pc->curveColorB);
...
void HidePreferences(void) {
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(pc->rangeToggleA)) ||
    ...
    Closure->readStart =
        gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(pc->rangeSpin1A));
    Closure->readEnd =
        gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(pc->rangeSpin2A));
  } else
    Closure->readStart = Closure->readEnd = 0;

  /* Get fill byte and recalculate the dead sector marker */

  if (pc->byteCheckA &&
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(pc->byteCheckA))) {
    ...
    } else if (Closure->fillUnreadable != v1) {
  ...
  method_index =
      gtk_notebook_get_current_page(GTK_NOTEBOOK(pc->methodNotebook));
...
static void close_cb(GtkWidget *widget, gpointer data) { HidePreferences(); }
...
void UpdatePrefsExhaustiveSearch(void) {
  ...
  if (Closure->prefsContext) {
    activate_toggle_button(GTK_TOGGLE_BUTTON(pc->recogRS02A),
                           Closure->examineRS02);
    activate_toggle_button(GTK_TOGGLE_BUTTON(pc->recogRS02B),
                           Closure->examineRS02);
    activate_toggle_button(GTK_TOGGLE_BUTTON(pc->recogRS03A),
                           Closure->examineRS03);
    activate_toggle_button(GTK_TOGGLE_BUTTON(pc->recogRS03B),
                           Closure->examineRS03);
  }
}
...
void UpdatePrefsConfirmDeletion(void) {
  ...
  if (Closure->prefsContext) {
    activate_toggle_button(GTK_TOGGLE_BUTTON(pc->confirmDeletionA),
                           Closure->confirmDeletion);
    activate_toggle_button(GTK_TOGGLE_BUTTON(pc->confirmDeletionB),
                           Closure->confirmDeletion);
  }
}
...
static void toggle_cb(GtkWidget *widget, gpointer data) {
  ...
  case TOGGLE_RANGE: {
    ...
    if (state) {
      gtk_spin_button_set_range(GTK_SPIN_BUTTON(pc->rangeSpin2A), 0.0,
                                image_size);
      gtk_spin_button_set_range(GTK_SPIN_BUTTON(pc->rangeSpin2B), 0.0,
                                image_size);
  ...
  case SPIN_INTERNAL_ATTEMPTS:
    ...
    if (widget == pc->internalAttemptsA) {
      if (pc->internalAttemptsB)
        gtk_spin_button_set_value(GTK_SPIN_BUTTON(pc->internalAttemptsB),
                                  value);
    }
    if (widget == pc->internalAttemptsB) {
      if (pc->internalAttemptsA)
        gtk_spin_button_set_value(GTK_SPIN_BUTTON(pc->internalAttemptsA),
                                  value);
    }
    break;
...
static void read_range_cb(GtkWidget *widget, gpointer data) {
  ...
  if (pc->rangeSpin1A == widget || pc->rangeSpin2A == widget) {
    ...
    int from =
        gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(pc->rangeSpin1B));
...
static gboolean color_delete_cb(GtkWidget *widget, GdkEvent *event,
                                gpointer data) {
...
static void update_color_buttons(color_button_info *a, color_button_info *b,
                                 int redraw) {
  GdkRectangle rect;
  GdkWindow *window;
  gint ignore;
  ...
  gdk_draw_rectangle(a->pixmap, a->gc, TRUE, 0, 0, COLOR_BUTTON_WIDTH,
                     COLOR_BUTTON_HEIGHT);
  gdk_draw_rectangle(b->pixmap, b->gc, TRUE, 0, 0, COLOR_BUTTON_WIDTH,
                     COLOR_BUTTON_HEIGHT);
...
static void color_ok_cb(GtkWidget *widget, gpointer data) {
  ...
  gtk_color_selection_get_current_color(
      GTK_COLOR_SELECTION(((GtkColorSelectionDialog *)cbi->dialog)->colorsel),
      cbi->color);
...
static color_button_info *create_color_button(GdkColor *color, int user_data) {
  ...
  cbi->pixmap = gdk_pixmap_new(gdk_get_default_root_window(),
                               COLOR_BUTTON_WIDTH, COLOR_BUTTON_HEIGHT, -1);
  ...
  gdk_draw_rectangle(cbi->pixmap, cbi->gc, TRUE, 0, 0, COLOR_BUTTON_WIDTH,
                     COLOR_BUTTON_HEIGHT);
  cbi->color = color;
  cbi->userData = user_data;
  g_signal_connect(G_OBJECT(cbi->button), "clicked",
                   G_CALLBACK(color_choose_cb), cbi);
...
static int jump_values[] = {0,   16,  32,   64,   128,  256,   384,
                            512, 768, 1024, 2048, 4096, 10240, 20480};
#define JUMP_VALUE_LENGTH 14

static int min_attempts_values[] = {1,  2,  3,  4,  5,  7,  9,
                                    11, 15, 20, 25, 30, 40, 50};
static int max_attempts_values[] = {1,  2,  3,  4,  5,  7,  9,  11, 15, 20,
...
static gchar *non_linear_format_cb(GtkScale *scale, gdouble value,
                                   gpointer data) {
  char *text;
...
static GtkWidget *non_linear_scale(GtkWidget **hbox_out, non_linear_info *nli,
  ...
  g_signal_connect(scale, "format-value", G_CALLBACK(non_linear_format_cb),
                   nli);
  ...
  text =
      g_strdup_printf(nli->format, nli->values[index > 0 ? index - 1 : index]);
  utf = g_locale_to_utf8(text, -1, NULL, NULL, NULL);
  nli->label = label;
  ...
  if (nli->lwoh) {
    ...
    gtk_box_pack_start(GTK_BOX(hbox), nli->label, FALSE, FALSE, 0);
  g_free(utf);
  g_free(text);
  ...
  if (pc->jumpScaleA && pc->jumpScaleB && state == TRUE) {
    ...
    if (pc->radioAdaptiveA == widget || pc->radioAdaptiveB == widget) {
      ...
      if (Closure->readAndCreate) {
        ...
        activate_toggle_button(GTK_TOGGLE_BUTTON(pc->readAndCreateButtonA),
                               FALSE);
        activate_toggle_button(GTK_TOGGLE_BUTTON(pc->readAndCreateButtonB),
                               FALSE);
        ShowMessage(Closure->prefsWindow,
...
static void cache_defective_select_cb(GtkWidget *widget, gpointer data) {
  ...
  case 1: /* OK */
    ...
    Closure->dDumpDir = g_strdup(gtk_file_selection_get_filename(
        GTK_FILE_SELECTION(pc->cacheDefectiveChooser)));
...
static void cache_defective_dir_cb(GtkWidget *widget, gpointer data) {
  ...
  if (!pc->cacheDefectiveChooser) {
    ...
    pc->cacheDefectiveChooser =
        gtk_file_selection_new(_utf("Raw sector caching"));
    ReverseCancelOK(GTK_DIALOG(pc->cacheDefectiveChooser));

    g_signal_connect(G_OBJECT(pc->cacheDefectiveChooser), "destroy",
                     G_CALLBACK(cache_defective_select_cb), GINT_TO_POINTER(0));
    g_signal_connect(
        G_OBJECT(GTK_FILE_SELECTION(pc->cacheDefectiveChooser)->ok_button),
        "clicked", G_CALLBACK(cache_defective_select_cb), GINT_TO_POINTER(1));
    g_signal_connect(
        G_OBJECT(GTK_FILE_SELECTION(pc->cacheDefectiveChooser)->cancel_button),
        "clicked", G_CALLBACK(cache_defective_select_cb), GINT_TO_POINTER(2));

    sprintf(filename, "%s/", Closure->dDumpDir);
    gtk_file_selection_set_filename(
        GTK_FILE_SELECTION(pc->cacheDefectiveChooser), filename);
    ...
    gtk_widget_hide(
        GTK_FILE_SELECTION(pc->cacheDefectiveChooser)->selection_entry);
    set_entry_text(
        GTK_ENTRY(
            GTK_FILE_SELECTION(pc->cacheDefectiveChooser)->selection_entry),
        "");
...
static void logfile_select_cb(GtkWidget *widget, gpointer data) {
  ...
  case 1: /* OK */
    ...
    Closure->logFile = g_strdup(gtk_file_selection_get_filename(
        GTK_FILE_SELECTION(pc->logFileChooser)));
...
static void logfile_cb(GtkWidget *widget, gpointer data) {
  ...
  case LOGFILE_SELECT:
    if (!pc->logFileChooser) {
      ...
      g_signal_connect(G_OBJECT(pc->logFileChooser), "destroy",
                       G_CALLBACK(logfile_select_cb), GINT_TO_POINTER(0));
      g_signal_connect(
          G_OBJECT(GTK_FILE_SELECTION(pc->logFileChooser)->ok_button),
          "clicked", G_CALLBACK(logfile_select_cb), GINT_TO_POINTER(1));
      g_signal_connect(
          G_OBJECT(GTK_FILE_SELECTION(pc->logFileChooser)->cancel_button),
  ...
  case LOGFILE_DELETE: {
    GtkWidget *dialog = gtk_message_dialog_new(
        Closure->prefsWindow, GTK_DIALOG_DESTROY_WITH_PARENT,
...
void CreatePreferencesWindow(void) {
  if (!Closure->prefsWindow) /* No window to reuse? */
  {
    ...
    lwoh = CreateLabelWithOnlineHelp(_("Reading strategy"),
                                     _("Reading strategy: "));
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (!i)
        pc->radioLinearA = radio1;
      else
        pc->radioLinearB = radio1;
      g_signal_connect(G_OBJECT(radio1), "toggled", G_CALLBACK(strategy_cb),
                       pc);
      ...
      if (!i)
        pc->radioAdaptiveA = radio2;
      else
        pc->radioAdaptiveB = radio2;
      g_signal_connect(G_OBJECT(radio2), "toggled", G_CALLBACK(strategy_cb),
                       pc);
    ...
    AddHelpParagraph(
        ...
        _("<b>Reading strategy</b>\n\n"
          ...
          "- reading defective media when no error correction data is "
          "available.\n\n"
          ...
          "is possible but it is recommended to use linear reading in that "
          "case."),
        Closure->invisibleDash);

    /* Reading range */

    lwoh = CreateLabelWithOnlineHelp(_("Reading range"),
    ...
    for (i = 0; i < 2; i++) {
      ...
      //	 toggle = gtk_check_button_new_with_label(_utf("Read/Scan from
      // sector"));
      toggle = gtk_check_button_new();
      if (!i)
        pc->rangeToggleA = toggle;
      else
        pc->rangeToggleB = toggle;
      g_signal_connect(G_OBJECT(toggle), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_RANGE));
      ...
      if (!i)
        pc->rangeSpin1A = spin1;
      else
        pc->rangeSpin1B = spin1;
      ...
      if (!i)
        pc->rangeSpin2A = spin2;
      else
        pc->rangeSpin2B = spin2;
    ...
    AddHelpParagraph(
        lwoh,
        _("<b>Reading range</b>\n\n"
          "Reading can be limited to a part of the medium (in sectors holding "
          "2KB each). "
          "The values include the borders: 0-100 will read 101 sectors.\n\n"

          "<b>Note:</b> Limiting the reading range is not recommended for "
          "<i>adaptive reading</i> since it might "
          "prevent sectors from being read which are required for a successful "
          "error correction.\n\n"
          "These settings are only effective for the current session and will "
          "not be saved."));
    ...
    lwoh = CreateLabelWithOnlineHelp(
        _("Exhaustive RS02 header search"),
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (!i)
        pc->recogRS02A = button;
      else
        pc->recogRS02B = button;
      ...
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_RECOG_RS02));
      if (!i)
        gtk_box_pack_start(GTK_BOX(vbox2), hbox, FALSE, FALSE, 0);
      else
        AddHelpWidget(lwoh, hbox);
    ...
    lwoh = CreateLabelWithOnlineHelp(_("Recover RS03 signatures"),
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (!i)
        pc->recogRS03A = button;
      else
        pc->recogRS03B = button;

      activate_toggle_button(GTK_TOGGLE_BUTTON(button), Closure->examineRS03);
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_RECOG_RS03));
      if (!i)
        gtk_box_pack_start(GTK_BOX(vbox2), hbox, FALSE, FALSE, 0);
      else
        AddHelpWidget(lwoh, hbox);
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (!i)
        pc->ignoreISOSizeA = button;
      else
        pc->ignoreISOSizeB = button;

      activate_toggle_button(GTK_TOGGLE_BUTTON(button), Closure->ignoreIsoSize);
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_SIZEDRIVE));
      if (!i)
        gtk_box_pack_start(GTK_BOX(vbox2), hbox, FALSE, FALSE, 0);
      else
        AddHelpWidget(lwoh, hbox);
    ...
    AddHelpParagraph(
        ...
        _("<b>Ignore image size recorded in ISO/UDF filesystem</b>\n\n"
          "When reading or scanning optical discs, the overall size of the "
          "medium "
          ...
          "image from such CDs and its md5sum does not match the advertised "
          "one, "
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (!i)
        pc->daoButtonA = button;
      else
        pc->daoButtonB = button;

      activate_toggle_button(GTK_TOGGLE_BUTTON(button), Closure->noTruncate);
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_DAO));
      if (!i)
        gtk_box_pack_start(GTK_BOX(vbox2), hbox, FALSE, FALSE, 0);
      else
        AddHelpWidget(lwoh, hbox);
    ...
    AddHelpParagraph(lwoh, _("<b>Assume DAO mode</b>\n\n"
                             "Media written in \"TAO\" (\"track at once\") "
                             "mode may contain two sectors "
                             "with pseudo read errors at the end. By default "
                             "these two sectors are ignored.\n\n"

                             "If you are extremely unlucky to have a \"DAO\" "
                             "(\"disc at once\") medium "
                             "with exactly one or two real read errors at the "
                             "end, dvdisaster may treat "
                             "this as a \"TAO\" disc and truncate the image by "
                             "two sectors. In that case "
                             "activate this option to have the last two read "
                             "errors handled correctly.\n\n"

                             "<b>Tip:</b> To avoid these problems, consider "
                             "using the \"DAO / Disc at once\" "
                             "(sometimes also called \"SAO / Session at "
                             "once\") mode for writing single "
                             "session media."));
    ...
    lwoh = CreateLabelWithOnlineHelp(_("Missing sector tags"),
                                     _("Use new style missing sector tags "
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (!i)
        pc->dsmButtonA = button;
      else
        pc->dsmButtonB = button;

      activate_toggle_button(GTK_TOGGLE_BUTTON(button), Closure->dsmVersion);
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_DSM));
      if (!i)
        gtk_box_pack_start(GTK_BOX(vbox2), hbox, FALSE, FALSE, 0);
      else
        AddHelpWidget(lwoh, hbox);
    ...
    AddHelpParagraph(
        lwoh, _("<b>Missing sector tagging</b>\n\n"
    ...
    if (Closure->debugMode) {
      ...
      AddHelpParagraph(lwoh,
                       _("<b>Filling of unreadable sectors</b>\n\n"
                         ...
                         "dvdisaster marks unreadable sectors with a special "
                         "filling pattern which "
                         "is very unlikely to occur in undamaged media.\n"
                         "In other data recovery software it is common to fill "
                         "unreadable sectors "
                         "with a certain byte value. To allow interoperability "
                         "with such programs, "
                         "you can specify the byte value they are using:\n"));

      AddHelpListItem(lwoh, _("0xb0 (176 decimal): for compatibility with "
                              "h2cdimage published by \"c't\", "
                              "a German periodical.\n"));

      AddHelpParagraph(lwoh,
                       _("<b>Note:</b> Using zero filling (0x00, decimal 0) is "
                         "highly discouraged. "
                         "Most media contain regular zero filled sectors which "
    ...
    lwoh = CreateLabelWithOnlineHelp(_("Drive initialisation"), _("Wait"));
    RegisterPreferencesHelpWindow(lwoh);

    lwoh_clone =
        CloneLabelWithOnlineHelp(lwoh, _("seconds for drive to spin up"));
    ...
    for (i = 0; i < 2; i++) {
      ...
      g_signal_connect(spin, "value-changed", G_CALLBACK(spin_cb),
                       (gpointer)SPIN_DELAY);
      gtk_box_pack_start(GTK_BOX(hbox), spin, FALSE, FALSE, 0);

      gtk_box_pack_start(GTK_BOX(hbox),
                         i ? lwoh_clone->normalLabel : lwoh_clone->linkBox,
                         FALSE, FALSE, 0);
    ...
    AddHelpParagraph(
        ...
        _("<b>Drive initialisation</b>\n\n"
          "Waits the specified amount of seconds for letting the drive spin "
          "up. "
    ...
    lwoh = CreateLabelWithOnlineHelp(_("Raw reading mode"),
                                     _("Raw reading mode: "));
    RegisterPreferencesHelpWindow(lwoh);
    ...
    for (i = 0; i < 2; i++) {
      ...
      radio1 = gtk_radio_button_new(NULL);
      if (!i)
        pc->radioRawMode20A = radio1;
      else
        pc->radioRawMode20B = radio1;
      g_signal_connect(G_OBJECT(radio1), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_RAW_20H));
      gtk_box_pack_start(GTK_BOX(hbox), radio1, FALSE, FALSE, 0);
      ...
      radio2 = gtk_radio_button_new_from_widget(GTK_RADIO_BUTTON(radio1));
      if (!i)
        pc->radioRawMode21A = radio2;
      else
        pc->radioRawMode21B = radio2;
      g_signal_connect(G_OBJECT(radio2), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_RAW_21H));
      gtk_box_pack_start(GTK_BOX(hbox), radio2, FALSE, FALSE, 0);
      ...
      if (!i)
        pc->radioRawModeOtherA = radio3;
      else
        pc->radioRawModeOtherB = radio3;
      g_signal_connect(G_OBJECT(radio3), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_RAW_OTHER));
      ...
      gtk_box_pack_start(GTK_BOX(hbox), entry, FALSE, FALSE, 0);
      if (!i)
        pc->rawModeValueA = entry;
      else
        pc->rawModeValueB = entry;
    ...
    AddHelpParagraph(
        ...
        _("<b>Raw reading mode</b>\n\n"
          ...
          "some mode 0x21 reads to the cache might deliver additional "
          "information.\n\n"
          "<b>0x01</b> Some drives do the right thing when given this value, "
          "although this makes no sense according to the SCSI specs. Try "
          "entering "
    ...
    for (i = 0; i < 2; i++) {
      ...
      g_signal_connect(spin, "value-changed", G_CALLBACK(spin_cb),
                       (gpointer)SPIN_INTERNAL_ATTEMPTS);
      gtk_box_pack_start(GTK_BOX(hbox), spin, FALSE, FALSE, 0);
    ...
    AddHelpParagraph(
        lwoh, _("<b>Internal read attempts</b>\n\n"
    ...
    for (i = 0; i < 2; i++) {
      ...
      toggle = gtk_check_button_new();
      activate_toggle_button(GTK_TOGGLE_BUTTON(toggle),
                             Closure->ignoreFatalSense);
      g_signal_connect(G_OBJECT(toggle), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_FATAL_SENSE));
      gtk_box_pack_start(GTK_BOX(hbox), toggle, FALSE, FALSE, 0);
    ...
    lwoh = CreateLabelWithOnlineHelp(_("Raw reading"),
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (!i)
        pc->rawButtonA = button;
      else
        pc->rawButtonB = button;

      activate_toggle_button(GTK_TOGGLE_BUTTON(button), Closure->readRaw);
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_RAW));
    ...
    AddHelpParagraph(
        ...
        _("<b>Raw reading</b> (affects CD media only)\n\n"
          ...
          "- linear reading is configured to skip 0 sectors after a read "
          "error.\n"
          "Raw sector caching also needs checking of the respective option."));

    /* Minimum reading attempts */

    lwoh = CreateLabelWithOnlineHelp(_("Minimum number of reading attempts"),
                                     "ignore");
    RegisterPreferencesHelpWindow(lwoh);
    LockLabelSize(GTK_LABEL(lwoh->normalLabel),
                  _utf("Min. %d reading attempts per sector"), 99);
    LockLabelSize(GTK_LABEL(lwoh->linkLabel),
                  _utf("Min. %d reading attempts per sector"), 99);
    ...
    pc->minAttemptsScaleInfoA->format =
        g_strdup(_("Min. %d reading attempts per sector"));
    pc->minAttemptsScaleInfoB->format =
        g_strdup(_("Min. %d reading attempts per sector"));
    ...
    for (i = 0; i < 2; i++) {
      ...
      scale = non_linear_scale(
          &scale_box, i ? pc->minAttemptsScaleInfoB : pc->minAttemptsScaleInfoA,
          i ? lwoh->normalLabel : lwoh->linkLabel, pc, SLIDER_MIN_READ_ATTEMPTS,
          min_attempts_values, MIN_ATTEMPTS_VALUE_LENGTH,
          Closure->minReadAttempts);

      if (!i)
        pc->minAttemptsScaleA = scale;
      else
        pc->minAttemptsScaleB = scale;

      if (!i)
        gtk_box_pack_start(GTK_BOX(vbox2), scale_box, FALSE, FALSE, 0);
      else
        AddHelpWidget(lwoh, scale_box);
    }
    ...
    lwoh = CreateLabelWithOnlineHelp(_("Maximum number of reading attempts"),
                                     "ignore");
    RegisterPreferencesHelpWindow(lwoh);
    LockLabelSize(GTK_LABEL(lwoh->normalLabel),
                  _utf("Max. %d reading attempts per sector"), 100);
    LockLabelSize(GTK_LABEL(lwoh->linkLabel),
                  _utf("Max. %d reading attempts per sector"), 100);
    ...
    pc->maxAttemptsScaleInfoA->format =
        g_strdup(_("Max. %d reading attempts per sector"));
    pc->maxAttemptsScaleInfoB->format =
        g_strdup(_("Max. %d reading attempts per sector"));
    ...
    for (i = 0; i < 2; i++) {
      ...
      scale = non_linear_scale(
          &scale_box, i ? pc->maxAttemptsScaleInfoB : pc->maxAttemptsScaleInfoA,
          i ? lwoh->normalLabel : lwoh->linkLabel, pc, SLIDER_MAX_READ_ATTEMPTS,
          max_attempts_values, MAX_ATTEMPTS_VALUE_LENGTH,
          Closure->maxReadAttempts);

      if (!i)
        pc->maxAttemptsScaleA = scale;
      else
        pc->maxAttemptsScaleB = scale;
    ...
    lwoh =
        CreateLabelWithOnlineHelp(_("Treatment of unreadable areas"), "ignore");
    RegisterPreferencesHelpWindow(lwoh);
    if (GetLabelWidth(GTK_LABEL(lwoh->normalLabel),
                      _utf("Skip %d sectors after read error"), 20480) >
        GetLabelWidth(GTK_LABEL(lwoh->normalLabel),
                      _utf("Stop reading when unreadable intervals &lt; %d"),
                      20480)) {
      LockLabelSize(GTK_LABEL(lwoh->normalLabel),
                    _utf("Skip %d sectors after read error"), 20480);
      LockLabelSize(GTK_LABEL(lwoh->linkLabel),
    ...
    pc->formatAdaptive =
        g_strdup(_("Stop reading when unreadable intervals &lt; %d"));

    pc->jumpScaleInfoA = g_malloc0(sizeof(non_linear_info));
    pc->jumpScaleInfoB = g_malloc0(sizeof(non_linear_info));
    pc->jumpScaleInfoA->format =
        Closure->adaptiveRead ? pc->formatAdaptive : pc->formatLinear;
    pc->jumpScaleInfoB->format =
        Closure->adaptiveRead ? pc->formatAdaptive : pc->formatLinear;
    ...
    for (i = 0; i < 2; i++) {
      ...
      scale = non_linear_scale(
          &scale_box, i ? pc->jumpScaleInfoB : pc->jumpScaleInfoA,
          i ? lwoh->normalLabel : lwoh->linkLabel, pc, SLIDER_JUMP, jump_values,
          JUMP_VALUE_LENGTH, Closure->sectorSkip);
    ...
    AddHelpParagraph(lwoh, _("<b>Treatment of unreadable areas</b>\n\n"
                             "Defective media usually contain numerous read "
                             "errors in a contigous region. "
                             "Skipping sectors after a read error reduces the "
                             "processing time and the "
                             "mechanical wear on the drive, but leaves larger "
                             "gaps in the image file.\n\n"
                             "Effects on the <b>linear reading strategy</b>:"));

    AddHelpListItem(lwoh, _("Skipping a large number of sectors (e.g. 1024) "
                            "gives a quick overview of "
                            "damaged areas, but will usually not collect "
                            "enough data for repairing the image."));

    AddHelpListItem(lwoh, _("Smaller values like 16, 32 or 64 are a good "
                            "trade-off: The processing time will be"
                            "considerably shortened, but still enough data for "
                            "repairing the image is collected.\n"));

    AddHelpParagraph(lwoh, _("The <b>adaptive reading strategy</b> uses this "
                             "setting only if no error correction data "
                             "is available. In that case the reading process "
                             "will stop when no unread areas "
                             "larger than the selected size remain. Values "
                             "smaller than 128 <i>are not recommended</i> "
                             "as they cause the drive to carry out lots of "
                             "laser head repositioning during the "
                             "final phase of the reading process. If adaptive "
                             "reading with a setting of 128 is not "
                             "sufficient, try reading the remaining sectors "
                             "with an additional linear reading pass.\n\n"

                             "On DVD and BD media read errors do usually "
                             "extend over at least 16 sectors for technical "
                             "reasons. Therefore selecting a value less than "
                             "16 is not recommended for DVD and BD."));
    ...
    AddHelpParagraph(
        lwoh, _("<b>Media read attempts</b> for the linear reading strategy\n\n"
                "If unreadable sectors remain after reading the medium from "
                "start to end, "
                "the medium is read again upto he given number of times.\n\n"
                "Only the missing sectors will be tried in the additional "
                "reading passes."));
    ...
    for (i = 0; i < 2; i++) {
      ...
      button = gtk_check_button_new();
      gtk_table_attach(GTK_TABLE(table), button, 0, 1, 0, 1, GTK_SHRINK,
                       GTK_SHRINK, 0, 0);
      ...
      gtk_table_attach(GTK_TABLE(table), select, 2, 3, 0, 2, GTK_SHRINK,
                       GTK_SHRINK, 0, 0);
      g_signal_connect(G_OBJECT(select), "clicked",
                       G_CALLBACK(cache_defective_dir_cb), pc);
      ...
      if (Closure->readRaw && Closure->defectiveDump)
        ...
        activate_toggle_button(GTK_TOGGLE_BUTTON(button), FALSE);
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_CACHE_DEFECTIVE));

      if (!i)
        gtk_box_pack_start(GTK_BOX(vbox2), table, FALSE, FALSE, 0);
      else
        AddHelpWidget(lwoh, table);
    }
    ...
    AddHelpParagraph(
        ...
        _("<b>Raw sector caching</b>\n\n"
          ...
          "you need to clean it up manually after a successful medium "
          "recovery."));
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (!i)
        pc->cacheDefectivePrefixA = entry;
      else
        pc->cacheDefectivePrefixB = entry;
    ...
    for (i = 0; i < 2; i++) {
      ...
      g_signal_connect(G_OBJECT(chooser), "changed",
                       G_CALLBACK(method_select_cb), pc);
    ...
    AddHelpParagraph(
        ...
        _("<b>Error correction method</b>\n\n"
          ...
          "different codecs and ways available for storing the error "
          "correction "
          "information:\n"));

    AddHelpListItem(lwoh, _("The RS01 codec\n"
                            "RS01 is the recommended codec for storing error "
                            "correction data in separate files.\n"));
    ...
    AddHelpListItem(
        lwoh,
        _("The RS03 codec (Warning: experimental)\n"
          "RS03 can either store error correction data in a separate file "
          "or augment the image with it. It provides multithreading "
          "to scale with multicore processors and contains some subtle "
          "improvements "
          "over RS01 and RS02. However it should not be used for productive "
          "work "
    ...
    for (i = 0; i < Closure->methodList->len; i++) {
      ...
      if (i == method_idx)
        gtk_notebook_set_current_page(GTK_NOTEBOOK(pc->methodNotebook),
                                      method_idx);
    }
    ...
    AddHelpParagraph(lwoh, _("<b>Automatically add file suffixes</b>\n\n"
                             "When this switch is set, files will be "
                             "automatically appended with \".iso\" "
                             "or \".ecc\" suffixes if no other file name "
    ...
    for (i = 0; i < 2; i++) {
      ...
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_READ_CREATE));
    ...
    AddHelpParagraph(lwoh,
                     _("<b>Automatic error correction file creation</b>\n\n"
                       "Automatically creates an error correction file after "
                       "reading an image. "
                       "Together with the \"Remove image\" option this will "
    ...
    lwoh = CreateLabelWithOnlineHelp(
        _("Automatic image file removal"),
    ...
    AddHelpParagraph(lwoh,
                     _("<b>Automatic image file removal</b>\n\n"
                       "If this switch is set the image file will be deleted "
    ...
    for (i = 0; i < 2; i++) {
      ...
      gtk_box_pack_start(GTK_BOX(hbox), i ? lwoh->normalLabel : lwoh->linkBox,
                         FALSE, FALSE, 0);
      activate_toggle_button(GTK_TOGGLE_BUTTON(button),
                             Closure->confirmDeletion);
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_CONFIRM_DELETION));
    ...
    gtk_table_attach(GTK_TABLE(table), vbox3, 0, 1, 0, 3, GTK_EXPAND | GTK_FILL,
                     GTK_SHRINK, 5, 5);
    ...
    AddHelpParagraph(lwoh, _("<b>Good sectors</b>\n\n"
    ...
    AddHelpParagraph(lwoh, _("<b>Highlit sectors</b>\n\n"
    ...
    gtk_table_attach(GTK_TABLE(table), frame, 1, 2, 0, 1, GTK_EXPAND | GTK_FILL,
                     GTK_SHRINK, 5, 5);
    ...
    AddHelpParagraph(lwoh, _("<b>Positive text</b>\n\n"
    ...
    AddHelpParagraph(lwoh, _("<b>Negative text</b>\n\n"
                             "Bad news are printed in this color."));

    /** Curve colors */

    frame = gtk_frame_new(_utf("Curve colors"));
    gtk_table_attach(GTK_TABLE(table), frame, 1, 2, 1, 2, GTK_EXPAND | GTK_FILL,
                     GTK_SHRINK, 5, 5);
    vbox2 = gtk_vbox_new(FALSE, 20);
    gtk_container_set_border_width(GTK_CONTAINER(vbox2), 10);
    gtk_container_add(GTK_CONTAINER(frame), vbox2);
    ...
    AddHelpParagraph(lwoh, _("<b>C2 error color</b>\n\n"
    ...
    lwoh = CreateLabelWithOnlineHelp(_("Error correction load"),
                                     _("Error correction load"));
    RegisterPreferencesHelpWindow(lwoh);
    ...
    AddHelpParagraph(
        lwoh, _("<b>Error correction load</b>\n\n"
    ...
    g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(default_color_cb),
                     NULL);
    ...
    lwoh = CreateLabelWithOnlineHelp(_("Reverse OK / Cancel buttons"),
    ...
    for (i = 0; i < 2; i++) {
      ...
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_CANCEL_OK));
    ...
    AddHelpParagraph(
        lwoh, _("<b>Reverse OK / Cancel buttons</b>\n\n"
    ...
    lwoh =
        CreateLabelWithOnlineHelp(_("Verbose logging"), _("Verbose logging"));
    RegisterPreferencesHelpWindow(lwoh);
    ...
    for (i = 0; i < 2; i++) {
      ...
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_VERBOSE));
    ...
    lwoh = CreateLabelWithOnlineHelp(_("Logfile:"), _("Copy log to file:"));
    RegisterPreferencesHelpWindow(lwoh);
    ...
    for (i = 0; i < 2; i++) {
      ...
      gtk_table_attach(GTK_TABLE(table), delete, 3, 4, 0, 2, GTK_SHRINK,
                       GTK_SHRINK, 0, 0);
      ...
      if (Closure->verbose && Closure->logFileEnabled)
        ...
        activate_toggle_button(GTK_TOGGLE_BUTTON(button), FALSE);
      g_signal_connect(G_OBJECT(button), "toggled", G_CALLBACK(toggle_cb),
                       GINT_TO_POINTER(TOGGLE_LOGFILE));
