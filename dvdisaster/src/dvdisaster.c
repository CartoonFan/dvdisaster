...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(locale_test, "ok")) {
    ...
    if (getcwd(buf, 256)) {
      ...
      locale_test =
          gettext("test phrase for verifying the locale installation");
    }
  }
#endif
  ...
  for (;;) {
    ...
    c = getopt_long(argc, argv,
                    "a:cd:e:fhi:j:lm::n:o:p:r::s::t::uvx:", long_options,
                    &option_index);

    if (c == -1)
      break;

    switch (c) { /* Mingw has not strcasestr() */
    case 'a':
      if (strstr(optarg, "rs02") || strstr(optarg, "RS02"))
    ...
    case 'j':
      if (optarg)
        Closure->sectorSkip = atoi(optarg) & ~0xf;
      if (Closure->sectorSkip < 0)
    ...
    case 'o':
      if (!strcmp(optarg, "file"))
        ...
        Stop(_("-o/--ecc-target expects 'file' or 'image'"));
      break;
    case 'p':
      if (optarg) {
        g_free(Closure->imageName);
    ...
    case 'r':
      mode = MODE_SEQUENCE;
      sequence |= 1 << MODE_READ;
      if (optarg)
        read_range = g_strdup(optarg);
      break;

    case 's':
      mode = MODE_SEQUENCE;
      sequence |= 1 << MODE_SCAN;
      if (optarg)
        read_range = g_strdup(optarg);
      break;
    case 'v':
      Closure->verbose = TRUE;
      break;
    case 't':
      mode = MODE_SEQUENCE;
      sequence |= 1 << MODE_VERIFY;
      if (optarg)
        Closure->quickVerify = TRUE;
      break;
    case 'u':
      Closure->unlinkImage = TRUE;
      break;
    case 'x':
      Closure->codecThreads = atoi(optarg);
      if (Closure->codecThreads < 1 ||
          Closure->codecThreads > MAX_CODEC_THREADS)
    ...
    case MODIFIER_CACHE_SIZE:
      ...
      if (Closure->cacheMB < 8)
        Stop(_("--cache-size must at least be 8MB; 16MB or higher is "
               "recommended."));
    ...
    case MODIFIER_DRIVER:
      ...
      else if (optarg && !strcmp(optarg, "cdrom"))
    ...
    case MODIFIER_FILL_UNREADABLE:
      if (optarg)
        Closure->fillUnreadable = strtol(optarg, NULL, 0);
      break;
    ...
    case MODIFIER_TRUNCATE:
      if (optarg) /* debugging truncate mode */
      {
        ...
        Closure->truncate = 1; /* truncate confirmation for fix mode */
      break;
    case MODIFIER_DEBUG:
    ...
    case MODIFIER_RANDOM_SEED:
      if (optarg)
        Closure->randomSeed = atoi(optarg);
      break;
    case MODIFIER_RAW_MODE:
      if (optarg)
        Closure->rawMode = strtol(optarg, NULL, 16);
      break;
    ...
    case MODIFIER_SIMULATE_DEFECTS:
      if (optarg)
        Closure->simulateDefects = atoi(optarg);
      else
        Closure->simulateDefects = 10;
      break;
    case MODIFIER_SPINUP_DELAY:
      if (optarg)
        Closure->spinupDelay = atoi(optarg);
      break;
    case MODIFIER_SPEED_WARNING:
      if (optarg)
        Closure->speedWarning = atoi(optarg);
      else
        Closure->speedWarning = 10;
      break;
    ...
    case '?':
      ...
      break;
    default:
      PrintCLI(_("?? illegal getopt return value %d\n"), c);
      break;
    }
  }
  ...
  if (mode != MODE_SIGN && !VerifySignature()) {
    ...
    if (Closure->version % 100)
      sprintf(version, "dvdisaster-%s.%d-setup.exe", VERSION,
              Closure->version % 100);
    else
      sprintf(version, "dvdisaster-%s-setup.exe", VERSION);
    ...
    if (dashpos) {
      *dashpos = 0;
      ...
      if (strlen(dashpos + 1) == 3 && !strncmp(dashpos + 1, "end", 3))
        ...
        Closure->readEnd = atoi(dashpos + 1);
    } else
      Closure->readStart = Closure->readEnd = atoi(read_range);
    g_free(read_range);
  }
  ...
  case MODE_SEQUENCE:
    ...
    if (sequence & 1 << MODE_FIX) {
      ...
      if (!image) {
        ...
        if (image->inLast == 2048)
          ...
          PrintLog(_(": %lld medium sectors and %d bytes.\n"),
      ...
      if (image && image->eccFileMethod)
        method = image->eccFileMethod;
      else if (image && image->eccMethod)
        method = image->eccMethod;
      else
        Stop("Internal error: No suitable method for repairing image.");
    ...
    if (sequence & 1 << MODE_VERIFY) {
      ...
      if (!image || !image->eccMethod)
        image =
            OpenEccFileForImage(image, Closure->eccName, O_RDONLY, IMG_PERMS);
...
#endif
  {
    ...
    PrintCLI(
        _("\nCommon usage examples:\n"
          ...
          "                         # Use -rn-m to read a certain sector "
          "range, e.g. -r100-200\n"
          "  dvdisaster -c,--create # Create .ecc information for the medium "
          "image.\n"
          "  dvdisaster -f,--fix    # Try to fix medium image using .ecc "
          "information.\n"
          "  dvdisaster -s,--scan   # Scan the medium for read errors.\n"
          "  dvdisaster -t,--test   # Test integrity of the .iso and .ecc "
          "files.\n"
          "  dvdisaster -u,--unlink # Delete .iso files (when other actions "
          "complete)\n\n"));

    PrintCLI(
        _("Drive and file specification:\n"
          "  -d,--device device     - read from given device   (default: %s)\n"
          "  -p,--prefix prefix     - prefix of .iso/.ecc file (default: "
          "medium.*  )\n"
          "  -i,--image  imagefile  - name of image file       (default: "
          "medium.iso)\n"
          "  -e,--ecc    eccfile    - name of parity file      (default: "
          "medium.ecc)\n"
    ...
    PrintCLI(_("  -a,--assume x,y,...    - assume image is augmented with "
               "codec(s) x,y,...\n"));
    PrintCLI(_("  -j,--jump n            - jump n sectors forward after a read "
               "error (default: 16)\n"));
    PrintCLI(_("  -m n                   - list/select error correction "
               "methods (default: RS01)\n"));
    PrintCLI(_("  -n,--redundancy n%%     - error correction data redundancy\n"
               "                           allowed values depend on codec (see "
               "manual)\n"));
    PrintCLI(_("  -v,--verbose           - more diagnostic messages\n"));
    PrintCLI(_("  -x,--threads n         - use n threads for en-/decoding (if "
               "supported by codec)\n"));
    PrintCLI(_("  --adaptive-read        - use optimized strategy for reading "
               "damaged media\n"));
    PrintCLI(_("  --auto-suffix          - automatically add .iso and .ecc "
               "file suffixes\n"));
    PrintCLI(_("  --cache-size n         - image cache size in MB during -c "
               "mode (default: 32MB)\n"));
    PrintCLI(_(
        "  --dao                  - assume DAO disc; do not trim image end\n"));
    PrintCLI(_("  --defective-dump d     - directory for saving incomplete raw "
               "sectors\n"));
#ifdef SYS_LINUX
    PrintCLI(_("  --driver=sg/cdrom      - use sg(default) or alternative "
               "cdrom driver (see man page!)\n"));
#endif
    PrintCLI(
        _("  --eject                - eject medium after successful read\n"));
    PrintCLI(
        _("  --fill-unreadable n    - fill unreadable sectors with byte n\n"));
    PrintCLI(_("  --ignore-fatal-sense   - continue reading after potentially "
               "fatal error conditon\n"));
    PrintCLI(_("  --ignore-iso-size      - ignore image size from ISO/UDF data "
               "(dangerous - see man page!)\n"));
    PrintCLI(_("  --internal-rereads n   - drive may attempt n rereads before "
               "reporting an error\n"));
    ...
    PrintCLI(_("  --read-attempts n-m    - attempts n upto m reads of a "
               "defective sector\n"));
    PrintCLI(
        _("  --read-medium n        - read the whole medium up to n times\n"));
    PrintCLI(_(
        "  --read-raw             - performs read in raw mode if possible\n"));
    PrintCLI(_("  --speed-warning n      - print warning if speed changes by "
               "more than n percent\n"));
    PrintCLI(
        _("  --spinup-delay n       - wait n seconds for drive to spin up\n"));

    if (Closure->debugMode) {
      PrintCLI("\n");
      PrintCLI(_("Debugging options (purposefully undocumented and possibly "
                 "harmful)\n"));
      PrintCLI(_("  --debug           - enables the following options\n"));
      PrintCLI(_("  --byteset s,i,b   - set byte i in sector s to b\n"));
      PrintCLI(_("  --cdump           - creates C #include file dumps instead "
                 "of hexdumps\n"));
      PrintCLI(
          _("  --compare-images a,b  - compare sectors in images a and b\n"));
      PrintCLI(_("  --copy-sector a,n,b,m - copy sector n from image a to "
                 "sector m in image b\n"));
      ...
      PrintCLI(_("  --merge-images a,b  merge image a with b (a receives "
                 "sectors from b)\n"));
      PrintCLI(_("  --random-errors r,e seed image with (correctable) random "
                 "errors\n"));
      PrintCLI(_("  --random-image n  - create image with n sectors of random "
                 "numbers\n"));
      PrintCLI(_("  --random-seed n   - random seed for built-in random number "
                 "generator\n"));
      PrintCLI(_("  --raw-sector n    - shows hexdump of the given raw sector "
                 "from medium in drive\n"));
      PrintCLI(_("  --read-sector n   - shows hexdump of the given sector from "
                 "medium in drive\n"));
      PrintCLI(_("  --screen-shot     - useful for generating screen shots\n"));
      PrintCLI(_("  --send-cdb arg    - executes given cdb at drive; kills "
                 "system if used wrong\n"));
      PrintCLI(_("  --show-sector n   - shows hexdump of the given sector in "
                 "an image file\n"));
      PrintCLI(_(
          "  --sim-defects n   - simulate n%% defective sectors on medium\n"));
      PrintCLI(_("  --truncate n      - truncates image to n sectors\n"));
      PrintCLI(_("  --zero-unreadable - replace the \"unreadable sector\" "
                 "markers with zeros\n\n"));
    }

#ifdef WIN_CONSOLE
    PrintCLI(
        _("NOTE: This is the Windows console version of dvdisaster.\n"
