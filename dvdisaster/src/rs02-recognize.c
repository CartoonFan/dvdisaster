...
static void insert_buttons(GtkDialog *dialog) {
  ...
  gtk_dialog_add_buttons(dialog, _utf("Skip RS02 test"), 1,
  ...
  check = gtk_check_button_new_with_label(
      _utf("Disable RS02 initialization in the preferences"));
...
static int try_sector(Image *image, gint64 pos, EccHeader **ehptr,
  ...
  if (strncmp((char *)eh->cookie, "*dvdisaster*RS02", 16)) {
    if (image->type == IMAGE_MEDIUM && image->dh->rewriteable) {
      Verbose(
          "try_sector: no cookie but rewriteable medium: skipping header\n");
      return TRY_NEXT_HEADER;
    } else {
    ...
    Verbose("try_sector: header at %lld: magic cookie found\n",
            (long long int)pos);
...
int RS02Recognize(Image *image) {
  ...
  if (image->isoInfo) {
    ...
    if (try_sector(image, iso_size, &image->eccHeader, ab->buf) ==
        HEADER_FOUND) {
    ...
    if (try_sector(image, iso_size - 150, &image->eccHeader, ab->buf) ==
        HEADER_FOUND) {
    ...
    while (pos > 0) {
      ...
      case TRY_NEXT_HEADER:
        ...
        if (!answered_continue && read_count > 5) {
          if (Closure->guiMode) {
            int answer = ModalDialog(
                GTK_MESSAGE_QUESTION, GTK_BUTTONS_NONE, insert_buttons,
                _("Faster medium initialization\n\n"
                  "Searching this medium for error correction data may take a "
                  "long time.\n"
                  "Press \"Skip RS02 test\" if you are certain that this "
                  "medium was\n"
...
static int read_fingerprint(LargeFile *file, unsigned char *fingerprint,
                            gint64 sector) {
    ...
    while (pos > 0) {
      if (LargeSeek(file, 2048 * pos)) {
        ...
        if ((CheckForMissingSector(buf, pos, NULL, 0) != SECTOR_PRESENT) ||
            (CheckForMissingSector(buf + 2048, pos + 1, NULL, 0) !=
             SECTOR_PRESENT)) {
