...
static void expand_image(fix_closure *fc, gint64 new_size) {
  ...
  for (sectors = 0; sectors < new_sectors; sectors++) {
    ...
    CreateMissingSector(buf, image->sectorSize + sectors, fc->eh->mediumFP,
                        FINGERPRINT_SECTOR, "RS02 fix placeholder");
...
void RS02Fix(Image *image) {
  ...
  msg = g_strdup_printf(_("Image contains error correction data: Method RS02, "
                          "%d roots, %4.1f%% redundancy."),
                        eh->eccBytes,
  ...
  crc_sector_byte =
      2048 * (lay->dataSectors + 2); /* first sector with CRC info */
  crc_idx = 0;
  ...
  for (s = 0; s < lay->sectorsPerLayer; s++) {
    ...
    if (cache_sector >= cache_size) {
      ...
      for (i = 0; i < ndata; i++) /* Read data portion */
      {
        ...
        for (j = 0; j < cache_size; j++) {
          RS02ReadSector(image, lay, fc->imgBlock[i] + offset,
                         block_idx[i] + j);
          offset += 2048;
        }
      }
      ...
      for (i = 0; i < nroots; i++) /* and ecc portion */
      {
        ...
        for (j = 0; j < cache_size; j++) {
          ...
          if (!LargeSeek(image->file, 2048 * esi))
            Stop(_("Failed seeking to sector %lld in image: %s"), esi,
                 strerror(errno));

          if (LargeRead(image->file, fc->imgBlock[i + ndata] + offset, 2048) !=
              2048)
            Stop(_("Failed reading sector %lld in image: %s"), esi,
                 strerror(errno));
    ...
    for (i = 0; i < lay->ndata; i++) /* Check the data sectors */
    {
      erasure_map[i] = 0;
      if (block_idx[i] <
          lay->protectedSectors) /* ignore the padding sectors! */
      {
        int err =
            CheckForMissingSector(fc->imgBlock[i] + cache_offset, block_idx[i],
        ...
        if (block_idx[i] < lay->dataSectors) /* only data sectors have CRCs */
        {
          ...
          if (crc_idx >= 512) {
            ...
            err = CheckForMissingSector((unsigned char *)crc_buf,
                                        crc_sector_byte / 2048, eh->mediumFP,
                                        eh->fpSector);
          ...
          data_count++;
          crc_idx++;
        } else if (block_idx[i] >= lay->dataSectors + 2)
          crc_count++;
      }
    }
    ...
    if (erasure_count) {
      ...
      for (i = 0; i < 255; i++) {
        ...
        if (!LargeSeek(image->file, (gint64)(2048 * sec)))
          Stop(_("Failed seeking to sector %lld in image [%s]: %s"), sec, "FW",
               strerror(errno));

        if (sec < lay->dataSectors - 1)
          length = 2048; // FIXME: sec != ...
        else
          length =
              image->inLast; /* error: use inLast calculated from eh->sectors */
