...
void CreateRS02EncWindow(Method *method, GtkWidget *parent) {
  ...
  gtk_table_attach(GTK_TABLE(table), wid, 0, 1, 0, 1, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 20, 20);

  pbar = wl->encPBar1 = gtk_progress_bar_new();
  gtk_table_attach(GTK_TABLE(table), pbar, 1, 2, 0, 1, GTK_EXPAND | GTK_FILL,
                   GTK_EXPAND, 20, 20);
  ...
  gtk_misc_set_alignment(GTK_MISC(wid), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table), wid, 0, 1, 1, 2, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 20, 20);

  pbar = wl->encPBar2 = gtk_progress_bar_new();
  gtk_table_attach(GTK_TABLE(table), pbar, 1, 2, 1, 2, GTK_EXPAND | GTK_FILL,
                   GTK_EXPAND, 20, 20);
...
void RS02SetFixMaxValues(RS02Widgets *wl, int data_bytes, int ecc_bytes,
                         gint64 sectors) {
...
static gboolean results_idle_func(gpointer data) {
  ...
  SetLabelText(GTK_LABEL(wl->fixUncorrected),
               _("Unrepairable: <span %s>%lld</span>"), Closure->redMarkup,
               wl->uncorrected);
  SetLabelText(GTK_LABEL(wl->fixProgress), _("Progress: %3d.%1d%%"),
               wl->percent / 10, wl->percent % 10);

  return FALSE;
}

void RS02UpdateFixResults(RS02Widgets *wl, gint64 corrected,
                          gint64 uncorrected) {
...
static gboolean curve_idle_func(gpointer data) {
  ...
  if (wl->fixCurve->ivalue[wl->percent] > 0) {
    ...
    gdk_draw_rectangle(wl->fixCurve->widget->window, Closure->drawGC, TRUE, x0,
                       y, x0 == x1 ? 1 : x1 - x0, wl->fixCurve->bottomY - y);
  }
  wl->lastPercent = wl->percent;
  ...
  gdk_draw_line(wl->fixCurve->widget->window, Closure->drawGC,
...
static void update_geometry(RS02Widgets *wl) {
  ...
  gtk_box_set_child_packing(GTK_BOX(wl->fixFootlineBox), wl->fixCorrected, TRUE,
                            TRUE, wl->fixCurve->leftX, GTK_PACK_START);
...
void CreateRS02FixWindow(Method *method, GtkWidget *parent) {
  ...
  wl->fixCurve =
      CreateCurve(d_area, _("Errors/Ecc block"), "%d", 1000, CURVE_PERCENT);
...
enum { PREF_NROOTS = 0, PREF_CACHE = 1, PREF_ECC_SIZE = 2 };

#ifdef HAVE_32BIT
static int cache_size[] = {8,   16,  32,  64,  96,   128, 192,
...
static gchar *format_cb(GtkScale *scale, gdouble value, gpointer data) {
  ...
  if (GPOINTER_TO_INT(data) == PREF_CACHE)
    ...
    label = g_strdup_printf(_utf("%4.1f%% redundancy (%d roots)"),
                            ((double)nroots * 100.0) / (double)ndata, nroots);
...
void ResetRS02PrefsPage(Method *method) {
  ...
  if (wl->cacheScaleA)
    gtk_range_set_value(GTK_RANGE(wl->cacheScaleA),
                        index > 0 ? index - 1 : index);
  if (wl->cacheScaleB)
    gtk_range_set_value(GTK_RANGE(wl->cacheScaleB),
                        index > 0 ? index - 1 : index);
}
...
void CreateRS02PrefsPage(Method *method, GtkWidget *parent) {
  ...
  lwoh = CreateLabelWithOnlineHelp(
      _("Using the smallest possible size from table"),
  ...
  gtk_table_attach(GTK_TABLE(table), lwoh->linkBox, 0, 5, 0, 1,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);
  ...
  gtk_misc_set_alignment(GTK_MISC(lwoh->linkLabel), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table), lwoh->linkBox, 0, 1, 1, 2,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  wl->cdEntryA = gtk_entry_new();
  gtk_entry_set_width_chars(GTK_ENTRY(wl->cdEntryA), 9);
  g_signal_connect(G_OBJECT(wl->cdEntryA), "activate",
                   G_CALLBACK(entry_tracker_cb), wl);
  gtk_table_attach(GTK_TABLE(table), wl->cdEntryA, 1, 2, 1, 2,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  wl->cdButtonA = gtk_button_new_with_label(_utf("query medium"));
  g_signal_connect(G_OBJECT(wl->cdButtonA), "clicked", G_CALLBACK(query_cb),
                   wl);
  gtk_table_attach(GTK_TABLE(table), wl->cdButtonA, 2, 3, 1, 2,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);
  ...
  g_signal_connect(G_OBJECT(wl->cdUndoButtonA), "clicked", G_CALLBACK(query_cb),
                   wl);
  gtk_table_attach(GTK_TABLE(table), wl->cdUndoButtonA, 3, 4, 1, 2,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);
  ...
  g_signal_connect(G_OBJECT(wl->cdUndoButtonB), "clicked", G_CALLBACK(query_cb),
                   wl);
  ...
  gtk_misc_set_alignment(GTK_MISC(lwoh->linkLabel), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table), lwoh->linkBox, 0, 1, 2, 3,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  wl->dvdEntry1A = gtk_entry_new();
  gtk_entry_set_width_chars(GTK_ENTRY(wl->dvdEntry1A), 9);
  g_signal_connect(G_OBJECT(wl->dvdEntry1A), "activate",
                   G_CALLBACK(entry_tracker_cb), wl);
  gtk_table_attach(GTK_TABLE(table), wl->dvdEntry1A, 1, 2, 2, 3,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  wl->dvdButton1A = gtk_button_new_with_label(_utf("query medium"));
  g_signal_connect(G_OBJECT(wl->dvdButton1A), "clicked", G_CALLBACK(query_cb),
                   wl);
  gtk_table_attach(GTK_TABLE(table), wl->dvdButton1A, 2, 3, 2, 3,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  icon = gtk_image_new_from_stock(GTK_STOCK_UNDO, GTK_ICON_SIZE_SMALL_TOOLBAR);
  wl->dvdUndoButton1A = gtk_button_new();
  gtk_container_add(GTK_CONTAINER(wl->dvdUndoButton1A), icon);
  g_signal_connect(G_OBJECT(wl->dvdUndoButton1A), "clicked",
                   G_CALLBACK(query_cb), wl);
  gtk_table_attach(GTK_TABLE(table), wl->dvdUndoButton1A, 3, 4, 2, 3,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);
  ...
  g_signal_connect(G_OBJECT(wl->dvdUndoButton1B), "clicked",
                   G_CALLBACK(query_cb), wl);
  ...
  gtk_misc_set_alignment(GTK_MISC(lwoh->linkLabel), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table), lwoh->linkBox, 0, 1, 3, 4,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  wl->dvdEntry2A = gtk_entry_new();
  gtk_entry_set_width_chars(GTK_ENTRY(wl->dvdEntry2A), 9);
  g_signal_connect(G_OBJECT(wl->dvdEntry2A), "activate",
                   G_CALLBACK(entry_tracker_cb), wl);
  gtk_table_attach(GTK_TABLE(table), wl->dvdEntry2A, 1, 2, 3, 4,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  wl->dvdButton2A = gtk_button_new_with_label(_utf("query medium"));
  g_signal_connect(G_OBJECT(wl->dvdButton2A), "clicked", G_CALLBACK(query_cb),
                   wl);
  gtk_table_attach(GTK_TABLE(table), wl->dvdButton2A, 2, 3, 3, 4,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  icon = gtk_image_new_from_stock(GTK_STOCK_UNDO, GTK_ICON_SIZE_SMALL_TOOLBAR);
  wl->dvdUndoButton2A = gtk_button_new();
  gtk_container_add(GTK_CONTAINER(wl->dvdUndoButton2A), icon);
  g_signal_connect(G_OBJECT(wl->dvdUndoButton2A), "clicked",
                   G_CALLBACK(query_cb), wl);
  gtk_table_attach(GTK_TABLE(table), wl->dvdUndoButton2A, 3, 4, 3, 4,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);
  ...
  g_signal_connect(G_OBJECT(wl->dvdUndoButton2B), "clicked",
                   G_CALLBACK(query_cb), wl);
  ...
  gtk_misc_set_alignment(GTK_MISC(lwoh->linkLabel), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table), lwoh->linkBox, 0, 1, 4, 5,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  wl->bdEntry1A = gtk_entry_new();
  gtk_entry_set_width_chars(GTK_ENTRY(wl->bdEntry1A), 9);
  g_signal_connect(G_OBJECT(wl->bdEntry1A), "activate",
                   G_CALLBACK(entry_tracker_cb), wl);
  gtk_table_attach(GTK_TABLE(table), wl->bdEntry1A, 1, 2, 4, 5,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  wl->bdButton1A = gtk_button_new_with_label(_utf("query medium"));
  g_signal_connect(G_OBJECT(wl->bdButton1A), "clicked", G_CALLBACK(query_cb),
                   wl);
  gtk_table_attach(GTK_TABLE(table), wl->bdButton1A, 2, 3, 4, 5,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  icon = gtk_image_new_from_stock(GTK_STOCK_UNDO, GTK_ICON_SIZE_SMALL_TOOLBAR);
  wl->bdUndoButton1A = gtk_button_new();
  gtk_container_add(GTK_CONTAINER(wl->bdUndoButton1A), icon);
  g_signal_connect(G_OBJECT(wl->bdUndoButton1A), "clicked",
                   G_CALLBACK(query_cb), wl);
  gtk_table_attach(GTK_TABLE(table), wl->bdUndoButton1A, 3, 4, 4, 5,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);
  ...
  g_signal_connect(G_OBJECT(wl->bdUndoButton1B), "clicked",
                   G_CALLBACK(query_cb), wl);
  ...
  gtk_misc_set_alignment(GTK_MISC(lwoh->linkLabel), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table), lwoh->linkBox, 0, 1, 5, 6,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  wl->bdEntry2A = gtk_entry_new();
  gtk_entry_set_width_chars(GTK_ENTRY(wl->bdEntry2A), 9);
  g_signal_connect(G_OBJECT(wl->bdEntry2A), "activate",
                   G_CALLBACK(entry_tracker_cb), wl);
  gtk_table_attach(GTK_TABLE(table), wl->bdEntry2A, 1, 2, 5, 6,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  wl->bdButton2A = gtk_button_new_with_label(_utf("query medium"));
  g_signal_connect(G_OBJECT(wl->bdButton2A), "clicked", G_CALLBACK(query_cb),
                   wl);
  gtk_table_attach(GTK_TABLE(table), wl->bdButton2A, 2, 3, 5, 6,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);

  icon = gtk_image_new_from_stock(GTK_STOCK_UNDO, GTK_ICON_SIZE_SMALL_TOOLBAR);
  wl->bdUndoButton2A = gtk_button_new();
  gtk_container_add(GTK_CONTAINER(wl->bdUndoButton2A), icon);
  g_signal_connect(G_OBJECT(wl->bdUndoButton2A), "clicked",
                   G_CALLBACK(query_cb), wl);
  gtk_table_attach(GTK_TABLE(table), wl->bdUndoButton2A, 3, 4, 5, 6,
                   GTK_SHRINK | GTK_FILL, GTK_SHRINK, 5, 5);
  ...
  g_signal_connect(G_OBJECT(wl->bdUndoButton2B), "clicked",
                   G_CALLBACK(query_cb), wl);
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (!i)
      wl->radio2A = radio =
          gtk_radio_button_new_from_widget(GTK_RADIO_BUTTON(wl->radio1A));
    else
      wl->radio2B = radio =
          gtk_radio_button_new_from_widget(GTK_RADIO_BUTTON(wl->radio1B));

    g_signal_connect(G_OBJECT(radio), "toggled", G_CALLBACK(toggle_cb), method);
    gtk_box_pack_start(GTK_BOX(hbox), radio, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(hbox), !i ? lwoh->linkBox : lwoh->normalLabel,
                       FALSE, FALSE, 0);

    entry = gtk_entry_new();
    gtk_entry_set_width_chars(GTK_ENTRY(entry), 9);
    g_signal_connect(G_OBJECT(entry), "activate", G_CALLBACK(entry_tracker_cb),
                     wl);
  ...
  AddHelpParagraph(
      lwoh, _("<b>Use at most ... sectors</b>\n\n"
  ...
  wl->cacheLwoh = lwoh;
  LockLabelSize(GTK_LABEL(lwoh->normalLabel), _utf("%d MB of file cache"),
                2222);
  ...
  for (i = 0; i < 2; i++) {
    ...
    g_signal_connect(scale, "format-value", G_CALLBACK(format_cb),
                     (gpointer)PREF_CACHE);
    g_signal_connect(scale, "value-changed", G_CALLBACK(cache_cb),
                     (gpointer)wl);
