...
#ifdef HAVE_BIG_ENDIAN
#define bp_get_long_lsb(buf, idx)                                              \
  (((guint32)buf[(idx) + 2]) << 24 | ((guint32)buf[(idx) + 1]) << 16 |         \
   ((guint32)buf[(idx)]) << 8 | ((guint32)buf[(idx)-1]))
#else
#define bp_get_long_lsb(buf, idx) (*((guint32 *)(buf + idx - 1)))
#endif
#define bp_get_long_msb(buf, idx)                                              \
  (((guint32)buf[(idx)-1]) << 24 | ((guint32)buf[idx]) << 16 |                 \
   ((guint32)buf[(idx) + 1]) << 8 | ((guint32)buf[(idx) + 2]))
...
static void bp_get(unsigned char *dest, unsigned char *src, int begin,
                   int end) {
...
static void bp_get_string(unsigned char *dest, unsigned char *src, int begin,
                          int end) {
  int length = end - begin + 1;
...
static void bp_set_d1_string(unsigned char *dest, char *src, int begin,
                             int end) {
  dest = dest + begin - 1;
...
static void set_date(unsigned char *date, int idx, int year, int month, int day,
                     int hour, int minute, int second, int hsecond,
                     int gmt_offset) {
  sprintf((char *)(date + idx - 1), "%04d%02d%02d%02d%02d%02d%02d", year, month,
          day, hour, minute, second, hsecond);

  *(date + idx + 15) = (unsigned char)gmt_offset;
}

static void set_binary_date(unsigned char *dest, int idx, int year, int month,
...
void FreeIsoInfo(IsoInfo *ii) { g_free(ii); }
...
static IsoInfo *examine_primary_vd(unsigned char *buf) {
  ...
  if (!Closure->screenShotMode) {
    ...
    strcpy(ii->volumeLabel, _("Example disc"));
...
static IsoInfo *examine_iso(Image *image) {
  ...
  for (sector = 16; sector < 32; sector++) {
    ...
    switch (vdt) {
    case 0:
      Verbose("   -> boot record: *skipped*\n");
      break;
    case 1:
      Verbose("   -> primary volume descriptor:\n");
      ii = examine_primary_vd(buf);
      break;
    case 2:
      Verbose("   -> supplementary volume descriptor: *skipped*\n");
      break;
    case 255:
      Verbose("   -> volume descriptor set terminator;\n"
              "      end of ISO file system parsing.\n");
      goto finished;
      break;
    default:
      Verbose("   -> unknown volume descriptor: *skipped*\n");
      break;
    }
  }
...
static void add_directory_record(IsoDir *iso_dir, int extent, int file_flags,
                                 int file_identifier, int year, int month,
                                 int day, int hour, int minute, int second,
                                 int gmt_offset) {
...
static void add_file_record(IsoDir *iso_dir, char *filename, guint64 size,
                            int extent, int year, int month, int day, int hour,
                            int minute, int second, int gmt_offset) {
  unsigned char *dir = iso_dir->dir + iso_dir->tail;
...
static void add_d1_file_record(IsoDir *iso_dir, char *filename, guint64 size,
                               int extent, int year, int month, int day,
...
IsoHeader *InitIsoHeader(void) {
  ...
  add_directory_record(ih->proot, 21, 2, 0, 106, 7, 16, 12, 35, 46, 8);
  add_directory_record(ih->proot, 21, 2, 1, 106, 7, 16, 12, 35, 46, 8);

  ih->ppath = create_iso_path_table();
  add_path(ih->ppath, 21);

  ih->sroot = create_iso_dir();
  add_directory_record(ih->sroot, 24, 2, 0, 106, 7, 16, 12, 35, 46, 8);
  add_directory_record(ih->sroot, 24, 2, 1, 106, 7, 16, 12, 35, 46, 8);
...
void AddFile(IsoHeader *ih, char *name, guint64 size) {
  ...
  add_file_record(ih->proot, iso, size, 25, 106, 7, 16, 12, 28, 10, 8);
  sprintf(joliet, "%s;1", name);
  add_d1_file_record(ih->sroot, joliet, size, 25, 106, 7, 16, 12, 28, 10, 8);
...
void WriteIsoHeader(IsoHeader *ih, LargeFile *image) {
  ...
  for (i = 0; i < 16; i++) {
    ...
    if (n != 2048)
      Stop(_("Failed writing to sector %lld in image: %s"), (gint64)i,
           strerror(errno));
  }
  ...
  if (LargeWrite(image, pvd, 2048) != 2048)
    Stop(_("Failed writing to sector %lld in image: %s"), (gint64)16,
         strerror(errno));
  ...
  if (LargeWrite(image, svd, 2048) != 2048)
    Stop(_("Failed writing to sector %lld in image: %s"), (gint64)17,
         strerror(errno));
  ...
  if (LargeWrite(image, sector, 2048) != 2048)
    Stop(_("Failed writing to sector %lld in image: %s"), (gint64)18,
         strerror(errno));

  /*** Write the primary and supplementary path tables and root directories */

  if (LargeWrite(image, ih->ppath->lpath, 2048) != 2048)
    Stop(_("Failed writing to sector %lld in image: %s"), (gint64)19,
         strerror(errno));

  if (LargeWrite(image, ih->ppath->mpath, 2048) != 2048)
    Stop(_("Failed writing to sector %lld in image: %s"), (gint64)20,
         strerror(errno));

  if (LargeWrite(image, ih->proot->dir, 2048) != 2048)
    Stop(_("Failed writing to sector %lld in image: %s"), (gint64)21,
         strerror(errno));

  if (LargeWrite(image, ih->spath->lpath, 2048) != 2048)
    Stop(_("Failed writing to sector %lld in image: %s"), (gint64)22,
         strerror(errno));

  if (LargeWrite(image, ih->spath->mpath, 2048) != 2048)
    Stop(_("Failed writing to sector %lld in image: %s"), (gint64)23,
         strerror(errno));

  if (LargeWrite(image, ih->sroot->dir, 2048) != 2048)
    Stop(_("Failed writing to sector %lld in image: %s"), (gint64)24,
         strerror(errno));
}
