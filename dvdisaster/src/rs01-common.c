...
void RS01ResetCksums(Image *image) {
  RS01CksumClosure *csc =
      (RS01CksumClosure *)image->eccFileMethod->ckSumClosure;
...
char *RS01FinalizeCksums(Image *image) {
  ...
  if (good_fp)
    ...
    return g_strdup(
        _("All sectors successfully read, but wrong image checksum."));
}
 ...
 * - Missing sectors (beyond the range recorded in eh->sectors) will be padded
 *with zeros, since we need a multiple of ndata sectors for the parity
 *generation.
 * - Missing sectors beyond the range recorded in ii->sectors, but before the
 *real end as defined above are treated as "dead sectors".
 */
...
void RS01ScanImage(Method *method, Image *image, struct MD5Context *ecc_ctxt,
                   int mode) {
  RS01Widgets *wl = NULL;
  ...
  if (mode & PRINT_MODE)
    ...
    msg = _("Scanning image sectors: %3d%%");
  ...
  for (s = 0; s < image->sectorSize; s++) {
    ...
    err = CheckForMissingSector(buf, s,
                                image->fpState == 2 ? image->imageFP : NULL,
                                FINGERPRINT_SECTOR);
       ...
       */
        ...
        if (crcidx >= CRCBUFSIZE) {
          ...
          if (remain < CRCBUFSIZE)
            ...
            size = CRCBUFSIZE * sizeof(guint32);
    ...
    if (last_percent != percent) {
      ...
      if (Closure->guiMode && mode & PRINT_MODE) {
        RS01AddVerifyValues(method, percent, image->sectorsMissing,
                            image->crcErrors,
  ...
  /*** The image md5sum can only be calculated if all blocks have been
   * successfully read. */
