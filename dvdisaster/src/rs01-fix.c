 ...
 * - from Phil Karn's excellent Reed-Solomon library:
...
static void read_crc(LargeFile *ecc, guint32 *buf, int first_sector,
                     int n_sectors) {
  int n;

  if (!LargeSeek(ecc,
                 (gint64)(sizeof(EccHeader) + first_sector * sizeof(guint32))))
...
void RS01Fix(Image *image) {
  ...
  fc->msg = g_strdup_printf(
      _("Error correction file using Method RS01, %d roots, %4.1f%% "
        "redundancy."),
  ...
  if (image->sectorSize == image->expectedSectors &&
    ...
    if (Closure->guiMode) {
      int answer =
          ModalDialog(GTK_MESSAGE_QUESTION, GTK_BUTTONS_OK_CANCEL, NULL,
  ...
  if (image->sectorSize < image->expectedSectors) {
    ...
    answer = ModalWarning(GTK_MESSAGE_QUESTION, GTK_BUTTONS_OK_CANCEL, NULL,
                          _("Image file appears to be truncated.\n"
                            "Consider completing it with another reading pass "
                            "before going on.\n"));
    if (!answer) {
      SwitchAndSetFootline(fc->wl->fixNotebook, 1, fc->wl->fixFootline,
                           _("<span %s>Aborted by user request!</span>"),
  ...
  if (image->fpState != FP_PRESENT) {
    ...
    answer = ModalWarning(
        GTK_MESSAGE_QUESTION, GTK_BUTTONS_OK_CANCEL, NULL,
  ...
  rt = fc->rt =
      CreateReedSolomonTables(gt, RS_FIRST_ROOT, RS_PRIM_ELEM, eh->eccBytes);
  ...
  Our ecc blocks are built from ndata medium sectors spread over the full medium
  size. We read cache_size * ndata medium sectors ahead. */

  cache_size =
      2 * Closure->cacheMB; /* ndata medium sectors are approx. 0.5MB */
  ...
  for (si = 0; si < s; si++) {
    ...
    for (i = 0; i < ndata; i++) {
      ...
      if (block_idx[i] <
          image->expectedSectors) /* ignore the padding sectors! */
      {
    ...
    if (erasure_count > nroots) /* uncorrectable */
    {
      ...
      for (i = 0; i < erasure_count; i++) {
        ...
        if (!LargeSeek(image->file, (gint64)(2048 * idx)))
          Stop(_("Failed seeking to sector %lld in image [%s]: %s"), idx, "FD",
               strerror(errno));
        ...
        if (n != 2048)
          Stop(_("Failed writing to sector %lld in image [%s]: %s"), idx, "WD",
               strerror(errno));
      }
    } else /* try to correct them */
    {
      ...
      for (bi = 0; bi < 2048; bi++) {
        ...
        int lambda[nroots + 1],
            s[nroots]; /* Err+Eras Locator poly * and syndrome poly */
        ...
        if (!LargeSeek(image->eccFile,
                       (gint64)(sizeof(EccHeader) +
                                image->expectedSectors * sizeof(guint32) +
                                nroots * parity_block)))
        ...
        for (j = 1; j < GF_FIELDMAX; j++) {
          ...
          for (i = 0; i < nroots; i++) {
            if (s[i] == 0)
              ...
              s[i] =
                  data ^
                  gf_alpha_to[mod_fieldmax(gf_index_of[s[i]] +
          ...
          if (num1 != 0) {
            ...
            if (location >= 0 && location < ndata) {
              if (erasure_map[location] == 3) {
                ...
                int new = old ^ gf_alpha_to[mod_fieldmax(
                                    gf_index_of[num1] + gf_index_of[num2] +
                                    GF_FIELDMAX - gf_index_of[den])];

                PrintCLI(_("-> Error located in sector %lld at byte %4d (value "
                           "%02x '%c', expected %02x '%c')\n"),
                         block_idx[location], bi, old, isprint(old) ? old : '.',
                         new, isprint(new) ? new : '.');
              }
              ...
              fc->imgBlock[location][offset] ^= gf_alpha_to[mod_fieldmax(
                  gf_index_of[num1] + gf_index_of[num2] + GF_FIELDMAX -
                  gf_index_of[den])];
            } else
    ...
    if (erasure_count && erasure_count <= nroots) {
      ...
      for (i = 0; i < erasure_count; i++) {
        ...
        if (!LargeSeek(image->file, (gint64)(2048 * idx)))
          Stop(_("Failed seeking to sector %lld in image [%s]: %s"), idx, "FW",
               strerror(errno));

        if (idx < image->expectedSectors - 1)
          length = 2048;
        else
          length = eh->inLast;

        n = LargeWrite(image->file,
                       cache_offset + fc->imgBlock[erasure_list[i]], length);
        if (n != length)
  ...
  if (corrected > 0)
    PrintLog(_("Repaired sectors: %lld     \n"), corrected);
  if (uncorrected > 0) {
