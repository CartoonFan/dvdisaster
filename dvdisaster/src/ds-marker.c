...
void CreateMissingSector(unsigned char *out, guint64 sector,
  ...
  if (fingerprint)
    memcpy(buf + 0x1a0, fingerprint, 16);
  else
    memcpy(buf + 0x1b0, "none", 4);
...
void CreatePaddingSector(unsigned char *out, guint64 sector,
  ...
  g_sprintf(buf, "dvdisaster padding sector       "
...
static char *get_volume_label(unsigned char *buf) {
  if (!strcmp((char *)buf + 0x200, "Volume label (if any)")) {
    if (!strcmp((char *)buf + 0x220, "none"))
      ...
      return g_strdup((char *)buf + 0x220);
  }
...
int CheckForMissingSector(unsigned char *buf, guint64 sector,
  ...
  if (strncmp((char *)buf,
              "dvdisaster dead sector marker\n"
              "This sector could not be read from the image.\n"
              "Its contents have been substituted by the dvdisaster read "
              "routine.\n",
              143) ||
...
int CheckForMissingSectors(unsigned char *buf, guint64 sector,
                           unsigned char *fingerprint,
                           guint64 fingerprint_sector, int n_sectors,
                           guint64 *first_defect) {
  int i, result;

  for (i = 0; i < n_sectors; i++) {
    result =
        CheckForMissingSector(buf, sector, fingerprint, fingerprint_sector);
...
static void insert_buttons(GtkDialog *dialog) {
  gtk_dialog_add_buttons(dialog, _utf("Stop reporting these errors"), 1,
                         _utf("Continue reporting"), 0, NULL);
}

void ExplainMissingSector(unsigned char *buf, guint64 sector, int error,
                          int image) {
  int answer;
  guint64 recorded_number;
  ...
  if (!Closure->guiMode &&
      (error == SECTOR_MISSING_DISPLACED || error == SECTOR_MISSING_WRONG_FP)) {
    printf(
        "* This image/medium was probably mastered from defective source(s).\n"
  ...
  if (image == TRUE) {
    ...
    case SECTOR_MISSING_DISPLACED:
      answer = ModalDialog(
          GTK_MESSAGE_ERROR, GTK_BUTTONS_NONE, insert_buttons,
      ...
      if (answer)
        Closure->noMissingWarnings = TRUE;
      break;
