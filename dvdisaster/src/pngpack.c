...
int main(int argc, char *argv[]) {
  ...
  for (;;) {
    ...
    c = getopt_long(argc, argv, "hp:u:t::", long_options, &option_index);
    if (c == -1)
      break;
    ...
    case '?':
      ...
      break;
    default:
      fprintf(stdout, "?? illegal getopt return value %d\n", c);
      break;
    }
  }
  ...
  if (mode != MODE_NONE && mode != MODE_HELP)
    fprintf(stdout, "pngpack-0.21 *** Copyright 2005-2012 Carsten Gnoerlich.\n"
  ...
  case MODE_UNPACK: {
    ...
    for (i = 0; i < img_n; i++) {
      ...
      fprintf(stdout, "rendering %s (opcodes %d - %d)", pi->name,
              pi->first_opcode, pi->last_opcode);

      pi->bytesize = sizeof(unsigned int) * pi->width * pi->height;
      pi->image = malloc(pi->bytesize);
      if (!pi->image)
        Stop("out of memory for image");
  ...
  case MODE_HELP:
    fprintf(
        stdout,
        "Usage: pngpack [OPTION...] [FILE]...\n"
        "pngpack maintains archives of PNG images. Depending on the similarity "
        "between\n"
        "the images, very high compression ratios can be reached. It is "
        "especially\n"
        "suitable for distribution of screen shots for online "
        "documentation.\n\n"
        "Examples:\n"
