...
static void add_verify_values(Method *method, int percent, gint64 newMissing,
                              gint64 newCrcErrors) {
  ...
  if (newCrcErrors)
    sii->segColor = Closure->yellowSector;
  if (newMissing)
    sii->segColor = Closure->redSector;
...
static void redraw_spiral(RS03Widgets *wl) {
  ...
  DrawSpiralLabel(wl->cmpSpiral, wl->cmpLayout, _("Good sectors"),
                  Closure->greenSector, x, 1);

  DrawSpiralLabel(wl->cmpSpiral, wl->cmpLayout, _("Sectors with CRC errors"),
                  Closure->yellowSector, x, 2);

  DrawSpiralLabel(wl->cmpSpiral, wl->cmpLayout, _("Missing sectors"),
                  Closure->redSector, x, 3);
...
static gboolean expose_cb(GtkWidget *widget, GdkEventExpose *event,
  ...
  size = wl->cmpSpiral->diameter + 20 +
         3 * (10 + h); /* approx. size of spiral + labels */
...
void CreateRS03VerifyWindow(Method *self, GtkWidget *parent) {
  ...
  gtk_table_attach(GTK_TABLE(table), frame, 0, 1, 0, 1, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 5);
  ...
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 1, y1, y2, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 2);
  lab = wl->cmpEccType = gtk_label_new(NULL);
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 1, 2, y1, y2, GTK_EXPAND | GTK_FILL,
                   GTK_SHRINK, 0, 0);
  y1++;
  y2++;
  ...
  gtk_misc_set_alignment(GTK_MISC(lab), 0.0, 0.0);
  gtk_table_attach(GTK_TABLE(table2), lab, 0, 2, y1, y2, GTK_SHRINK | GTK_FILL,
                   GTK_SHRINK, 5, 4);

  /*** Image spiral */

  frame = gtk_frame_new(_utf("Image state"));
  gtk_table_attach(GTK_TABLE(table), frame, 1, 2, 0, 2, GTK_SHRINK | GTK_FILL,
                   GTK_EXPAND | GTK_FILL, 5, 5);

  wl->cmpSpiral = CreateSpiral(Closure->grid, Closure->background, 10, 5,
                               VERIFY_IMAGE_SEGMENTS);
  ...
  g_signal_connect(G_OBJECT(d_area), "expose_event", G_CALLBACK(expose_cb),
                   (gpointer)wl);

  /*** Image info */

  frame = gtk_frame_new(_utf("Data integrity"));
  gtk_table_attach(GTK_TABLE(table), frame, 0, 1, 1, 2, GTK_EXPAND | GTK_FILL,
                   GTK_EXPAND | GTK_FILL, 5, 5);
...
static void cleanup(gpointer data) {
  ...
  if (vc->gt)
    FreeGaloisTables(vc->gt);
  if (vc->rt)
    FreeReedSolomonTables(vc->rt);
...
static int prognosis(verify_closure *vc, gint64 missing, gint64 expected) {
  ...
  if (damaged_sectors > 0) {
    ...
    PrintLog(
        _("- erasure counts   :  avg =  %.1f; worst = %d per ecc block.\n"),
        (double)damaged_sectors / (double)damaged_eccsecs, worst_ecc);

    PrintLog(
        _("- prognosis        : %lld of %lld sectors recoverable (%d.%d%%)\n"),
    ...
    if (Closure->guiMode) {
      ...
      SetLabelText(GTK_LABEL(vc->wl->cmpImageErasureCnt),
                   ...
                   worst_ecc <= vc->lay->nroots ? Closure->greenMarkup
                                                : Closure->redMarkup,
                   (double)damaged_sectors / (double)damaged_eccsecs,
                   worst_ecc);
  ...
  if (damaged_sectors && worst_ecc <= vc->lay->nroots &&
...
static int check_syndromes(verify_closure *vc) {
    ...
    if (!vc->eccBlock[i]) /* out of memory */
    {
      ...
      if (Closure->guiMode)
        SetLabelText(
            ...
            Closure->redMarkup);
      PrintLog(_("* Ecc block test   : out of memory; try reducing sector "
                 "prefetch!\n"));
      return 0;
    }
  }
  ...
  vc->rt =
      CreateReedSolomonTables(vc->gt, RS_FIRST_ROOT, RS_PRIM_ELEM, lay->nroots);
  ...
  for (ecc_block = 0; ecc_block < lay->sectorsPerLayer; ecc_block++) {
    ...
    if (cache_idx == Closure->prefetchSectors) {
      ...
      for (layer = 0; layer < GF_FIELDMAX; layer++)
        if (layer < lay->ndata - 1)
          RS03ReadSectors(image, vc->lay, vc->eccBlock[layer], layer, ecc_block,
                          num_sectors, RS03_READ_DATA);
        else
          RS03ReadSectors(image, vc->lay, vc->eccBlock[layer], layer, ecc_block,
                          num_sectors, RS03_READ_CRC | RS03_READ_ECC);
    }
    ...
    if (percent != last_percent) {
      ...
      if (!ecc_bad) {
        if (Closure->guiMode)
          SetLabelText(GTK_LABEL(vc->wl->cmpEccSyndromes), _("%d%% tested"),
                       percent);
        ...
        PrintProgress(
            _("* Ecc block test   : %lld good, %lld bad; %d%% tested"),
  ...
  if (!ecc_bad) {
    ...
    PrintLog(
        _("* Ecc block test   : %lld good, %lld bad; %lld bad sub blocks\n"),
...
void RS03Verify(Image *image) {
  ...
  if (eh->methodFlags[0] & MFLAG_ECC_FILE) {
    ...
    if (Closure->guiMode)
      SetLabelText(GTK_LABEL(wl->cmpHeadline), "<big>%s</big>\n<i>%s</i>",
                   _("Checking the image file."), _("- Checking image file -"));
  }
  ...
  if (eh->methodFlags[0] & MFLAG_ECC_FILE)
    lay = vc->lay = CalcRS03Layout(eh_sectors, eh, ECC_FILE);
  else
    lay = vc->lay = CalcRS03Layout(eh_sectors, eh, ECC_IMAGE);
  ...
  if (lay->target == ECC_FILE) {
    ...
    if (expected_eccfile_sectors != eccfile_sectors) {
      ...
      if (expected_eccfile_sectors > eccfile_sectors)
        msg = g_strdup_printf(
            _("Ecc file is %lld sectors shorter than expected."),
            expected_eccfile_sectors - eccfile_sectors);
      else
        msg =
            g_strdup_printf(_("Ecc file is %lld sectors longer than expected."),
                            eccfile_sectors - expected_eccfile_sectors);

      if (Closure->guiMode)
        ecc_advice =
            g_strdup_printf("<span %s>%s</span>", Closure->redMarkup, msg);
  ...
  if (eh->methodFlags[0] & MFLAG_ECC_FILE)
    ...
    PrintLog(_("- type             : Augmented image\n"));

  if (Closure->guiMode) {
    if (eh->methodFlags[0] & MFLAG_ECC_FILE)
      SetLabelText(GTK_LABEL(wl->cmpEccType), _("Error correction file"));
    else
      SetLabelText(GTK_LABEL(wl->cmpEccType), _("Augmented image"));
  }
  ...
  if (Closure->guiMode)
    SetLabelText(GTK_LABEL(wl->cmpEccMethod),
                 _("%4s, %d roots, %4.1f%% redundancy"), method, eh->eccBytes,
                 ((double)eh->eccBytes * 100.0) / (double)eh->dataBytes);
  ...
  if (eh->creatorVersion % 100) {
    ...
    if (Closure->guiMode) {
      if (!color_format)
        SetLabelText(GTK_LABEL(wl->cmpEccCreatedBy), color_format, "dvdisaster",
                     major, minor, Closure->redMarkup, pl);
      else
        SetLabelText(GTK_LABEL(wl->cmpEccCreatedBy), format, "dvdisaster",
                     major, minor, pl);
    }
  } else {
    ...
    if (Closure->guiMode)
      SetLabelText(GTK_LABEL(wl->cmpEccCreatedBy), "dvdisaster-%d.%d", major,
                   minor);
  }
  ...
  if (eh->neededVersion % 100)
    version = g_strdup_printf("%d.%d (pl%d)", eh->neededVersion / 10000,
                              (eh->neededVersion % 10000) / 100,
                              eh->neededVersion % 100);
  else
    version = g_strdup_printf("%d.%d", eh->neededVersion / 10000,
                              (eh->neededVersion % 10000) / 100);
  ...
  if (!VerifyVersion(eh, 0)) {
    ...
    PrintLog(_("* requires         : dvdisaster-%s (BAD)\n"
               ...
               "*                  : Please visit http://www.dvdisaster.org "
               "for an upgrade.\n"),
             version);

    if (Closure->guiMode) {
      SetLabelText(GTK_LABEL(wl->cmpEccRequires),
                   "<span %s>dvdisaster-%s</span>", Closure->redMarkup,
                   version);
      if (!ecc_advice)
        ecc_advice = g_strdup_printf(
            _("<span %s>Please upgrade your version of dvdisaster!</span>"),
            Closure->redMarkup);
    }
  ...
  if (eh->methodFlags[0] & MFLAG_DATA_MD5)
    ...
    strcpy(hdr_digest, _("none available"));
  ...
  if (lay->target == ECC_FILE) { // expected_sectors = lay->dataSectors +
    // lay->totalSectors;  /* image + ecc file */
    virtual_expected =
        GF_FIELDMAX * lay->sectorsPerLayer; /* for prognosis map */
    expected_image_sectors =
        lay->dataSectors; /* just the expected image size */
  ...
  if (expected_image_sectors == image->sectorSize && matching_byte_size) {
    if (lay->target == ECC_FILE) {
      if (Closure->guiMode) {
        if (image->inLast == 2048)
          SetLabelText(GTK_LABEL(wl->cmpImageSectors),
                       _("%lld in image; %lld in ecc file"), image->sectorSize,
                       eccfile_sectors);
        else
          SetLabelText(GTK_LABEL(wl->cmpImageSectors),
                       _("%lld sectors + %d bytes in image; %lld in ecc file"),
      ...
      if (image->inLast == 2048)
        ...
        PrintLog(_("- sectors          : %lld sectors + %d bytes in image; "),
      ...
      if (Closure->guiMode)
        SetLabelText(GTK_LABEL(wl->cmpImageSectors),
                     _("%lld total / %lld data"), image->sectorSize,
                     lay->dataSectors);
    ...
    if (Closure->guiMode) {
      if (matching_byte_size)
        SetLabelText(GTK_LABEL(wl->cmpImageSectors),
                     _("<span %s>%lld (%lld expected)</span>"),
        ...
        if (image->inLast < eh->inLast) {
          ...
          img_advice =
              g_strdup_printf(_("<span %s>Image file is %d bytes shorter than "
                                "expected.</span>"),
          ...
          img_advice = g_strdup_printf(
              _("<span %s>Image file is %d bytes longer than expected.</span>"),
      ...
      if (expected_image_sectors > image->sectorSize)
        img_advice = g_strdup_printf(
            _("<span %s>Image file is %lld sectors shorter than "
              "expected.</span>"),
            Closure->redMarkup, expected_image_sectors - image->sectorSize);
      if (expected_image_sectors < image->sectorSize)
        img_advice = g_strdup_printf(
            _("<span %s>Image file is %lld sectors longer than "
              "expected.</span>"),
    ...
    if (matching_byte_size)
      ...
      PrintLog(_("* sectors          : %lld sectors + %d of %d bytes in image; "
                 "%lld sectors in ecc file\n"),
               image->sectorSize - 1, image->inLast, eh->inLast,
               eccfile_sectors);
  }
  ...
  for (s = 0; s < virtual_expected; s++) {
    ...
    if (lay->target == ECC_IMAGE ||
        s < lay->dataSectors) {  /* Read from image file */
      if (s < image->sectorSize) /* image may be truncated */
      {
        int n;
        int expected_read = 2048;

        /* If ecc file and image agree on expected image size,
           use the smaller byte size of them. */

        if (s == image->sectorSize - 1 && image->sectorSize == eh_sectors) {
          memset(buf, 0, 2048);

          if (image->inLast < eh->inLast)
            expected_read = image->inLast;
          else
            expected_read = eh->inLast;

          if (!expected_read)
            expected_read = 2048;
        } else {
          /* Last sector in a truncated image may be incomplete */

          if (s == image->sectorSize - 1) {
            memset(buf, 0, 2048);
          ...
          /* Ecc file says last sector is partially filled */

          if (s == eh_sectors - 1) /* last sector may be incomplete */
          {
            memset(buf, 0, 2048);
            expected_read = eh->inLast;
            if (!expected_read)
        ...
        CreateMissingSector(buf, s, eh->mediumFP, eh->fpSector,
                            "padding beyond the image");
      }
    } else /* lay->target == ECC_FILE && s > lay->dataSectors */
    {      /* Simulate the non-existent padding area in ecc files */
      if (s >= lay->dataSectors &&
          s < (lay->ndata - 1) * lay->sectorsPerLayer) {
        memset(buf, 0, 2048);
      }

      /* Read from ecc file */
      else if (s <
               (lay->ndata - 1) * lay->sectorsPerLayer + eccfile_sectors - 2) {
        int n = LargeRead(image->eccFile, buf, 2048);
        if (n != 2048) {
          exitCode = EXIT_CODE_UNEXPECTED_EOF;
    ...
    if (s < lay->dataSectors) {
      if (s < lay->dataSectors - 1)
        ...
        MD5Update(&image_md5, buf, eh->inLast);
    }
    ...
    if (current_missing != SECTOR_PRESENT && !missing_sector_explained) {
      missing_sector_explained = 1;
    ...
    if (current_missing) {
      ...
      if (lay->target == ECC_IMAGE) {
        if (s < lay->firstCrcPos)
          data_missing++;
        else if (s >= lay->firstCrcPos && s < lay->firstEccPos)
          crc_missing++;
        else
          ecc_missing++;
      } else /* ecc file case */
      {
        if (s < lay->dataSectors)
          data_missing++;
        else if (s < lay->ndata * lay->sectorsPerLayer)
          crc_missing++;
        else
          ecc_missing++;
      }
    ...
    if (!current_missing || s == virtual_expected - 1) {
      if (first_missing >= 0) {
        ...
        if (lay->target == ECC_FILE &&
            last_missing >= (lay->ndata - 1) * lay->sectorsPerLayer) {
        ...
        if (first_missing == last_missing)
          ...
          PrintCLI(_("* missing sectors  : %lld - %lld%s\n"), first, last,
                   ecc_msg);
        first_missing = -1;
        g_free(ecc_msg);
      }
    ...
    if (Closure->guiMode) { /* data part / spiral animation */
      ...
      if (lay->target == ECC_FILE && s >= lay->dataSectors)
        percent = (100 * (s + 1 - lay->dataSectors) /
                   (virtual_expected - lay->dataSectors));
    } else
    ...
    if (last_percent != percent) /* Update sector results */
    {
      ...
      if (Closure->guiMode) {
        ...
        } else /* do not include ecc file sectors in the spiral! */
        {
          if (s < lay->dataSectors) {
            int image_percent =
                (VERIFY_IMAGE_SEGMENTS * (s + 1)) / lay->dataSectors;

            add_verify_values(self, image_percent, new_missing, new_crc_errors);
          } else {
            SetLabelText(GTK_LABEL(wl->cmpEccSyndromes), "%d%% tested",
                         percent);
          }
        }
    ...
    if (lay->target == ECC_FILE && s == lay->dataSectors - 1) {
      if (Closure->guiMode) { /* flush/complete spiral */
        add_verify_values(self, VERIFY_IMAGE_SEGMENTS, new_missing,
                          new_crc_errors);
  ...
  if (!total_missing && !data_crc_errors && !csc->signatureErrors)
    ...
    if (!total_missing)
      PrintLog(
          _("* suspicious image : all sectors present, but %lld CRC errors\n"),
          data_crc_errors);
    if (total_missing && data_crc_errors)
      PrintLog(
          _("* BAD image        : %lld sectors missing, %lld CRC errors\n"),
          total_missing, data_crc_errors);

    PrintLog(
        _("  ... data section   : %lld sectors missing; %lld CRC errors\n"),
        data_missing, data_crc_errors);
    if (!total_missing && !data_crc_errors && !csc->signatureErrors)
      PrintLog(_("  ... data md5sum    : %s\n"), data_digest);

    if (csc->signatureErrors)
      PrintLog(_("  ... crc section    : %lld sectors missing; %lld signature "
                 "errors\n"),
  ...
  if (Closure->guiMode) {
    ...
    if (!ecc_missing)
      SetLabelText(GTK_LABEL(wl->cmpEccSection), _("complete"));

    SetLabelText(GTK_LABEL(wl->cmpImageMd5Sum), "%s",
                 data_missing ? "-" : data_digest);
  }
  ...
  if (0 && total_missing + data_crc_errors != 0) {
    if (Closure->guiMode)
      SetLabelText(
          GTK_LABEL(wl->cmpEccSyndromes),
          _("<span %s>Skipped; not useful on known defective image</span>"),
          Closure->redMarkup);

    PrintLog(
        _("* Ecc block test   : skipped; not useful on defective image\n"));
  } else
    ...
    if (img_advice) {
      ...
      if (!total_missing && !data_crc_errors && !syn_error)
        SetLabelText(GTK_LABEL(wl->cmpImageErasure), /* avoid two blank lines */
                     _("<span %s>Good image.</span>"), Closure->greenMarkup);
      else
        SetLabelText(GTK_LABEL(wl->cmpImageResult),
                     _("<span %s>Damaged image.</span>"), Closure->redMarkup);
    }
  }
