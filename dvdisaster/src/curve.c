...
Curve *CreateCurve(GtkWidget *widget, char *left_label, char *left_format,
                   int n_values, int bottom_format) {
...
int CurveY(Curve *curve, gdouble y) {
  ...
  hfact = (gdouble)(curve->bottomY - curve->topY) / (gdouble)curve->maxY;
...
int CurveLogY(Curve *curve, gdouble y) /* not really a log */
{
  ...
  if (y < 1)
    return curve->bottomLY;

  hfact = (gdouble)(curve->bottomLY - curve->topLY);

  if (y == 1)
    return curve->bottomLY - ((log(2) / log(curve->logMaxY)) * hfact) / 2;
...
void UpdateCurveGeometry(Curve *curve, char *largest_left_label,
                         int right_padding) {
...
void RedrawAxes(Curve *curve) {
  ...
  gdk_draw_line(d, Closure->drawGC, curve->leftX, curve->topY, curve->leftX,
                curve->bottomY);

  if (curve->enable & DRAW_LCURVE) {
    gdk_draw_line(d, Closure->drawGC, curve->leftX, curve->topLY, curve->leftX,
                  curve->bottomLY);
  }
  ...
  if (x < 5)
    x = 5;
  gdk_draw_layout(d, Closure->drawGC, x, curve->topY - h - 5, curve->layout);
  ...
  if (curve->enable & DRAW_LCURVE) {
    ...
    if (x < 5)
      x = 5;
    gdk_draw_layout(d, Closure->drawGC, x, curve->topLY - h - 5, curve->layout);
    ...
    for (val = 400; val > 3; val /= 2) {
      ...
      gdk_gc_set_rgb_fg_color(Closure->drawGC, Closure->logColor);
      gdk_draw_layout(d, Closure->drawGC, curve->leftX - 9 - w, y - h / 2,
                      curve->layout);
      gdk_gc_set_rgb_fg_color(Closure->drawGC, Closure->foreground);
  ...
  gdk_draw_line(d, Closure->drawGC, curve->rightX, curve->topY, curve->rightX,
                curve->bottomY);

  if (curve->enable & DRAW_LCURVE)
    gdk_draw_line(d, Closure->drawGC, curve->rightX, curve->topLY,
                  curve->rightX, curve->bottomLY);

  /* Draw and label the bottom coordinate axis */

  gdk_gc_set_rgb_fg_color(Closure->drawGC, Closure->foreground);

  gdk_draw_line(d, Closure->drawGC, curve->leftX, curve->bottomY, curve->rightX,
                curve->bottomY);

  if (curve->enable & DRAW_LCURVE) {
    gdk_draw_line(d, Closure->drawGC, curve->leftX, curve->bottomLY,
                  curve->rightX, curve->bottomLY);
    bottom_y = curve->bottomLY;
  } else
    bottom_y = curve->bottomY;

  if (curve->maxX <= 100)
    step = 20; /* <100M */
  else if (curve->maxX < 1000)
    step = 100; /* 100M ... 1000M */
  else if (curve->maxX < 8000)
    step = 1024; /* 1G .. 8G */
  else if (curve->maxX < 15000)
    step = 2560; /* 8G .. 15G */
  else if (curve->maxX < 25000)
    step = 5120; /* 15G .. 25G */
  else
    step = 10240;
  ...
  for (i = 0; i <= curve->maxX; i += step) {
    ...
    case CURVE_MEGABYTES:
      if (step <= 100)
        ...
        g_snprintf(buf, 10, "%3.1fG", (gdouble)i / 1024.0);
      break;
    }
    SetText(curve->layout, buf, &w, &h);
    ...
    if (i && x < curve->rightX) {
      ...
      if (curve->enable & DRAW_LCURVE)
        gdk_draw_line(d, Closure->drawGC, x, curve->bottomLY - 1, x,
                      curve->topLY);
    }
  ...
  for (i = 1; i <= last; i++) {
    ...
    if (curve->enable & DRAW_ICURVE) {
      ...
      if (curve->ivalue[i] > 0) {
        ...
        gdk_draw_rectangle(curve->widget->window, Closure->drawGC, TRUE, x0, iy,
                           x0 == x1 ? 1 : x1 - x0, curve->bottomY - iy);
      }
    }
    ...
    if (curve->enable & DRAW_LCURVE) {
      ...
      if (curve->lvalue[i] > 0) {
        gdk_gc_set_rgb_fg_color(Closure->drawGC, Closure->logColor);
        gdk_draw_rectangle(curve->widget->window, Closure->drawGC, TRUE, x0, iy,
                           x0 == x1 ? 1 : x1 - x0, curve->bottomLY - iy);
      }
    }
