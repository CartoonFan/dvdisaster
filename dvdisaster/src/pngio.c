...
void FreeImage(Image *i) {
  ...
  if (i->row_pointers)
    free(i->row_pointers);
  if (i->png_read)
    png_destroy_read_struct(&i->png_read, &i->png_info, NULL);
  if (i->png_write)
    png_destroy_write_struct(&i->png_write, &i->png_info);
...
Image *LoadPNG(char *name) {
  ...
  fprintf(stdout, "%dx%d %s image", pi->width, pi->height,
          pi->channels == 3 ? "RBG" : "RBGA");
  ...
  if (png_get_bKGD(pi->png_read, pi->png_info, &background))
    pi->png_background =
        (background->red << 16) | (background->green << 8) | background->blue;
  else
    pi->png_background = 0;
...
void SavePNG(Image *pi, char *name) {
  ...
  png_set_IHDR(pi->png_write, pi->png_info, pi->width, pi->height, 8,
               pi->channels == 4 ? PNG_COLOR_TYPE_RGB_ALPHA
                                 : PNG_COLOR_TYPE_RGB,
