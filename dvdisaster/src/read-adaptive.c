...
static void cleanup(gpointer data) {
  ...
  if (rc->readMode == ECC_IN_IMAGE) {
    ...
    if (rc->highestWrittenSector + 1 <
        end) /* image may have been partially read */
...
bail_out:
  if (Closure->guiMode) {
    if (rc->earlyTermination)
      SetAdaptiveReadFootline(_("Aborted by unrecoverable error."),
                              Closure->redText);
  ...
  if (rc->subtitle)
    g_free(rc->subtitle);
  if (rc->segmentState)
    g_free(rc->segmentState);

  if (rc->ab)
    FreeAlignedBuffer(rc->ab);

  if (rc->intervals)
    g_free(rc->intervals);
  if (rc->crcBuf)
    FreeCrcBuf(rc->crcBuf);

  if (rc->fingerprint)
    g_free(rc->fingerprint);
  if (rc->volumeLabel)
    g_free(rc->volumeLabel);
...
static void add_interval(read_closure *rc, gint64 start, gint64 size) {
  ...
  if (rc->nIntervals > rc->maxIntervals) {
    ...
    rc->intervals =
        g_realloc(rc->intervals, rc->maxIntervals * 2 * sizeof(gint64));
  }

  /* Shift unless we insert at the list tail */

  if (i < rc->nIntervals - 1)
    memmove(rc->intervals + si + 2, rc->intervals + si,
            2 * sizeof(gint64) * (int)(rc->nIntervals - i - 1));
...
static void pop_interval(read_closure *rc) {
  if (rc->nIntervals > 0) {
    rc->nIntervals--;
    memmove(rc->intervals, rc->intervals + 2,
            2 * sizeof(gint64) * (int)rc->nIntervals);
  }
}
...
static void print_progress(read_closure *rc, int immediate) {
  ...
  if (rc->lastPercent >= percent && rc->lastCorrectable == rc->correctable &&
      rc->lastUnreadable == rc->unreadable && !immediate)
    return;
  ...
  if (rc->ei)
    n = g_snprintf(rc->progressMsg, 256,
                   _("Repairable: %2d.%1d%% (correctable: %lld; now reading "
                     "[%lld..%lld], size %lld)"),
    ...
    n = g_snprintf(rc->progressMsg, 256,
                   _("Repairable: %2d.%1d%% (missing: %lld; now reading "
                     "[%lld..%lld], size %lld)"),
...
static void mark_sector(read_closure *rc, gint64 sector, GdkColor *color) {
  ...
  if (color) {
    ...
    if (color == Closure->yellowSector && old != Closure->redSector &&
  ...
  if (changed)
    UpdateAdaptiveResults(
        rc->readable, rc->correctable,
...
static void open_and_determine_mode(read_closure *rc) {
  ...
  /* See if we have ecc information available.
     Prefer the error correction file over augmented images if both are
     available. */
  ...
  if (rc->ei) /* RS01 type ecc */
  {
    ...
    rc->rs01LayerSectors =
        (rc->ei->sectors + rc->eh->dataBytes - 1) / rc->eh->dataBytes;

    SetAdaptiveReadMinimumPercentage(
        (1000 * (rc->eh->dataBytes - rc->eh->eccBytes)) / rc->eh->dataBytes);
  } else /* see if we have RS02 type ecc */
      if (rc->medium->eccHeader &&
          !strncmp((char *)rc->medium->eccHeader->method, "RS02", 4)) {
    rc->readMode = ECC_IN_IMAGE;
    rc->eh = rc->medium->eccHeader;
    rc->lay =
        CalcRS02Layout(uchar_to_gint64(rc->eh->sectors), rc->eh->eccBytes);
    ...
    if (Closure->verbose) /* for testing purposes */
    {
      ...
      for (s = 0; s < rc->dh->sectors - 1; s++) {
        ...
        if (slice == -1)
          ...
          Verbose("Failed for sector %lld / %lld:\n"
                  "slice %lld, idx %lld\n",
                  s, sinv, slice, idx);
      }
  ...
  switch (rc->readMode) {
  case IMAGE_ONLY:
    rc->subtitle = g_strdup_printf(
        _("Stopping when unreadable intervals < %d."), Closure->sectorSkip);
  ...
  case ECC_IN_IMAGE:
    rc->subtitle =
        g_strdup(_("Trying to collect enough data for error correction."));
    PrintLog(_("Adaptive reading: %s\n"), rc->subtitle);
    break;
  }
}
...
/*
 * Compare medium fingerprint against fingerprint stored in error correction
 * file
 */
...
int check_image_fingerprint(read_closure *rc) {
  ...
  if (rc->eh) /* If ecc information is present get fp sector number from there
    ...
    fingerprint_sector = FINGERPRINT_SECTOR;
  ...
  if (n != 2048 || !fp_read ||
      (CheckForMissingSector(rc->buf, fingerprint_sector, NULL, 0) !=
       SECTOR_PRESENT))
    return 0; /* can't tell, assume okay */
  ...
  if (memcmp(image_fp, medium_fp, 16)) {
    if (!Closure->guiMode)
      ...
      if (!answer) {
        rc->earlyTermination = FALSE;
        SetAdaptiveReadFootline(
            _("Reading aborted. Please select a different image file."),
            Closure->redText);
        cleanup((gpointer)rc);
      } else {
        LargeClose(rc->image);
...
void check_image_size(read_closure *rc, gint64 image_file_sectors) {
  if (image_file_sectors > rc->sectors) {
    ...
    rc->highestWrittenSector = image_file_sectors - 1;
}
...
static void build_interval_from_image(read_closure *rc) {
  ...
  for (s = 0; s <= rc->highestWrittenSector; s++) {
    ...
    if ((!current_missing || s >= rc->highestWrittenSector) &&
        first_missing >= 0) {
      if (s >=
          rc->highestWrittenSector) /* special case: interval end = image end */
      {
  ...
  if (rc->readMode == ECC_IN_FILE || rc->readMode == ECC_IN_IMAGE)
    PrintLog(_("Analysing existing image file: %lld readable, %lld "
               "correctable, %lld still missing.\n"),
             rc->readable, rc->correctable,
             rc->sectors - rc->readable - rc->correctable);
  else
    PrintLog(_("Analysing existing image file: %lld readable, %lld still "
               "missing.\n"),
...
static void mark_rs02_headers(read_closure *rc) {
  ...
  hpos = (rc->lay->protectedSectors + rc->lay->headerModulo - 1) /
         rc->lay->headerModulo;
...
static void insert_buttons(GtkDialog *dialog) {
  gtk_dialog_add_buttons(dialog, _utf("Ignore once"), 1, _utf("Ignore always"),
                         2, _utf("Abort"), 0, NULL);
}

/*
 * Fill the gap between rc->intervalStart and rc->highestWrittenSector
 * with dead sector markers. These are needed in case the user aborts the
 * operation; otherwise the gap will just be zero-filled and we don't know its
 * contents are unprocessed if we try to re-read the image later. Also, this
 * prevents fragmentation under most filesystems.
 */
...
void fill_gap(read_closure *rc) {
  ...
  t = g_strdup_printf(_("Filling image area [%lld..%lld]"), firstUnwritten,
                      rc->intervalStart - 1);
  clear_progress(rc);
  if (Closure->guiMode) {
  ...
  if (!LargeSeek(rc->image, (gint64)(2048 * firstUnwritten)))
    Stop(_("Failed seeking to sector %lld in image [%s]: %s"), firstUnwritten,
         "fill", strerror(errno));
  ...
  for (i = firstUnwritten, j = 0; i < rc->intervalStart; i++) {
    ...
    CreateMissingSector(buf, i, rc->fingerprint, FINGERPRINT_SECTOR,
                        rc->volumeLabel);
    n = LargeWrite(rc->image, buf, 2048);
    if (n != 2048)
      Stop(_("Failed writing to sector %lld in image [%s]: %s"), i, "fill",
           strerror(errno));
    ...
    if (Closure->stopActions) {
      if (Closure->guiMode)
        SetAdaptiveReadFootline(_("Aborted by user request!"),
                                Closure->redText);
    ...
    if (Closure->guiMode) {
      ...
      if (Closure->readAdaptiveSpiral->segmentColor[segment] ==
          Closure->background)
...
void fill_correctable_gap(read_closure *rc, gint64 correctable) {
  if (correctable > rc->highestWrittenSector) {
    ...
    if (!LargeSeek(rc->image, (gint64)(2048 * ds)))
      Stop(_("Failed seeking to sector %lld in image [%s]: %s"), ds,
           "skip-corr", strerror(errno));

    for (ds = rc->highestWrittenSector + 1; ds <= correctable; ds++) {
      CreateMissingSector(buf, ds, rc->fingerprint, FINGERPRINT_SECTOR,
                          rc->volumeLabel);
      if (LargeWrite(rc->image, buf, 2048) != 2048)
        Stop(_("Failed writing to sector %lld in image [%s]: %s"), ds,
             "skip-corr", strerror(errno));
    }
...
void ReadMediumAdaptive(gpointer data) {
  ...
  if (Closure->guiMode) { // rc->sectorsPerSegment = 1 + (rc->sectors /
    // ADAPTIVE_READ_SPIRAL_SIZE);
    rc->sectorsPerSegment = ((rc->sectors + ADAPTIVE_READ_SPIRAL_SIZE - 1) /
                             ADAPTIVE_READ_SPIRAL_SIZE);
    rc->segmentState = g_malloc0(ADAPTIVE_READ_SPIRAL_SIZE * sizeof(int));
    //      ClipReadAdaptiveSpiral(rc->sectors/rc->sectorsPerSegment);
    ClipReadAdaptiveSpiral((rc->sectors + rc->sectorsPerSegment - 1) /
                           rc->sectorsPerSegment);
  }
...
reopen_image:
  if (!LargeStat(Closure->imageName, &image_file_size)) {
    if (!(rc->image =
              LargeOpen(Closure->imageName, O_RDWR | O_CREAT, IMG_PERMS)))
      Stop(_("Can't open %s:\n%s"), Closure->imageName, strerror(errno));

    PrintLog(_("Creating new %s image.\n"), Closure->imageName);
    if (Closure->guiMode)
      SetLabelText(GTK_LABEL(Closure->readAdaptiveHeadline),
                   "<big>%s</big>\n<i>%s</i>", _("Reading new medium image."),
                   rc->dh->mediumDescr);
  ...
  /*** else examine the existing image file ***/
    ...
    if (Closure->guiMode)
      SetLabelText(GTK_LABEL(Closure->readAdaptiveHeadline),
                   "<big>%s</big>\n<i>%s</i>",
                   _("Completing existing medium image."), rc->dh->mediumDescr);
  ...
  for (;;) {
    ...
    for (s = rc->intervalStart;
         s <= rc->intervalEnd;) /* s is incremented elsewhere */
    {
      int nsectors, cnt;
      ...
      if (Closure->guiMode)
        ChangeSpiralCursor(Closure->readAdaptiveSpiral,
                           s / rc->sectorsPerSegment);

      /* Determine number of sectors to read. Read the next dh->clusterSize
         sectors unless we're at the end of the interval or at a position which
         is not divideable by the cluster size. */

      if (s & cluster_mask)
        nsectors = 1;
      else
        nsectors = rc->dh->clusterSize;

      if (s + nsectors > rc->intervalEnd)
        nsectors = rc->intervalEnd - s + 1;
    ...
    reread:
      ...
      if (status && !Closure->ignoreFatalSense && rc->dh->sense.sense_key &&
        ...
        answer =
            ModalDialog(GTK_MESSAGE_QUESTION, GTK_BUTTONS_NONE, insert_buttons,
                        _("Sector %lld: %s\n\n"
      ...
      if (!status) {
        ...
        if (!LargeSeek(rc->image, (gint64)(2048 * s)))
          Stop(_("Failed seeking to sector %lld in image [%s]: %s"), s, "store",
               strerror(errno));
        ...
        for (i = 0, b = s; i < nsectors; i++, b++) {
          ...
          switch (result) {
          case CRC_BAD: { // unsigned char buf[2048];
            ...
            if (n != 2048)
              Stop(_("Failed writing to sector %lld in image [%s]: %s"), b,
                   "unv", strerror(errno));
            ...
            if (rc->eh->dataBytes - present <= rc->eh->eccBytes) {
              ...
              for (j = 0; j < rc->eh->dataBytes; j++) {
                if (layer_idx < rc->ei->sectors /* skip padding sector */
                  ...
                  /* If the correctable sector lies beyond the highest written
                     sector, fill the gap with dead sector markers */
      ...
      else /* Process the read error. */
      {
        ...
        if (nsectors > 1)
          ...
          PrintCLIorLabel(Closure->status, _("Sector %lld: %s\n"), s,
        ...
        if (!LargeSeek(rc->image, (gint64)(2048 * s)))
          Stop(_("Failed seeking to sector %lld in image [%s]: %s"), s, "nds",
               strerror(errno));

        for (i = 0; i < nsectors; i++) {
          CreateMissingSector(buf, s + i, rc->fingerprint, FINGERPRINT_SECTOR,
                              rc->volumeLabel);

          n = LargeWrite(rc->image, buf, 2048);
          if (n != 2048)
            Stop(_("Failed writing to sector %lld in image [%s]: %s"), s, "nds",
                 strerror(errno));
        ...
        if (s + nsectors - 1 >=
            rc->intervalEnd) /* This was the last sector; interval used up */
        {
    ...
    or the loop was terminated early by a read error.
           In both cases, the current interval has already been remove from the
    queue and the queue contains only the still unprocessed intervals. */

    if (s >= rc->intervalEnd) /* we fell out of the reading loop with interval
             completed */
    {
      print_progress(rc, TRUE);
      PrintCLI("\n");
    ...
    if (rc->intervalSize > 1) {
      Verbose("*** Splitting [%lld..%lld]\n", rc->intervalStart,
              rc->intervalStart + rc->intervalSize - 1);
...
#ifdef CHECK_VISITED
{
  ...
  if (rc->map && (rc->readable + rc->correctable < rc->sectors)) {
    ...
    char *t = g_strdup_printf(
        _("Only %2d.%1d%% of the image are readable or correctable"),
  ...
  if (rc->readMode == IMAGE_ONLY) {
    if (rc->readable == rc->sectors) {
      ...
      char *t =
          g_strdup_printf(_("No unreadable intervals with >= %d sectors left."),
                          Closure->sectorSkip);
