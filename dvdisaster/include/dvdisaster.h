...
#define DVD_SL_SIZE                                                            \
  2295104                   /* DVD+R/RW size used at least common denominator  \
                             */
...
#define MAX_CDR_SIZE                                                           \
  (100 * 60 * 75)               /* CDs can't have >100min w/o severe hacks  */
...
typedef struct _GlobalClosure {
  ...
  int speedWarning;    /* Print warning if speed changes by more than given
                percentage */
  ...
  int simulateDefects;   /* if >0, this is the percentage of simulated media
                  defects */
  ...
  void (*cleanupProc)(gpointer); /* Procedure to cleanup running threads after
                          an error condition */
  ...
  gint64 crcErrors, readErrors; /* these are passed between threads and must
                         therefore be global */
...
#define FINGERPRINT_SECTOR                                                     \
  16 /* Sector currently used to calculate the fingerprint. */
...
typedef struct _EccHeader {
  ...
  gint8 methodFlags[4];  /* 0-2 for free use by the respective methods; 3 see
                  above */
...
typedef struct _CrcBlock {
  ...
  guint8 mediumSum[16];  /* complete md5sum of whole medium */
  aligned_guint64
      dataSectors;  /* number of sectors of the payload (e.g. iso file sys) */
  gint32 inLast;    /* bytes contained in last sector */
  gint32 dataBytes; /* data bytes per ecc block */
  gint32 eccBytes;  /* ecc bytes per ecc block */
...
enum { CRC_UNKNOWN, CRC_BAD, CRC_GOOD };
...
enum { CURVE_PERCENT, CURVE_MEGABYTES };
...
void CreateMissingSector(unsigned char *, guint64, unsigned char *, guint64,
                         char *);
int CheckForMissingSector(unsigned char *, guint64, unsigned char *, guint64);
int CheckForMissingSectors(unsigned char *, guint64, unsigned char *, guint64,
                           int, guint64 *);
...
GtkWidget *ShowTextfile(char *, char *, char *, GtkScrolledWindow **,
                        GtkTextBuffer **);
void ShowGPL();
void ShowLog();
void UpdateLog();
...
typedef struct _Image {
  ...
  guint64 expectedSectors; /* Image size according to codec (including augmented
                    parts) */
...
gchar *g_locale_to_utf8_ext(const gchar *, gssize, gsize *, gsize *, GError **,
                            char *, int);
...
typedef struct _Method {
  ...
  int (*recognizeEccFile)(
      LargeFile *,
      EccHeader **); /* checks whether we can handle this ecc file */
  int (*recognizeEccImage)(
      Image *); /* checks whether we can handle this augmented image */
  guint64 (*expectedImageSize)(
      EccHeader *); /* calculates expected image size (incl. augmented data) */
  CrcBuf *(*getCrcBuf)(Image *); /* read and cache CRC info from ecc data */
  void (*resetCksums)(Image *);  /* Methods for building internal */
  void (*updateCksums)(Image *, gint64,
                       unsigned char *); /* checksum while reading an image */
...
#define MudflapRegister(ptr, size, location)                                   \
  __mf_register(ptr, size, __MF_TYPE_GUESS, location)
...
void ReadDefectiveSectorFile(DefectiveSectorHeader *, struct _RawBuffer *,
                             char *);
...
#define MAX_RAW_TRANSFER_SIZE                                                  \
  (2352 + 296) /* main channel plus C2 vector and mask */
...
typedef struct _RawBuffer {
  ...
  int recommendedAttempts;        /* number of retries recommended by reading
                           heuristics */

  unsigned char *recovered; /* working buffer for cd frame recovery */
  unsigned char *byteState; /* state of error correction */
  unsigned char
      *reference; /* NULL or the correct sector (for debugging purposes) */
  unsigned char *byteCount; /* stores how many different bytes were read for
...
void EncodeNextLayer(ReedSolomonTables *, unsigned char *, unsigned char *,
                     guint64, int);
int ProbeSSE2(void);
