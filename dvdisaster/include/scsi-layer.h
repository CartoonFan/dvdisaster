...
typedef struct _DeviceHandle {
  ...
  int canReadDefective;     /* TRUE if drive claims to raw read uncorrectable
                     sectors */
  int canC2Scan;            /* TRUE if drive supports C2 error scanning */
  int c2[MAX_CLUSTER_SIZE]; /* C2 errors per sector */
  unsigned char
      previousReadMode;          /* read mode prior to switching to raw reads */
...
int SendPacket(DeviceHandle *, unsigned char *, int, unsigned char *, int,
               Sense *, int);
...
enum { MODE_PAGE_UNSET, MODE_PAGE_SET };
